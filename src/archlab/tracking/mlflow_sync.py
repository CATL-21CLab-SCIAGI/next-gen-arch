...
def historical_full_evaluation(directory, variant):
    ...
    math_result = json.loads((root/'HELDOUT_MATH.json').read_text())['overall']['variants'][variant]
...
def scheduled_full_evaluations(directory, variant):
    ...
    for reference in sorted(Path(directory).glob(f'{variant}-step-*.json')):
        item=json.loads(reference.read_text());root=Path(item['result_directory'])
        if item['variant']!=variant:raise ValueError('wrong scheduled evaluation variant')
        ...
        step=item['cursor']['step'];tokens=item['cursor']['supervised_tokens']
        values=json.loads((root/'HELDOUT_MATH.json').read_text())['overall']['variants'][variant]
...
def run_params(spec, contract):
    ...
    return {'variant':spec['variant'],'phase':spec['phase'],'gpu_type':'NVIDIA B300',
            ...
            'dataset':spec['dataset'],'eval_targets':evaluation.get('targets'),
            'eval_interval_tokens':evaluation.get('interval_tokens'),'eval_plan_sha256':evaluation.get('plan_sha256'),
...
class Sync:
    ...
    def ensure_run(self,spec,contract):
        ...
        if local:
            ...
            if local['source_commit']!=contract['project_commit']:
                ...
                if (transition.get('from_commit')!=local['source_commit'] or transition.get('to_commit')!=contract['project_commit']
                        or transition.get('training_math_unchanged') is not True
                        or transition['cursor']['step'] < (local['last_step'] or 0)):
                    raise ValueError('Existing tracking run source changed without a qualified continuation')
                history=[*local.get('source_history',[]),local['source_commit']]
                self.client.log_batch(local['run_id'],tags=[RunTag('archlab.current_source_commit',contract['project_commit']),RunTag('archlab.prior_source_commits',json.dumps(history))])
                local.update(source_commit=contract['project_commit'],source_history=history);self.save()
            return local
    ...
    def sync_run(self,spec):
        ...
        for first in range(0,len(diagnostic_pending),60):
            ...
            metrics = [Metric(k,float(v),local['history_timestamp_ms'],r['step'])
                       for r in group for k,v in diagnostic_values(r).items()]
            if metrics:
                self.client.log_batch(run_id,metrics=metrics,synchronous=True)
            local['diagnostics_last_step'] = group[-1]['step'];self.save()
        aux=[];aux_times={};validation=root/'validation.jsonl'
        if validation.exists():
            mapping={r['step']:r['consumed_supervised_tokens'] for r in rows}
            for r in read_jsonl(validation):
                if spec['phase'] == 'full-finetuning':
                    observed = local.setdefault('validation_timestamps',{}).setdefault(str(r['step']),int(time.time()*1000))
                    for key in ('cross_entropy','perplexity','top1_token_accuracy','top5_token_accuracy','mean_predictive_entropy','targets','seconds'):
                        aux.append(('eval/math64k/'+key,r[key],r['step']))
                        aux_times[('eval/math64k/'+key,r['step'])] = observed
                    aux.append(('eval/math64k/cross_entropy_by_tokens',r['loss'],r['consumed_supervised_tokens']))
                    aux_times[('eval/math64k/cross_entropy_by_tokens',r['consumed_supervised_tokens'])] = observed
                else:
                    ...
                    aux.extend([('eval/fineweb_cross_entropy',r['loss'],r['step']),('eval/fineweb_cross_entropy_by_tokens',r['loss'],tokens)])
        if spec.get('historical_full_evaluation'):
            aux.extend(historical_full_evaluation(spec['historical_full_evaluation'],spec['variant']))
        ...
        if aux:
            ...
            if local['aux_fingerprints'].get('evaluation')!=digest:
                ...
                for first in range(0,len(aux),900):
                    self.client.log_batch(run_id,metrics=[Metric(k,float(v),aux_times.get((k,step),local['history_timestamp_ms']),int(step)) for k,v,step in aux[first:first+900]])
                local['aux_fingerprints']['evaluation']=digest;self.save()
        ...
        final_checkpoint = last['consumed_supervised_tokens'] >= spec['budget'] and (latest_dir/'COMPLETE.json').exists()
        ...
        activity=json.loads((root/'ACTIVITY.json').read_text()) if (root/'ACTIVITY.json').exists() else {}
        if phase not in ('failed','complete','paused') and time.time()-activity.get('heartbeat_unix',0)<180 and activity.get('phase') in ('restoring','evaluating','chat','running'):
            phase=activity['phase']
        elif phase == 'running' and contract.get('periodic_evaluation') and not read_jsonl(root/'train-metrics.jsonl'):
            phase = 'evaluating' if (root/'RESUME.json').exists() else 'restoring'
        elif phase == 'running' and latest_dir.exists() and not (latest_dir/'COMPLETE.json').exists():
            phase = 'checkpointing'
        elif phase == 'running' and (root/'STOP_REQUEST').exists() and (latest_dir/'COMPLETE.json').exists():
            phase = 'paused'
        elif phase == 'running' and age > max(600,5*last.get('wall_seconds',last['seconds'])):
            phase = 'stalled'
        status='FAILED' if phase=='failed' else ('FINISHED' if phase in ('paused','complete') else 'RUNNING')
        tags.update({'archlab.state':phase,'archlab.source_age_seconds':str(age),
                     ...
                     'archlab.eval_schedule':json.dumps(contract.get('periodic_evaluation',{'enabled':False}),sort_keys=True)})
        if spec.get('serving_path'):
            serving_failed=any(Path(spec['serving_path']).glob('*failure.json'))
            tags['serving.state']='failed' if serving_failed else (activity.get('phase','unknown') if time.time()-activity.get('heartbeat_unix',0)<180 else 'stalled')
