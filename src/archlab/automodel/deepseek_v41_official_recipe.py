...
from archlab.automodel.deepseek_v41_official_execution import MODEL_REVISION, UPSTREAM_COMMIT, official_core_hashes
...
class OfficialV41Config:
    ...
    def __post_init__(self):
        ...
        if self.backend != {
            "implementation": "official-automodel-deepseek-v41", "upstream_commit": UPSTREAM_COMMIT,
            "dense_fsdp": 32, "expert_fsdp": 4, "engram_owners": 32,
            "attention": "tilelang", "sinkhorn": "native-tilelang", "experts": "torch_mm",
            "dispatcher": "torch", "linear_compute": "bfloat16",
            ...
            "adapter_core": ("deterministic-triton-private-window-gradients" if self.adapter_variant == "simplicial"
                             else "flash-attn-deterministic-bf16-local512"),
            "expert_gather_backward": "private-clone-before-allreduce",
            "kv_index_quantization": "retained", "parity_reference": "released-structure-matching-bf16-compute",
        }:
            raise ValueError("official backend does not match the reviewed migration contract")
        for name in ("world_size", "ep_size", "context", "supervised_tokens",
                     "validation_tokens", "validation_interval_tokens", "checkpoint_interval_tokens",
                     "warmup_steps", "query_chunk"):
...
def admit_mesh(path, hashes, *, runtime=None, adapter_variant="simplicial"):
    ...
    expected_backend = "deterministic" if adapter_variant == "simplicial" else "flash-attn-deterministic"
    ...
    if (not marker.get("passed") or marker.get("world_size") != 32
    ...
    for rank in range(32):
        ...
        if (not receipt.get("passed") or receipt["rank"] != rank
        ...
        expected_topology = {"world_size": 32, "ep_size": 8, "expert_fsdp_size": 4, "engram_owners": 32}
        ...
        if (mesh.get("ep_ranks") != ep_ranks or len(mesh.get("ep_hosts", [])) != 8
        ...
        if (not receipt.get("unique_rank_windows")
            ...
            raise ValueError(f"incomplete update/reload checks in official mesh receipt: {receipt_path}")
        updates = receipt.get("updates", [])
        if len(updates) < 2 or any(
            not math.isfinite(update.get("loss", math.nan)) or
            not all(update.get(key) for key in ("replicated_fp32_masters", "world_gradient_agreement",
...
def run(args):
    ...
    from archlab.automodel.deepseek_v41_official_execution import (
        build_official_base, configure_official_reproducibility, runtime_identity,
    )
    from archlab.automodel.deepseek_v41_official_qualification import prepare_official_reference, qualify_official_forward
    from archlab.automodel.deepseek_v41_official_training import REPLAY_PROTOCOL, OfficialV41TrainingSession
    ...
    try:
        ...
        mesh_receipts = admit_mesh(resolve_path(config.mesh_qualification), hashes, runtime=runtime_identity(),
        ...
        output_created = True
        train = MathPilot(resolve_path(config.train_data), expected_split="train", expected_budget=config.supervised_tokens)
        validation = MathPilot(resolve_path(config.validation_data), expected_split="validation",
        ...
        prepared_reference = prepare_official_reference(
            assets=resolve_path(config.assets), weights=resolve_path(config.weights),
            pilot=train, output=output / "forward-qualification")
        model, setup, loading = build_official_base(
            weights=resolve_path(config.weights), assets=resolve_path(config.assets), ep_size=config.ep_size)
        atomic_write_json(output / f"rank-{dist.get_rank():02d}-loading.json", loading, allow_nan=False)
        parity, baseline = qualify_official_forward(
            model, setup, pilot=train, output=output / "forward-qualification",
            prepared_reference=prepared_reference)
        del prepared_reference
        adapter_backend = "deterministic" if config.adapter_variant == "simplicial" else "flash-attn-deterministic"
        adapters = install_official_adapters(model, device="cuda", backend=adapter_backend,
                                             variant=config.adapter_variant)
        inputs, _, _ = train.batch(dist.get_rank(), device="cuda", smoke_context=128, pad_to_full=True)
        with torch.no_grad():
            zero = model(input_ids=inputs, return_hidden_states=True).hidden_states
        identical = torch.tensor(int(torch.equal(zero.cpu(), baseline)), device="cuda", dtype=torch.int32)
        ...
        contract = {
            "schema": "archlab-official-v41-adapters-v1", "model_revision": MODEL_REVISION,
            "checkpoint_index_sha256": sha256_file(resolve_path(config.weights) / "model.safetensors.index.json"),
            "base_config_sha256": sha256_file(resolve_path(config.weights) / "config.json"),
            "reference_config_sha256": sha256_file(resolve_path(config.assets) / "inference/config.json"),
            "project_commit": commit, "implementation_sha256": hashes,
            "upstream_commit": UPSTREAM_COMMIT, "backend": config.backend,
            "recipe_sha256": sha256_file(args.recipe),
            "training": {key: value for key, value in asdict(config).items()
                         if isinstance(value, int)},
            "train_manifest_sha256": sha256_file(resolve_path(config.train_data) / "PILOT_READY.json"),
            "validation_manifest_sha256": sha256_file(resolve_path(config.validation_data) / "PILOT_READY.json"),
            "data_order_seed": 2234,
            "adapter_variant": config.adapter_variant,
            "adapter_config": asdict(next(iter(adapters.values())).config),
            "adapter_layers_0based": list(adapters),
            "trainable_parameters": sum(p.numel() for a in adapters.values() for p in a.parameters()),
            "container_image": loading["container_image"], "runtime": loading["packages"],
            "cuda": loading["cuda"], "nccl": loading["nccl"],
            "reproducibility": loading["reproducibility"],
            "optimizer": "headwise-Muon-QK/matrix-Muon/AdamW-nonmatrix",
            "forward_qualification": {"contexts": [128, 2048, 16384], "kl_limit": .02, "ce_delta_limit": .05},
        ...
        if dist.get_rank() == 0:
            ...
            atomic_write_json(output / "RUN_PROVENANCE.json", {
                "source_root": str(source_root), "recipe": str(args.recipe.resolve()),
                "mesh_qualification": str(resolve_path(config.mesh_qualification)),
                "mesh_receipts_sha256": mesh_receipts, "kernel_packages": kernels,
        ...
        session = OfficialV41TrainingSession(model, adapters, train, validation, output, contract, config)
        report = session.qualify_mesh()
        atomic_write_json(output / f"rank-{dist.get_rank():02d}-training-qualification.json", report, allow_nan=False)
        dist.barrier()
        if dist.get_rank() == 0:
