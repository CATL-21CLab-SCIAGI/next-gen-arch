...
def main():
    ...
    engine = LocalV41Inference(
        ...
        context=max(2048, *args.contexts),
        max_batch_size=8,
        expert_cache_gib=96,
    )
    ...
    for context in args.contexts:
        ...
        atomic_write_json(args.output / "PROGRESS.json", report, allow_nan=False)
        print(json.dumps({"event": "local_forward_qualified", **item}), flush=True)
        if not all(r["passed"] for r in actual):
            raise ValueError("local base CE differs from qualified released reference")
    report["passed"] = True
    atomic_write_json(args.output / "COMPLETE.json", report, allow_nan=False)
