...
def prepare_prefix_fixture(*, assets: Path, weights: Path, output: Path, layers: int = 3):
    ...
    for name, shard in mapping.items():
        ...
        if name in ("embed.weight", "head.weight", "norm.weight") or (
    ...
    if output.exists() and any(output.iterdir()):
        ...
        if (
            existing["parent_index_sha256"] != original_sha
            or existing["layers"] != layers
            or existing["parent_checkpoint"] != str(weights)
            or existing["parent_assets"] != str(assets)
        ):
    ...
    index_bytes = (
        json.dumps(
            {"metadata": {"archlab_partial_fixture": True}, "weight_map": selected},
            sort_keys=True,
            indent=2,
        )
        + "\n"
    ...
    for source in assets.iterdir():
        if (
            ...
            not in (
                ...
                "ARCHLAB_VERIFIED_COPY.json",
                "PREFIX_DERIVATION.json",
            )
        ):
    ...
    generated = {
        ...
        for name in ("model.safetensors.index.json", "config.json", "inference/config.json")
    }
    receipt = {
        "kind": "derived-real-weight-prefix-fixture-only",
        "partial_fixture": True,
        "layers": layers,
        "original_layers": original_layers,
        "parent_checkpoint": str(weights),
        "parent_assets": str(assets),
        ...
        "original_tensors": len(mapping),
        "generated_sha256": generated,
        "symlinks": links,
        "source_weight_files_modified": False,
        "production_qualification": False,
    }
    (output / "PREFIX_DERIVATION.json").write_text(
    ...
    (output / "ARCHLAB_VERIFIED_COPY.json").write_text(
        json.dumps(
            {
        ...
        if official:
            for site in ("attn", "ffn"):
                ...
                observe_method(
                    connection,
                    "collapse",
                    lambda args, value, k=key, s=site: save(k + "." + s + "_collapsed", value),
                )
                observe_method(
                    connection,
                    "expand",
                    lambda args, value, k=key, s=site: save(k + "." + s + "_streams", value),
                )
        else:
...
def compare_snapshots(reference, actual):
    ...
    for key in reference.keys() & actual.keys():
        ...
        row = {
            ...
            "official_dtype": str(right.dtype),
        }
        ...
        if key.endswith(".indices") and left.shape == right.shape:
            row["same_selected_expert_set"] = bool(
                torch.equal(left.sort(-1).values, right.sort(-1).values)
            )
        rows.append(row)
    return {
        "comparisons": sorted(rows, key=lambda row: row["boundary"]),
        "reference_execution_order": list(reference),
        "official_execution_order": list(actual),
...
def run(args):
    ...
    rank = dist.get_rank()
    report = {
        "kind": "three-layer-real-weight-prefix-diagnostic",
        "production_qualification": False,
        "rank": rank,
        "kernels": kernels,
        "completed": False,
    ...
    try:
        ...
        if rank == 0:
            prepare_prefix_fixture(
                assets=args.assets, weights=args.weights, output=fixture, layers=args.layers
            )
        dist.barrier()
        ...
        with torch.no_grad(), capture_prefix(model, official=True) as actual:
            actual_hidden = (
                model(input_ids=inputs, return_hidden_states=True)
        ...
        reference, native, report["reference_loading"] = build_matching_precision_reference(
            assets=fixture,
            weights=fixture,
            expert_group=setup.mesh_context.moe_mesh["ep"].get_group(),
            engram_group=dist.group.WORLD,
            context=128,
            module_name="_archlab_v41_prefix_reference",
        )
        ...
        if args.save_tensors:
            torch.save(
                {"native": expected, "official": actual}, args.output / f"rank{rank}-boundaries.pt"
            )
