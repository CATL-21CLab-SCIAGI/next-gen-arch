...
def _compare(actual, expected):
    ...
    return {
        ...
        "differing_elements": int(delta.count_nonzero()),
...
def main():
    ...
    for rank in range(args.first_rank, args.first_rank + args.ep_size):
        ...
        for kind in captures:
            captures[kind].append(
                {
                    name: saved[kind][f"{layer}.{name}"].clone()
                    for name in (
                        "input",
                        "output",
                        "gate.weights",
    ...
    for kind, rows in captures.items():
        ...
        for key in rows[0]:
            trailing = (
                width
                if key in ("input", "output", "shared_experts.output")
                else rows[0][key].shape[-1]
            )
    ...
    active = sorted(
        set(data["native"]["gate.indices"].flatten().tolist())
        | set(data["official"]["gate.indices"].flatten().tolist())
    )
    report = {
        "kind": "real-layer-native-expert-shared-rounding-diagnostic",
    ...
    expert = (
        native.Expert(
            width, intermediate, dtype=torch.bfloat16, swiglu_limit=config["swiglu_limit"]
        )
        .cuda()
    ...
    if args.official_leaf:
        ...
        moe_config = MoEConfig(
            ...
            norm_topk_prob=native_args.norm_topk_prob,
            swiglu_limit=config["swiglu_limit"],
            dtype=torch.bfloat16,
        )
        with torch.device("meta"):
            official_leaf = MoE(
                moe_config, BackendConfig(experts="torch_mm", dispatcher="torch", linear="torch")
            )
    ...
    routed = {
        kind: torch.zeros(
            args.ep_size, data[kind]["input"].shape[0], width, device="cuda", dtype=torch.float32
        )
        for kind in data
    }
    source_tensors = []
    with ExitStack() as stack:
        ...
        def tensor(name):
            ...
            if shard not in readers:
                readers[shard] = stack.enter_context(
                    safe_open(args.weights / shard, framework="pt", device="cpu")
                )
        ...
        def load_expert(prefix):
            for projection in ("w1", "w3", "w2"):
                ...
                for start in range(0, weight.shape[0], 1024):
                    ...
                    factors = (
                        scale[start // 32 : (end + 31) // 32]
                        if weight.dtype == torch.float8_e4m3fn
                        else scale[start:end]
                    )
        ...
        for offset, expert_id in enumerate(active):
            ...
            for kind in data:
                ...
                if token_ids.numel():
                    output = expert(
                        case["input"][token_ids], case["gate.weights"][token_ids, slots, None]
                    )
                    routed[kind][owner, token_ids] += output.float()
            if (offset + 1) % 32 == 0:
                print(
                    json.dumps(
                        {"event": "expert_replayed", "count": offset + 1, "total": len(active)}
                    ),
                    flush=True,
                )
        ...
        for kind, case in data.items():
            ...
            compared = {
                "shared_replay_vs_saved": _compare(shared, case["shared_experts.output"]),
            ...
            first = 0
            for rank, length in zip(
                range(args.first_rank, args.first_rank + args.ep_size), case["lengths"], strict=True
            ):
                section = slice(first, first + length)
                compared["by_rank"].append(
                    {
                        "rank": rank,
                        "shared_replay_vs_saved": _compare(
                            shared[section], case["shared_experts.output"][section]
                        ),
                        "late_sum_vs_saved": _compare(late[section], case["output"][section]),
                        "early_sum_vs_saved": _compare(early[section], case["output"][section]),
                        "owner_rounded_late_shared_vs_saved": _compare(
                            owner_late[section], case["output"][section]
                        ),
                        "owner_rounded_early_shared_vs_saved": _compare(
                            owner_early[section], case["output"][section]
                        ),
                        "early_vs_late_same_expert_outputs": _compare(
            ...
            torch.save(
                {
                    "routed_fp32": summed.cpu(),
                    "per_owner_fp32": routed[kind].cpu(),
                    "shared_bf16": shared.cpu(),
                    "late_sum_bf16": late.cpu(),
                    "early_sum_bf16": early.cpu(),
    ...
    report.update(
        ...
        source_shards=sorted(readers),
    )
    atomic_write_json(args.output / "REPORT.json", report, allow_nan=False)
    print(
        json.dumps(
            {
                "event": "moe_rounding_replay_complete",
                "seconds": report["seconds"],
                "cases": {kind: values["by_rank"][0] for kind, values in report["cases"].items()},
            }
        ),
        flush=True,
    )
