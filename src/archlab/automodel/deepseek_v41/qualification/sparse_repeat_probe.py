...
def main():
    ...
    for name, fn in (
        ...
        for repeat in range(args.repeats):
            ...
            y = fn(x, k, sink, idx, args.dim**-0.5, backend="tilelang", reference_rounding=True)
            y.backward(do)
            ...
            records.append(
                {
                    ...
                    **{
                        field: _compare(value, expected)
                        for field, value, expected in zip(
                            ("output", "dq", "dkv"), current, outputs[name], strict=True
                        )
                    },
                }
            )
    ...
    report["comparison"] = {
        field: _compare(actual, expected)
        for field, actual, expected in zip(
            ("output", "dq", "dkv"), outputs["deterministic"], outputs["official"], strict=True
        )
    }
    report["passed"] = (
        report["comparison"]["output"]["equal"]
        and all(value["relative_l2"] < 0.01 for value in report["comparison"].values())
        and all(
            row[key]["equal"]
            for row in report["repeats"][1]["results"]
            for key in ("output", "dq", "dkv")
        )
    )
