...
def _inspect_stages(model, args):
    ...
    native_up, native_act, native_down = (
        torch.empty_like(v) for v in (grouped_up, grouped_act, grouped_down)
    )
    ...
    for e, count in enumerate(counts.tolist()):
        ...
        if count:
            ...
            native_act[first:last] = expert.expert_activation_grouped(
                native_up[first:last], probs[first:last, None]
            )
    ...
    selected_x = x[ids]
    split_rows = torch.cat(
        [torch._grouped_mm(selected_x, weight.contiguous(), offs) for weight in up.chunk(2, -1)], -1
    )
    split_columns = torch.cat(
        [
            torch._grouped_mm(
                selected_x, weight.transpose(-2, -1).contiguous().transpose(-2, -1), offs
            )
            for weight in up.chunk(2, -1)
        ],
        -1,
    )
    atomic = torch.zeros_like(x, dtype=torch.float32).scatter_add_(
        0, ids[:, None].expand_as(native_down), native_down.float()
    )
    ...
    for e, count in enumerate(counts.tolist()):
        ...
        if count:
            ...
            owners[e // (expert.n_routed_experts // 8), ids[first:last]] += native_down[
                first:last
            ].float()
        first = last
    shared = model.shared_experts(x)
    report = {
        "event": "expert_precision_stages",
        ...
        "atomic_vs_ordered_final": _compare(
            (atomic + shared.float()).bfloat16(), (ordered + shared.float()).bfloat16()
        ),
        "ordered_vs_owner_final": _compare(
            (ordered + shared.float()).bfloat16(), (owners.sum(0) + shared.float()).bfloat16()
        ),
    }
...
def main():
    ...
    from archlab.automodel.deepseek_v41.qualification.moe_rounding_probe import main as replay
    ...
    def install_eager_activation(model):
        ...
        for module in model.modules():
            ...
            module.expert_activation_grouped = partial(
                body, *activation.args, **activation.keywords
            )
