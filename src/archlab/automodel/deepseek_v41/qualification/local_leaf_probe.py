...
def main():
    ...
    for name in ("assets", "weights", "boundaries", "output"):
    ...
    install_native_row_padding(native)
    native.linear = F.linear
    native.default_dtype = torch.bfloat16
    args = native.ModelArgs(**json.loads((a.assets / "inference/config.json").read_text()))
    args.max_batch_size = 8
    args.max_seq_len = 128
    args.vision_n_layers = 0
    saved = [
        torch.load(
            a.boundaries / f"rank{i}-boundaries.pt",
            map_location="cpu",
            mmap=True,
            weights_only=True,
        )["native"]
    ...
    mapping = json.loads((a.weights / "model.safetensors.index.json").read_text())["weight_map"]
    files = {}

    def tensor(name):
        filename = mapping[name]
        if filename not in files:
            files[filename] = safe_open(a.weights / filename, framework="pt", device="cpu")
    ...
    def measure(x, y):
        ...
        return {
            ...
            "relative_l2": float(delta.norm() / y.float().norm()),
            "dtype": str(x.dtype),
        }
    ...
    expected_mix = [
        torch.cat([r["layers.0.attn_mix." + name] for r in saved]).cuda()
        for name in ("pre", "post", "comb")
    ]
    gate = native.Gate(0, args).cuda()
    for name, _ in gate.named_parameters():
        gate._parameters[name] = torch.nn.Parameter(
            tensor("layers.0.ffn.gate." + name), requires_grad=False
        )
    gx = torch.cat([r["layers.0.ffn.gate.input"] for r in saved]).cuda()
    gw = torch.cat([r["layers.0.ffn.gate.weights"] for r in saved]).cuda()
    gi = torch.cat([r["layers.0.ffn.gate.indices"] for r in saved]).cuda()
    results = {}
    for tf32 in (False, True):
        ...
        for split in (False, True):
            ...
            if split:
                ...
                gp = [gate(part) for part in gx.split(128)]
                weights = torch.cat([v[0] for v in gp])
                indices = torch.cat([v[1] for v in gp])
            else:
                mixes = native.Block.hc_mixes(dummy, x, fn, scale, base)
                weights, indices = gate(gx)
            results[label] = {
                "mhc": [measure(v, e) for v, e in zip(mixes, expected_mix, strict=True)],
                "gate_weights": measure(weights, gw),
                "gate_indices_equal": torch.equal(indices, gi),
            }
    with native.set_dtype(torch.bfloat16), torch.device("cpu"):
        attn = native.Attention(0, args).requires_grad_(False)
    ...
    for module in attn.modules():
        for name, buffer in list(module.named_buffers(recurse=False)):
            module._buffers[name] = buffer.cuda()
    ax = torch.cat([r["layers.0.attn.input"] for r in saved]).cuda()
    expected = torch.cat([r["layers.0.attn.output"] for r in saved]).cuda()
    ...
    with torch.device("cuda"):
        gpu_freq = native.precompute_freqs_cis(
            args.rope_head_dim,
            128,
            0,
            args.rope_theta,
        ...
        for split in (False, True):
            for device, freq in (("cpu", cpu_freq), ("gpu", gpu_freq)):
                ...
                with native.set_dtype(torch.bfloat16), torch.device("cuda"), torch.inference_mode():
                    value = (
                        torch.cat([attn(part, 0) for part in ax.split(1)]) if split else attn(ax, 0)
                    )
