...
def main():
    ...
    report = {"kind": "native-fp4-inplace-production-shape-repeatability", "training_launched": False,
              "torch": str(torch.__version__), "cuda": torch.version.cuda,
              "container_image": os.environ.get("NGA_CONTAINER_DIGEST"), "kernel_packages": packages,
              "quantization_bridge": bridge,
              "probe_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
              "reference_kernel_sha256": hashlib.sha256((args.assets / "inference/kernel.py").read_bytes()).hexdigest(),
              "cases": []}
    ...
    with torch.inference_mode():
        for block_size, scale_dtype in ((16, torch.float8_e4m3fn), (32, torch.float8_e8m0fnu)):
            ...
            x[:, :32] = 0
            expected = rounded_activation(x, bits=4, block_size=block_size,
                                          e4m3_scale=scale_dtype == torch.float8_e4m3fn,
                                          straight_through=False)
            case = {"shape": list(x.shape), "block_size": block_size,
                    "scale_dtype": str(scale_dtype), "repeats": []}
            first = None
            for repeat in range(6):
                ...
                item = {"repeat": repeat, "inplace_alias_preserved": returned is actual,
                        "finite": finite, "nonfinite_count": int((~actual.isfinite()).sum()),
                ...
                if finite:
                    ...
                    item.update(max_abs=float(error.abs().max()),
                                relative_l2=float(error.norm() / expected.float().norm()))
                if first is None:
                    first = actual.clone()
                case["repeats"].append(item)
                print(json.dumps({"block_size": block_size, "scale_dtype": str(scale_dtype), **item}, allow_nan=False), flush=True)
                del actual, returned
            case["passed"] = all(item["finite"] and item["oracle_equal"] and item["repeat_equal"]
                                 and item["inplace_alias_preserved"] for item in case["repeats"])
