...
def select_cases(records, *, seed=20260923, mmlu_per_subject=2, arc_count=32, piqa_count=64):
    ...
    available = [row for row in records if row.get('exact_overlap_consumed_training') is False
    ...
    for subject in subjects:
        group = sorted((row for row in available if row['task'] == 'mmlu' and row['subject'] == subject), key=lambda r:r['id'])
    ...
    for task in sorted({row['task'] for row in records}):
        ...
        for variant in ('normal', 'simplicial'):
            ...
            for metric in ('accuracy', 'accuracy_norm'):
                comparison = paired_statistics([row['original'][metric] for row in rows],
...
def training_ready(paths):
    ...
    for path in paths:
        ...
        required = [root/name for name in ('TRAINING_ADMITTED.json', 'REAL_UPDATE_VERIFIED.json', 'MEMORY_QUALIFICATION.json')]
        ...
        receipts.append({'path':str(root), 'optimizer_step':real['optimizer_step'],
...
def main():
    ...
    parser.add_argument('--memory-budget-gib', type=float, default=56.)
    ...
    if len(args.training_run) != 2 or not 0 < args.memory_budget_gib <= 56 or args.wait_hours <= 0:
        parser.error('provide both actors, a positive evaluator budget <=56 GiB, and a positive wait limit')
    ...
    if (complete.get('passed') is not True or recorded.get('matched_steps') is not True
            or any(recorded.get('cursors', {}).get(v, {}).get('step') != 4537 for v in ('normal', 'simplicial'))):
        raise ValueError('reference predictions must belong to the complete matched step-4537 evaluation')
    ...
    if original_config.get('archlab') or (args.weights/'DIRECT_CHECKPOINT.json').exists():
        raise ValueError('baseline requires the released original checkpoint, not a fine-tuned export')
    selected = select_cases(records, seed=args.seed, mmlu_per_subject=args.mmlu_per_subject,
                            arc_count=args.arc_count, piqa_count=args.piqa_count)
    prompts = Path(__file__).resolve().parents[1]/'prompts/capability_regression.yaml'
    provenance = {'reference':str(args.reference), 'reference_sha256':sha256_file(args.reference),
                  'reference_run_sha256':sha256_file(reference_run),
                  'reference_checkpoints':recorded['checkpoint_paths'],
                  'prompts_sha256':sha256_file(prompts), 'seed':args.seed,
                  'case_ids':[row['id'] for row in selected], 'counts':dict(Counter(row['task'] for row in selected)),
                  'selection_uses_scores':False, 'comparison_checkpoint_step':4537,
                  'scoring_mode':'existing-choice-likelihood-no-generation',
                  'reference_scope':'matched adapted SFT parents; not a new RL checkpoint',
                  'original_weights':str(args.weights), 'original_config_sha256':sha256_file(args.weights/'config.json'),
    ...
    provenance.update(jobs_digest=jobs_digest(jobs), inference_jobs=len(jobs), maximum_input_tokens=longest)
    ...
    while (admission := training_ready(args.training_run)) is None:
        if time.monotonic() >= deadline:
            raise TimeoutError('training did not reach the required real-update and memory admission')
        time.sleep(10)
    ...
    try:
        ...
        model, setup, loading = build_official_base(weights=args.weights, assets=args.assets,
            ep_size=8, activation_checkpointing=False, tiny=args.tiny)
        model.eval()
        if any('simplicial_adapter' in name or p.requires_grad for name,p in model.named_parameters()):
            raise ValueError('original baseline must have no adapter and no trainable parameter')
        provenance.update(runtime=loading, resolved_kernel_packages=packages, allocator=budget,
            project_commit=subprocess.check_output(['git','-C',str(Path(__file__).resolve().parents[3]),'rev-parse','HEAD'],text=True).strip(),
            training_admission=admission,
            adapter_present=False, optimizer_created=False, tiny=args.tiny, shared_gpu=True)
        if rank == 0:
            ...
            if rank == 0:
                for packet in packets:
                    ...
                    if row['id'] not in completed and None not in scores[row['id']]:
                        case = lookup[row['id']]
                        completed[row['id']] = {**case, 'original_scores':scores[row['id']],
                                               'original':score_choice_vector(case,scores[row['id']])}
                activity.set('scoring', rounds_done=round_index+1, rounds_total=rounds,
                             cases_complete=len(completed), cases_total=len(selected))
        peak = {'rank':rank,'allocated_gib':torch.cuda.max_memory_allocated()/2**30,
        ...
        if rank == 0:
            ...
            if not args.tiny:
                ...
                (args.output/'predictions.jsonl').write_text(''.join(json.dumps(row,ensure_ascii=False)+'\n' for row in ordered))
                atomic_write_json(args.output/'COMPARISON.json', summarize(ordered), allow_nan=False)
            atomic_write_json(args.output/'COMPLETE.json', {'passed':True, 'tiny':args.tiny,
                'cases':len(completed), 'optimizer_updates':0, 'memory':peaks,
                'scope':'small paired regression screen; not a proof of general-task equivalence'}, allow_nan=False)
            activity.set('complete', tiny=args.tiny)
        del model, setup
    except BaseException:
        if owned:
            atomic_write_json(args.output/f'rank-{rank:02d}-failure.json', {'traceback':traceback.format_exc()})
        raise
    finally:
        if activity:
            activity.close()
