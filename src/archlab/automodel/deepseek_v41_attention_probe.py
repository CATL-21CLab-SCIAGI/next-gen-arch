...
def main():
    ...
    from archlab.architectures.deepseek_v41_torch import query_chunked_sparse_attention, rounded_activation
    ...
    for block_size in (32, 128):
        for scale_fmt, scale_dtype in ((None, torch.float32), ("ue8m0", torch.float8_e8m0fnu)):
            known = torch.arange(32 * 128, device="cuda", dtype=torch.float32).sin().bfloat16().reshape(32, 128)
            ...
            record = {"block_size": block_size, "scale_fmt": scale_fmt, "scale_dtype": str(scale_dtype),
                      ...
                      "scales_equal": torch.equal(original_scales, stable_scales)
                          if scale_dtype == torch.float32 else torch.equal(original_scales.view(torch.uint8), stable_scales.view(torch.uint8))}
    ...
    def read(name):
        with torch.device("cpu"), safe_open(args.weights / mapping[name], framework="pt", device="cpu") as reader:
    ...
    data = MathPilot(args.pilot, expected_split="train", expected_budget=1_000_000_000, order_seed=2234)
    weights = {id(module.weight): name for name, module in attention.named_modules()
    ...
    native.linear, native.act_quant, native.sparse_attn = observe_linear, observe_quantize, observe_sparse
    handles = [attention.q_norm.register_forward_hook(lambda module, values, out: capture("q_norm.output", out)),
               attention.kv_norm.register_forward_hook(lambda module, values, out: capture("kv_norm.output", out))]
    ...
    def describe(tensor):
        ...
        return {"shape": list(tensor.shape), "dtype": str(tensor.dtype),
                ...
                "finite_l2": float(clean.norm()), "finite_max_abs": float(clean.abs().max())}

    def compare(actual, expected):
        a, b = actual.float(), expected.float()
        valid = a.isfinite() & b.isfinite()
        difference = torch.where(valid, a - b, 0)
        return {"equal": torch.equal(a, b), "both_finite": bool(valid.all()),
                "same_nonfinite_pattern": bool(torch.equal(a.isnan(), b.isnan())),
                "finite_pair_max_abs": float(difference.abs().max()),
                "finite_pair_relative_l2": float(difference.norm() / b.masked_fill(~valid, 0).norm().clamp_min(1e-30))}

    report = {"kind": "actual-first-attention-replay", "training_launched": False,
              "torch": str(torch.__version__), "container_image": os.environ.get("NGA_CONTAINER_DIGEST"),
              "kernel_packages": packages, "order_seed": 2234,
              "quantization_bridge": quantization_bridge, "clean_native_quantizers": clean_quantizers,
              "probe_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), "cases": []}
    with torch.no_grad():
        for sample_id, context in ((1, 128), (0, 2048)):
            inputs, _, _ = data.batch(sample_id, device="cpu", smoke_context=context, pad_to_full=True)
            with torch.device("cpu"), safe_open(args.weights / mapping["embed.weight"], framework="pt", device="cpu") as reader:
                embedded = torch.nn.functional.embedding(inputs, reader.get_tensor("embed.weight")).cuda()
            # Identity pre-mix collapses identical embedding streams exactly.
            x = norm(embedded)
            first, case = None, {"sample_id": sample_id, "context": context, "input": describe(x), "passes": []}
            for repeat in range(4):
                ...
                if first is None:
                    first = captures
                stage["first_nonfinite"] = next((v["name"] for v in stage["boundaries"] if not v["finite"]), None)
                stage["first_unequal"] = next((v["name"] for v in stage["boundaries"]
                                               if not v.get("comparison", {}).get("equal", True)), None)
                case["passes"].append(stage)
                print(json.dumps({"event": "attention_replay", "sample_id": sample_id, "context": context,
                                  "repeat": repeat, "first_nonfinite": stage["first_nonfinite"],
                                  "first_unequal": stage["first_unequal"]}), flush=True)
            case["linear_oracles"] = {}
            for name in ("wq_a", "wq_b", "wkv", "wo_b"):
                module = getattr(attention, name)
                decoded = dequantize_frozen_weight(module.weight, module.scale)
                expected = torch.nn.functional.linear(rounded_activation(first[name + ".input"]), decoded)
                case["linear_oracles"][name] = compare(first[name + ".output"], expected)
                del decoded, expected
            q, kv, sink, indices = (first["sparse." + name] for name in ("q", "kv", "sink", "indices"))
            expected = query_chunked_sparse_attention(q, kv, sink, indices, attention.softmax_scale,
                                                      native_rounding=True, recompute=False)
            ...
            torch.save({"q": q.cpu(), "kv": kv.cpu(), "sink": sink.cpu(), "indices": indices.cpu(),
                        "native_output": first["sparse.output"].cpu(), "scale": attention.softmax_scale}, repro)
            case["sparse_reproducer"] = str(repro)
            report["cases"].append(case)
            args.output.write_text(json.dumps(report, indent=2, allow_nan=False))
            print(json.dumps({"event": "case_complete", "sample_id": sample_id, "context": context,
                              "linear_oracles": case["linear_oracles"],
                              "sparse_to_torch": case["sparse_to_torch"],
                              "standalone_sparse_repeats": case["standalone_sparse_repeats"]}), flush=True)
            del first, captures, q, kv, sink, indices, expected, actual, output, embedded, x
