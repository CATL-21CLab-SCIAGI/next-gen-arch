...
class _ScoreStorage(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if not isinstance(product, ast.BinOp) or not isinstance(product.op, ast.Mult):
            return node
        if (
            ast.unparse(product.left) != "scores.relu()"
            or ast.unparse(product.right) != "weights.unsqueeze(-1)"
        ):
            raise ValueError("the pinned indexer score expression changed")
        relu = ast.Call(
            func=ast.Attribute(
                value=ast.Name(id="scores", ctx=ast.Load()), attr="relu_", ctx=ast.Load()
            ),
            args=[],
            keywords=[],
        )
        value.func.value = ast.Call(
            func=ast.Attribute(value=relu, attr="mul_", ctx=ast.Load()),
            args=[product.right],
            keywords=[],
        )
        self.replacements += 1
