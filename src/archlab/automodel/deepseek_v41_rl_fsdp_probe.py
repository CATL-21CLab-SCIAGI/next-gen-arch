...
def main():
    ...
    tested_files = ('automodel/deepseek_v41_rl_fsdp_probe.py',
                    'automodel/deepseek_v41_rl_head.py', 'rl/rollout.py',
                    'rl/weight_residency.py', 'automodel/deepseek_v41_rl_update.py')
    tested_source = {name: hashlib.sha256((source_root / name).read_bytes()).hexdigest()
    ...
    try:
        ...
        model.lm_head.set_gradient_divide_factor(1.)
        ...
        rollout = sample_rollouts(model, [[3, 5, 7 + rank], [11, 13, 17]],
            policy_version='synthetic-qualification-only', max_new_tokens=4, context_limit=32,
            eos_token_ids={256}, pad_token_id=0, seed=701, bucket_multiple=8,
            prompt_group_ids=[f'rank-{rank}-first', f'rank-{rank}-second'])
        retained = sample_rollouts(model, [[3, 5, 7 + rank], [11, 13, 17]],
            policy_version='synthetic-qualification-only', max_new_tokens=4, context_limit=32,
            eos_token_ids={256}, pad_token_id=0, seed=701, bucket_multiple=8,
            prompt_group_ids=[f'rank-{rank}-first', f'rank-{rank}-second'], retain_weights=True)
        if retained.generated_ids != rollout.generated_ids:
            raise AssertionError('retained weights changed sampled tokens')
        torch.testing.assert_close(retained.policy_log_probs, rollout.policy_log_probs, atol=0, rtol=0)
        ...
        grouped = sample_rollouts(model, [[3, 5, 7 + rank]] * 4,
            policy_version='prefix-qualification-only', max_new_tokens=4, context_limit=32,
            eos_token_ids={256}, pad_token_id=0, seed=703, bucket_multiple=8, retain_weights=True)
        model._archlab_rl_policy_version = 'prefix-qualification-only'
        optimizer = torch.optim.SGD(model.parameters(), lr=.001)
        audit = policy_gradient_step(model, optimizer, [], grouped,
            torch.tensor([[1., 0., 0., 0.]], device='cuda'), lr=.001, group_size=4,
            audit=True, audit_only=True, replay_mode='sampled-prefix',
            replay_prefixes=4, replay_seed=88)
        ...
        record = {'rank': rank, 'passed': True, 'gradient_reference': 'dense-FP32-global-sum',
                  'signed_coefficients': True, 'masked_targets': True,
                  'rollout_replay_max_abs_error': error, 'rollout': rollout.receipt,
                  'retained_rollout': retained.receipt, 'prefix_gradient_audit': audit}
        records = [None] * world
        dist.all_gather_object(records, record)
        if rank == 0:
            if tested_source != {name: hashlib.sha256((source_root / name).read_bytes()).hexdigest()
                                  for name in tested_files}:
                raise RuntimeError('qualified source changed while probe was running')
            args.output.parent.mkdir(parents=True, exist_ok=True)
            args.output.write_text(json.dumps({'passed': True, 'world_size': world,
                ...
                'numerical_precision': {'allow_tf32': False,
                    'allow_bf16_reduced_precision_reduction': False},
                'container_image': os.environ.get('NGA_CONTAINER_DIGEST'),
                'torch': importlib.metadata.version('torch'), 'cuda': torch.version.cuda,
                'nccl': torch.cuda.nccl.version(), 'ranks': records}, indent=2) + '\n')
