...
SOURCE_FILES = (
    "automodel/deepseek_v41_rl_training.py", "automodel/deepseek_v41_rl_model.py",
    "automodel/deepseek_v41_rl_head.py", "automodel/deepseek_v41_rl_update.py",
...
def validate_recipe(config):
    required = ("assets", "weights", "container_kernel_packages", "data_manifest",
                "data_manifest_sha256", "prompt_template", "parents", "seed",
                "learning_rate", "max_new_tokens", "context_limit",
                "component_qualification", "component_qualification_sha256")
    ...
    if (config.get("schema_version") != 1 or config.get("family") != "full"
            or config.get("world_size") != 16 or config.get("group_size") != 4
            or config.get("prompts_per_rank") != 1):
        raise ValueError("This admitted pair uses full16, one prompt/rank, four samples/group")
    if config.get("temperature", 1.) != 1 or config.get("top_p", 1.) != 1:
        raise ValueError("On-policy RLOO requires temperature=top_p=1")
    integers = {"max_new_tokens": 128, "context_limit": 2048,
                "max_rollout_updates": 128, "pilot_updates": 2, "eval_count": 64,
                "eval_interval_updates": 8, "qualification_eval_count": 8,
                "eval_local_batch_size": 1, "checkpoint_interval_updates": 16,
                "head_chunk_size": 128, "qualification_max_new_tokens": 16,
                "replay_prefixes": 4}
    for key, default in integers.items():
        config.setdefault(key, default)
        if type(config[key]) is not int or config[key] < 1:
            raise ValueError(f"{key} must be a positive integer")
    if config["max_rollout_updates"] > 128 or config["pilot_updates"] > config["max_rollout_updates"]:
    ...
    if config["replay_mode"] != "sampled-prefix":
        raise ValueError("Production RL requires sampled-prefix replay of the actual behavior policy")
...
def load_data(config):
    ...
    for name in ("train", "heldout"):
        ...
        for row in rows:
            prompt = row["prompt"]
            if len(prompt) != 1 or set(prompt[0]) != {"role", "content"} or prompt[0]["role"] != "user":
                raise ValueError("RL policy input must contain one user problem, with no answer trace")
            key = problem_key(prompt[0]["content"])
            if row["id"] != key or row["problem_sha256"] != key or key in identities or key in exposed:
            ...
            if canonical_math_answer(row["expected_answer"]) != row["canonical_answer"] or row["canonical_answer"] is None:
    ...
    if len(splits["train"]) < config["max_rollout_updates"] * config["world_size"] * config["prompts_per_rank"]:
...
def encode_examples(rows, *, tokenizer, renderer, user_template, context_limit, max_new_tokens):
    ...
    for row in rows:
        ...
        encoded = renderer.encoder.encode_messages([{"role": "user", "content": content}], thinking_mode="chat")
        ...
        result.append({"problem_id": row["id"], "prompt_ids": tokens,
...
def length_bucket_order(examples, *, global_batch_size, seed):
    ...
    batches = [ordered[start:start + global_batch_size] for start in range(0, full_end, global_batch_size)]
    random.Random(seed).shuffle(batches)
    result = [row for batch in batches for row in batch] + ordered[full_end:]
    return result, {"algorithm": "length-bucketed-global-batches-shuffled-v1",
                    "seed": seed, "global_prompt_batch": global_batch_size,
...
def make_contract(config, variant, loading, marker, tokenizer_provenance):
    ...
    runtime = {key: value for key, value in loading.items()
    ...
    return json.loads(json.dumps({
        ...
        "reference_kl": False, "auxiliary_training_objectives": False,
        "optimizer": "fresh-FP32-factored-Adafactor-stochastic-BF16",
        "project_commit": commit, "source_commit": commit, "algorithm": "RLOO",
        "group_size": config["group_size"], "reward_backend": REWARD_BACKEND,
        "parent_checkpoint": {"path": config["parents"][variant]["checkpoint"],
                              "sha256": config["parents"][variant]["marker_sha256"]},
        "data_manifest": {"path": config["data_manifest"], "sha256": config["data_manifest_sha256"]},
        "recipe": config, "runtime": runtime, "tokenizer": tokenizer_provenance,
...
def component_gate(config, loading):
    ...
    root = Path(__file__).resolve().parents[1]
    required_sources = ("automodel/deepseek_v41_rl_head.py", "rl/rollout.py",
                        "automodel/deepseek_v41_rl_fsdp_probe.py", "rl/weight_residency.py",
    ...
    if (receipt.get('weight_residency_qualified') is not True
    ...
    if (receipt.get("passed") is not True or receipt.get("world_size") != 16
            or sorted(row.get("rank", -1) for row in ranks) != list(range(16))
            or any(row.get("passed") is not True or not row.get("signed_coefficients")
                   or not row.get("masked_targets")
                   or row.get("rollout_replay_max_abs_error", math.inf) > 2e-6 for row in ranks)):
    ...
    return {"path": str(path), "sha256": config["component_qualification_sha256"],
            "passed": True, "world_size": 16, "synthetic_policy_updates": 0}


def admit_qualification(receipt, contract):
    if (receipt.get("passed") is not True or receipt.get("contract_digest") != digest(contract)
...
def qualify_leaf_head(device, width):
    ...
    hidden = (torch.randn(2, 5, width, device=device, generator=generator) / math.sqrt(width)).requires_grad_()
    ...
    expected = F.linear(hidden, weight).log_softmax(-1).gather(-1, labels.clamp_min(0)[..., None]).squeeze(-1)
    ...
    return {"passed": True, "width": width, "vocabulary": 31,
            "signed_hidden_and_head_gradient_oracle": True,
            "output_max_abs": float((actual - expected).abs().max()),
            "gradient_max_abs": [float((a - b).abs().max()) for a, b in zip(first, second, strict=True)]}
...
def save_qualification_rollout(rollout, output, *, rank, replay):
    ...
    tensors = {
        ...
        for name in ("input_ids", "attention_mask", "labels", "response_mask",
                     "policy_log_probs", "behavior_log_probs")
    }
    payload = {
        "format": "archlab-rl-qualification-rollout-v1", "rank": rank, "replay": replay,
        "receipt": rollout.receipt, "generated_ids": rollout.generated_ids,
        "prompt_lengths": rollout.prompt_lengths, "finish_reasons": rollout.finish_reasons,
        "audit_only": True, "optimizer_updates_requested": False, **tensors,
    }
    ...
    result = {"path": str(path.resolve()), "sha256": sha256_file(path),
              "bytes": path.stat().st_size, "rank": rank, "replay": replay,
              "policy_version": rollout.receipt["policy_version"],
              "contains_model_weights": False, "optimizer_updates_requested": False}
    atomic_write_json(path.with_suffix(".json"), result, allow_nan=False)
    return result


def run_qualification(model, optimizer, indexers, encoded, tokenizer, config, contract, output, stops, pad):
    import torch
    import torch.distributed as dist
    ...
    try:
        ...
        groups, prompts = rank_groups(encoded["train"], 0, rank=rank, world=world,
        ...
        if config.get("profile_rollouts", False):
            ...
            if rank == 0:
                print(json.dumps({"event": "rl_rollout_profile_start", "batch_sizes": [1, 4]}), flush=True)
            profile_prompt = gather(groups[0]["prompt_ids"])[0]
            profile = profile_rollout_batches(model, profile_prompt, policy_version=version,
                max_new_tokens=config.get("profile_new_tokens", 4), context_limit=config["context_limit"],
                eos_token_ids=stops, pad_token_id=pad, seed=config["seed"] + 2000000,
                batch_sizes=(1, 4), warmup=1, repeats=1)
            if rank == 0:
                atomic_write_json(output / "PROFILE.json", profile, allow_nan=False)
        rollout = sample_rollouts(model, prompts, policy_version=version,
        ...
        synthetic = torch.zeros((len(groups), config["group_size"]), device=model.lm_head.weight.device)
        synthetic[:, 0] = 1
        try:
            audit = policy_gradient_step(model, optimizer, indexers, rollout, synthetic,
                lr=config["learning_rate"], group_size=config["group_size"],
                head_chunk_size=config["head_chunk_size"], replay_tolerance=config["replay_tolerance"],
                audit=True, audit_only=True, **replay)
        except BaseException as error:
            atomic_write_json(output / f"rank-{rank:02d}-qualification-replay-failure.json", {
                "passed": False, "error_type": type(error).__name__, "reason": str(error),
                "rollout_artifact": rollout_artifact, "replay": replay,
                "optimizer_updates_requested": False,
                "optimizer_state_nonempty": any(optimizer.state.values()),
            }, allow_nan=False)
            raise
        if (audit.get("updated") or any(optimizer.state.values())
        ...
        receipt = {"passed": True, "kind": "online-policy-numerical-v1",
                   "contract_digest": digest(contract), "contract": contract,
                   "synthetic_optimizer_updates": 0, "real_reward_update_proven": False,
...
def save_rl_checkpoint(path, model, optimizer, cursor, contract):
    ...
    if (path / "COMPLETE.json").exists():
        ...
        if saved["contract"] != contract or any(saved["cursor"].get(k) != v for k, v in cursor.items()):
...
def verify_critical_checkpoint(path, model, optimizer, cursor, contract):
    ...
    adapter = next((name for name in named if "simplicial_adapter" in name and name.endswith("output.weight")), None)
    ...
    for name in selected:
        ...
        for chunk in entries[name]["chunks"]:
            ...
            actual = hashlib.sha256(saved.contiguous().view(torch.uint8).numpy().tobytes()).hexdigest()
            if actual != chunk["sha256"] or not torch.equal(saved, live[offset:offset + saved.numel()].cpu()):
        ...
        saved_state = torch.load(folder / manifest["optimizer_states"][index], map_location="cpu", weights_only=True)
        ...
        for key, value in saved_state.items():
            ...
            equal = torch.equal(value, other.detach().cpu()) if isinstance(value, torch.Tensor) else value == other
            if not equal:
                raise ValueError("Pilot critical optimizer state differs")
    rng = torch.load(folder / "rng.pt", map_location="cpu", weights_only=True)
    if not torch.equal(rng["cpu_rng"], torch.get_rng_state()) or not torch.equal(rng["cuda_rng"], torch.cuda.get_rng_state()):
        raise ValueError("Pilot checkpoint Torch RNG readback differs")
    local = {"passed": True, "rank": rank, "tensor_names": selected,
             "critical_weight_and_optimizer_readback": True, "torch_rng_exact": True,
             "full_model_roundtrip_executed": False}
    receipts = gather(local)
    if rank == 0:
        atomic_write_json(Path(path) / "RL_CRITICAL_STATE_VERIFIED.json", {"passed": True,
            "ranks": receipts, "cursor": cursor, "scope": "bounded critical-state readback, not full restore"}, allow_nan=False)
...
def preflight_rl_checkpoint(path, contract, *, rank, world):
    ...
    if (marker.get("format") != "archlab-v41-full-sharded-v1"
            or marker.get("contract") != contract or marker.get("world_size") != world
    ...
    if manifest.get("rank") != rank or any(manifest.get(field) != marker[field]
    ...
    if len(sidecars) != world or [entry["file"] for entry in sidecars] != [f"rl-rank-{r:02d}-rng.json" for r in range(world)]:
...
def restore_rl_checkpoint(path, model, optimizer, contract):
    ...
    if sidecar.name != f"rl-rank-{dist.get_rank():02d}-rng.json" or sha256_file(sidecar) != spec["sha256"]:
...
def initial_cursor():
    return {"step": 0, "phase_step": 0, "rollout_step": 0, "optimizer_step": 0,
            "prompt_cursor": 0, "generated_tokens": 0}


def run_training_loop(model, optimizer, indexers, encoded, tokenizer, config, contract, output,
                      *, mode, step_limit, cursor, stops, pad, stop_requested=lambda: False,
                      sample_fn=None, update_fn=None, evaluate_fn=None, checkpoint_fn=None,
    ...
    def evaluate():
        ...
        if rank == 0:
            ...
            atomic_write_json(output / f"evaluation-step-{cursor['rollout_step']:06d}.json", records, allow_nan=False)
    ...
    while cursor["rollout_step"] < step_limit:
        ...
        for index, generated in enumerate(rollout.generated_ids):
            ...
            local_records.append({"problem_id": row["problem_id"], "generated_ids": generated,
        ...
        packet = {"sampling_seconds": sampling_seconds, "update_seconds": update_seconds,
                  "iteration_seconds": time.perf_counter() - began, "generated_tokens": count,
                  "correct": sum(values), "completions": len(values),
        ...
        model._archlab_rl_policy_version = version()
        timing = {key: max(row[key] for row in totals)
                  for key in ("sampling_seconds", "update_seconds", "iteration_seconds")}
        completions = sum(row["completions"] for row in totals)
        metrics.update(**cursor, policy_version=used_version, next_policy_version=version(),
                       update_step=cursor["rollout_step"], rollout_seconds=timing["sampling_seconds"],
                       rollout_tokens=generated_count,
                       rollout_tokens_per_second=generated_count / max(timing["sampling_seconds"], 1e-12),
                       **timing, rollout_generated_tokens=generated_count,
        ...
        if rank == 0:
            ...
            if packet["updated"] and cursor["optimizer_step"] == 1:
                atomic_write_json(output / "REAL_UPDATE_VERIFIED.json", {"passed": True, **cursor,
                    "real_outcome_rewards": True, "metrics": metrics}, allow_nan=False)
            atomic_write_json(output / "PROGRESS.json", {**cursor, "policy_version": version(),
                "phase": "online_rl", "latest": metrics}, allow_nan=False)
        ...
        if pilot_end:
            if rank == 0:
                atomic_write_json(output / "PILOT_RESULT.json", {**cursor,
                    "real_update_passed": cursor["optimizer_step"] > 0,
                    "numerical_admission_is_not_learning_evidence": True}, allow_nan=False)
            if cursor["optimizer_step"] == 0:
                reason = "pilot_no_policy_update"
                break
    else:
        reason = "budget_reached" if cursor["rollout_step"] == config["max_rollout_updates"] else "step_limit"
    if last_evaluated != cursor["rollout_step"]:
        evaluate()
    checkpoint()
    report = {**cursor, "stop_reason": reason, "mode": mode, "policy_version": version(),
              "real_policy_updates": cursor["optimizer_step"], "budget": config["max_rollout_updates"]}
    if rank == 0:
...
def main():
    ...
    try:
        ...
        encoded = {name: encode_examples(rows, tokenizer=tokenizer, renderer=renderer,
            user_template=template, context_limit=config["context_limit"], max_new_tokens=config["max_new_tokens"])
                   for name, rows in splits.items()}
        encoded["train"], ordering = length_bucket_order(encoded["train"],
            global_batch_size=16 * config["prompts_per_rank"], seed=config["seed"])
        ...
        provenance = {"native_encoder_sha256": sha256_file(assets / "encoding/encoding.py"),
                      "prompt_sha256": sha256_file(config["prompt_template"]),
                      "tokenizer_files": {p.name: sha256_file(p) for p in assets.glob("tokenizer*") if p.is_file()},
                      "thinking_mode": "chat", "eos_ids": sorted(stops), "pad_token_id": pad,
        ...
        model, indexers, loading, marker = construct_rl_actor(checkpoint=parent["checkpoint"],
            family="full", variant=args.variant, assets=assets, weights=Path(config["weights"]),
        ...
        loading["rl_numerical_precision"] = {"before_construction": precision_before,
        ...
        if rank == 0:
            ...
            commit = subprocess.check_output(["git", "-C", str(Path(__file__).resolve().parents[3]),
                                               "rev-parse", "HEAD"], text=True).strip()
            atomic_write_json(args.output / "RUN_PROVENANCE.json", {"project_commit": commit,
                "recipe_path": str(args.recipe), "mode": args.mode, "cli_steps": args.steps,
                "resume_rl": str(args.resume_rl) if args.resume_rl else None}, allow_nan=False)
        if args.qualification:
            path = args.qualification / "QUALIFIED.json" if args.qualification.is_dir() else args.qualification
            ...
            qualification = run_qualification(model, optimizer, indexers, encoded, tokenizer,
                                               config, contract, args.output, stops, pad)
            admit_qualification(qualification, contract)
        if args.mode == "qualify":
            if activity:
                activity.set("qualified")
            return
        cursor = restore_rl_checkpoint(args.resume_rl, model, optimizer, contract) if args.resume_rl else initial_cursor()
        ...
        if rank == 0:
            atomic_write_json(args.output / "TRAINING_ADMITTED.json", {"passed": True,
                "contract_digest": digest(contract), "real_reward_update_still_required": cursor["optimizer_step"] == 0}, allow_nan=False)
        limit = config["pilot_updates"] if args.mode == "pilot" else config["max_rollout_updates"]
        if args.steps is not None:
            limit = min(limit, args.steps)
        report = run_training_loop(model, optimizer, indexers, encoded, tokenizer, config, contract, args.output,
            mode=args.mode, step_limit=limit, cursor=cursor, stops=stops, pad=pad,
...
def write_owned_failure(output, rank, owns_output, traceback_text):
    ...
    if owns_output:
        atomic_write_json(Path(output) / f"rank-{rank:02d}-failure.json", {"traceback": traceback_text})
