...
def trainable_hc_forward(self, hidden_states):
    ...
    with torch.autocast(hidden_states.device.type, enabled=False):
        ...
        mixes = F.linear(flat, self.fn) * torch.rsqrt(flat.square().mean(-1, keepdim=True) + self.norm_eps)
        # Clones retain derivatives and outlive the owner's FSDP unshard storage.
        scale, base = self.scale.clone(), self.base.clone()
        values = NativeTrainableHC.apply(mixes, scale, base, self.streams,
                    self.iterations, self.eps, self._archlab_native_hc)
...
def install_full_training_boundaries(model):
    ...
    return {"trainable_hc_modules": count, "head_loss": "chunked-full-vocabulary-FP32",
