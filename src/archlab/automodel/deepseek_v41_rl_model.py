...
RUNTIME_FIELDS = ("container_image", "packages", "cuda", "nccl", "automodel_commit",
                  "automodel_root", "reproducibility", "world_size", "ep_size",
...
def read_parent_checkpoint(checkpoint, *, family, variant, world_size, tiny=False):
    ...
    if variant not in (("normal", "simplicial") if family == "full" else
                       ("normal", "simplicial", "linear", "linsimp")):
    ...
    expected_format = "archlab-v41-full-training-v1" if family == "full" else "archlab-v41-scratch-comparison-v1"
    if (marker.get("format") != "archlab-v41-full-sharded-v1"
            or marker.get("world_size") != world_size or contract.get("world_size") != world_size
            or contract.get("format") != expected_format or contract.get("variant") != variant
            or contract.get("tiny") is not tiny or contract.get("cpu_offload") is not False):
        raise ValueError("Parent checkpoint format, variant, geometry class, or ownership differs")
    runtime = contract["runtime"]
    for field, expected in (("world_size", world_size), ("ep_size", 8),
                            ("expert_fsdp_size", world_size // 8), ("engram_owners", world_size)):
    ...
    return marker, {"checkpoint": str(Path(checkpoint).resolve()), "marker_sha256": _sha(path),
                    "parent_contract_sha256": hashlib.sha256(json.dumps(contract, sort_keys=True).encode()).hexdigest()}


def audit_parent_sources(marker, *, source_root=None, declared_source_changes=None,
    ...
    for relative, expected in hashes.items():
        ...
        actual = _sha(path)
        receipt = {"relative": relative, "path": str(path.resolve()),
                   "parent_sha256": expected, "current_sha256": actual}
        if actual != expected:
            required = {"before_sha256": expected, "after_sha256": actual,
                        "reason": SOURCE_CHANGE_REASONS.get(relative)}
            if required["reason"] is None or declarations.get(relative) != required:
                raise ValueError(f"Undeclared or disallowed parent implementation change: {relative}")
...
def validate_constructed_runtime(parent, loading, *, family):
    ...
    if family == "scratch":
        for field in ("geometry", "parameters", "adapter_layers", "variant", "boundaries",
    ...
    if "resolved_kernel_packages" in old and old["resolved_kernel_packages"] != new.get("resolved_kernel_packages"):
...
def _restore_local_weights(model, checkpoint, marker, *, rank, expected_device):
    ...
    for (name, tensor), entry in zip(named, manifest["tensors"], strict=True):
        ...
        if (entry["name"] != name or entry["shape"] != list(local.shape)
                or entry["global_shape"] != list(tensor.shape) or entry["dtype"] != str(local.dtype)
                or not local.is_contiguous()):
            raise ValueError(f"Checkpoint tensor name/shape/dtype differs: {name}")
        elements = 0
        for chunk in entry["chunks"]:
            filename = chunk["file"]
            if (not re.fullmatch(r"tensor-\d+-\d+\.pt", filename) or filename in chunk_names
                    or type(chunk["elements"]) is not int or chunk["elements"] <= 0
    ...
    print(json.dumps({"event": "rl_weight_restore_start", "rank": rank,
    ...
    with torch.no_grad():
        for (_, tensor), entry in zip(named, manifest["tensors"], strict=True):
            ...
            for chunk in entry["chunks"]:
                ...
                if (not isinstance(host, torch.Tensor) or host.ndim != 1 or host.dtype != local.dtype
                        or host.numel() != chunk["elements"]
                        or hashlib.sha256(host.contiguous().view(torch.uint8).numpy().tobytes()).hexdigest() != chunk["sha256"]):
                ...
                if size >= next_progress:
                    print(json.dumps({"event": "rl_weight_restore_progress", "rank": rank,
                                      "loaded_local_gib": size / 2**30,
                                      "seconds": time.monotonic() - began}), flush=True)
                    next_progress += 8 * 2**30
    print(json.dumps({"event": "rl_weight_restore_complete", "rank": rank,
                      "loaded_local_gib": size / 2**30,
                      "seconds": time.monotonic() - began}), flush=True)
    return {"rank": rank, "rank_manifest_sha256": _sha(manifest_path), "tensor_entries": len(named),
            "verified_chunks": count, "bytes": size, "all_weight_checksums_verified": True,
            "optimizer_loaded": False, "rng_loaded": False, "cpu_weight_offload": False}
...
def _scratch_constructor(path):
    ...
    spec = importlib.util.spec_from_file_location("_archlab_rl_parent_scratch_constructor", Path(path))
...
def construct_rl_actor(*, checkpoint, family, variant, assets, weights=None, base_config=None,
                       tiny=False, declared_source_changes=None, declared_execution_changes=None,
                       scratch_constructor_path=None, resolved_kernel_packages=None,
    ...
    if checkpoint is not None:
        marker, parent_receipt = read_parent_checkpoint(checkpoint, family=family, variant=variant,
                                                         world_size=world, tiny=tiny)
        if (expected_parent_marker_sha256 is not None
        ...
        source_receipt = audit_parent_sources(marker, declared_source_changes=declared_source_changes,
    ...
    if family == "full":
        if variant not in ("normal", "simplicial") or weights is None:
            raise ValueError("Full actors need normal/simplicial and the recorded model config directory")
        ...
        adapters = install_official_adapters(model, V41AdapterConfig(width=256) if tiny else V41AdapterConfig(),
            layer_indices=(1, 3, 5) if tiny else (4, 9, 14, 19, 24, 29, 34, 39),
            device="cuda", variant=variant, allow_right_padding=True,
    ...
    if marker is not None:
        ...
        expected = (marker["contract"]["full_text_parameters"] if family == "full"
        ...
        restore = _restore_local_weights(model, checkpoint, marker, rank=dist.get_rank(), expected_device="cuda")
        dist.barrier()
    else:
        restore = {"optimizer_loaded": False, "rng_loaded": False, "qualification_random_initialization": True}
    model.requires_grad_(True)
    model.train()
    if any(local_tensor(p).device.type != "cuda" or not p.requires_grad for p in model.parameters()):
        raise ValueError("RL actor must keep every trainable weight resident on GPU")
    loading = {**_canonical(loading), "rl_transition": {
        "family": family, "variant": variant, "tiny": tiny,
        "qualification_only": bool(tiny), "numerical_qualification_required": True,
        "actor_weight_origin": "random-qualification" if marker is None else "parent-checkpoint",
        "parent": parent_receipt, "parent_contract": None if marker is None else marker["contract"],
        "source_provenance": source_receipt, "execution_changes": dict(declared_execution_changes),
        "restore": restore, "helper_sha256": _sha(Path(__file__)),
        "cpu_weight_offload": False, "optimizer_state_policy": "fresh-RL-optimizer-owned-by-caller",
    }}
