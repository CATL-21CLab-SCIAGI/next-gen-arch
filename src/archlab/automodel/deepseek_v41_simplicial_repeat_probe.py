...
def main():
    ...
    report = {"production_qualification": False, "context": args.context, "windows": [32, 512],
              "repeats": [], "passed": False}
    snapshots = {}
    names = ("output", "dq", "dk1", "dk2", "dv1", "dv2")
    for label, fn in (("atomic", simplicial_attention), ("deterministic", deterministic_simplicial_attention)):
        records = []
        for repeat in range(args.repeats):
            ...
            records.append({"repeat": repeat, "seconds": time.perf_counter() - start,
                            **{name: _compare(value, expected) for name, value, expected in
                               zip(names, current, snapshots[label], strict=True)}})
        report["repeats"].append({"implementation": label, "results": records})
    report["comparison"] = {name: _compare(value, expected) for name, value, expected in
                            zip(names, snapshots["deterministic"], snapshots["atomic"], strict=True)}
    ...
    if not report["passed"]:
        raise AssertionError("simplicial deterministic backward did not pass native/repetition checks")
