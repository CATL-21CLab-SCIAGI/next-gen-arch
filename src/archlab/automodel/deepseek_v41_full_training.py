...
def main():
    ...
    try:
        ...
        if args.resume_full:
            cursor = restore_full_checkpoint(args.resume_full, model, optimizer, contract)
        ...
        while cursor['step']*32 < len(data):
            ...
            if cursor['phase_step'] == 5 or cursor['supervised_tokens']-last_save >= 20000000 or bool(stop) or cursor['step']*32 >= len(data):
                save_full_checkpoint(output/'checkpoints'/f'step-{cursor["step"]:06d}', model, optimizer, cursor, contract)
                last_save = cursor['supervised_tokens']
