...
def main():
    ...
    from archlab.automodel.deepseek_v41_official_execution import (
        build_official_base, configure_official_reproducibility, official_core_hashes,
    )
    ...
    from archlab.automodel.deepseek_v41_official_training import (
        capture_adapter_training_state, restore_in_memory_training_state, optimizer_step,
        state_difference_statistics, _update_vector_statistics,
    )
    from archlab.automodel.deepseek_v41_training import restore_adapter_checkpoint, emit
    ...
    try:
        ...
        changed = [key for key, value in current_hashes.items() if contract["implementation_sha256"].get(key) != value]
        if changed and not args.allow_core_change:
            raise ValueError("diagnose the exact saved model/training implementation")
        model, _, loading = build_official_base(weights=args.weights, assets=args.assets,
        ...
        def watch_tensor(key, value):
            if trace_rank and isinstance(value, torch.Tensor) and value.requires_grad:
                def gradient(grad):
                    sampled = grad.index_select(1, sample_positions) if grad.ndim >= 2 and grad.shape[1] == inputs.shape[1] else grad.flatten()[:4096]
        ...
        def watch_module(key, module):
            ...
            def after(_module, _values, result):
                value = result[0] if isinstance(result, tuple) else getattr(result, "hidden_states", result)
        ...
        report = {"production_qualification": False, "rank": rank, "loading": loading,
                  "activation_checkpointing": not args.no_checkpointing, "repeats": [], "completed": False,
                  "changed_core_files": changed, "implementation_sha256": current_hashes}
        ...
        modes = (["atomic_adapter", "deterministic_adapter"] if args.compare_adapter else
                 ["upstream", "private_clone"] if args.compare_gather else ["current"])
        for iteration in range(args.repeats * len(modes)):
            ...
            if repeat == 0:
                ...
                if args.compare_gather:
                    from nemo_automodel.components.moe.experts import _AllGatherConcatVarlenFn, GroupedExperts
                    from archlab.automodel.deepseek_v41_official_moe import _NonMutatingVarlenGather
                    for module in model.modules():
                        if isinstance(module, GroupedExperts):
                            module._archlab_ep_gather = _AllGatherConcatVarlenFn if mode == "upstream" else _NonMutatingVarlenGather
            ...
            record = {"mode": mode, "repeat": repeat, "metric": metric, "hidden_sha256": tensor_digest(hidden["value"])}
            if rank == 0:
                current = capture_adapter_training_state(adapters, optimizers)
                gradients = {i: {name: p.grad.detach().cpu().clone() * max(metric["gradient_norm_before_clip"], 1.)
                    for name, p in adapter.named_parameters()} for i, adapter in adapters.items()}
                if baseline is None:
                    baseline, baseline_gradients = current, gradients
                record["adapter_update"] = _update_vector_statistics(saved["adapters"], baseline["adapters"], current["adapters"])
                record["raw_gradients"] = state_difference_statistics(baseline_gradients, gradients)
                record["per_adapter_raw_gradients"] = {str(i): state_difference_statistics(baseline_gradients[i], gradients[i]) for i in gradients}
            if trace_rank:
                gradients_trace = {key: value.cpu() for key, value in trace.items()}
                if baseline_trace is None:
                    baseline_trace = gradients_trace
                record["gradient_boundaries"] = {key: {
                    **state_difference_statistics(baseline_trace[key], value),
                    "baseline_norm": float(baseline_trace[key].double().norm())}
                    for key, value in gradients_trace.items() if key in baseline_trace}
        ...
        dist.barrier()
        if rank == 0:
            atomic_write_json(args.output / "DIAGNOSTIC_COMPLETE.json", {"completed": True, "production_qualification": False})
