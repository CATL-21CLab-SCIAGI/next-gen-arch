...
def main() -> None:
    ...
    report = {"training_launched": False, "kind": "native-full-shape-kernel-repeatability", "cases": []}
    ...
    def compare(name, actual, repeated, expected):
        ...
        value = {"name": name, "shape": list(actual.shape), "native_finite": finite,
                 "native_repeat_equal": torch.equal(actual, repeated), "torch_finite": bool(expected.isfinite().all())}
        if finite and value["torch_finite"]:
            value["relative_l2"] = float((actual.float() - expected.float()).norm() / expected.float().norm().clamp_min(1e-20))
    ...
    with torch.no_grad():
        for rows in (17, 32, 33):
            ...
            scales = torch.ones(guard_rows, width // block, device="cuda", dtype=torch.float32).to(torch.float8_e8m0fnu)
            ...
            record = {"name": f"native_fp8_scale_guard/rows={rows}",
        ...
        for layer in (0, 1, 20, 39):
            for sublayer in ("attn", "ffn"):
                ...
                for magnitude in (1., 10.):
                    ...
                    compare(f"{stem}/mix_std={magnitude}", torch.cat([x.flatten() for x in first]),
                            torch.cat([x.flatten() for x in second]), torch.cat([x.flatten() for x in expected]))
        for stem in ("layers.0.attn.wq_a", "layers.0.attn.wq_b", "layers.0.ffn.experts.0.w1",
                     "layers.0.ffn.experts.0.w2", "layers.1.engram.wkv"):
        ...
        for length in (128, 1024):
            ...
            first = native.sparse_attn(q, kv, sink, ids, 512**-.5)
            second = native.sparse_attn(q, kv, sink, ids, 512**-.5)
            expected = query_chunked_sparse_attention(q, kv, sink, ids, 512**-.5, native_rounding=True)
