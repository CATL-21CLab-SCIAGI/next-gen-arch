...
def _adapter_projections(adapter, streams):
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16,
        ...
        x = adapter.input_norm((streams.float() * read[None, None, :, None]).sum(-2).to(streams.dtype))
        ...
        if hasattr(adapter, "k1"):
            projected = {"k1": adapter.k1_norm(adapter.k1(x).reshape(shape)),
                         "k2": adapter.k2_norm(adapter.k2(x).reshape(shape)),
                         "v1": adapter.v1(x).reshape(shape), "v2": adapter.v2(x).reshape(shape)}
        else:
            projected = {"k": adapter.k_norm(adapter.k(x).reshape(shape)),
                         "v": adapter.v(x).reshape(shape)}
    return x, q, projected


def _adapter_step(adapter, cache, streams):
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16,
        ...
        with torch.autocast(streams.device.type, enabled=False):
            if "k" in cache:
                attended = flash_attn_func(q.contiguous(), cache["k"], cache["v"],
                    dropout_p=0.0, softmax_scale=c.head_dim ** -0.5,
                    causal=True, window_size=(c.long_window - 1, 0), deterministic=True)
            else:
                attended = simplicial_decode_attention(
                    q.float(), cache["k1"].float(), cache["k2"].float(),
                    cache["v1"].float(), cache["v2"].float()).to(q.dtype)
...
def _index_step(indexer, hidden, query_latent, latent, angles, compressed_angles, state,
                previous_keys, position):
    from nemo_automodel.components.models.deepseek_v41.attention import _apply_rope, _select_candidate_blocks
    ...
    if indexer.owns_keys:
        new_key = None if latent is None else quantize_cache(
            _apply_rope(indexer.k_norm(indexer.wk(latent)), compressed_angles),
            format="mxfp4", block_size=32)
    ...
    if width == 0:
        ...
        candidates = (torch.empty(hidden.shape[0], 1, 0, device=hidden.device, dtype=torch.bool)
                      if indexer.is_candidate_source else state.candidates)
    ...
    if state.compressed_valid is not None:
        allowed = allowed & state.compressed_valid
    scores = scores.masked_fill(~allowed.view(hidden.shape[0], 1, width), -torch.inf)
    candidates = state.candidates
    if indexer.is_candidate_source:
        counts = torch.full((hidden.shape[0], 1, 1), visible, device=hidden.device, dtype=torch.long)
        candidates = _select_candidate_blocks(scores, counts,
            topk_blocks=indexer.candidate_topk_blocks, block_size=indexer.candidate_block_size)
...
def _attention_step(attn, cache, hidden, *, position_ids, state, attention_mask=None, position):
    ...
    from nemo_automodel.components.models.deepseek_v41.attention import (
        DeepseekV41AttentionOutput, DeepseekV41AttentionState, _apply_rope,
    )
    from nemo_automodel.components.models.deepseek_v41.quantization import quantize_cache

    batch, length, _ = hidden.shape
    if (length != 1 or attention_mask is not None or attn.backend.attn != "tilelang"
    ...
    query = _apply_rope(attn.wq_b(query_latent).unflatten(-1, (attn.num_heads, attn.head_dim)), angles)
    new_kv = quantize_cache(_apply_rope(attn.kv_norm(attn.wkv(hidden)), angles),
    ...
    if attn.compress_ratio:
        ...
        if attn.compressor is not None:
            ...
            if partial.shape[1] == ratio:
                ...
                new_compressed = quantize_cache(_apply_rope(latent, compressed_angles),
                                                format="nvfp4", block_size=16)
        ...
        if attn.indexer is not None:
            next_state = _index_step(attn.indexer, hidden, query_latent, latent, angles,
                compressed_angles, next_state, cache["index_keys"], position)
    ...
    attended = dsv4_sparse_attention(query, kv, attn.sinks_param(query), slots,
        attn.head_dim ** -0.5, backend="tilelang", reference_rounding=True)
...
class V41PolicyCache:
    ...
    def decode(self, token_ids):
        ...
        self._check_mode()
        if (not self.valid or self.position is None or token_ids.shape != (self.tokens.shape[0], 1)
                or token_ids.dtype != torch.long or token_ids.device != self.tokens.device):
        ...
        try:
            ...
            pre_mix = torch.zeros(*token_ids.shape, text.config.hc_mult,
                                  device=embedded.device, dtype=torch.float32)
            pre_mix[..., 0] = 1
            ...
            for index, (wrapped, inner, attn) in enumerate(self.layers):
                ...
                def attention_forward(module, hidden, *, position_ids, state, attention_mask=None,
                                      target=cache):
                    return _attention_step(module, target, hidden, position_ids=position_ids,
                        state=state, attention_mask=attention_mask, position=self.position)
                ...
                if adapter is not None:
                    ...
                    def adapter_forward(module, streams, *, target=cache["adapter"]):
                        return _adapter_step(module, target, streams)
                    adapter.forward = MethodType(adapter_forward, adapter)
                engram = None if inner.engram is None else hashes[:, -1:, inner.engram.layer_hash_index]
                streams, pre_mix, state = wrapped(streams, pre_mix, state,
                    position_ids=positions, attention_mask=None, image_mask=None, engram_hash_ids=engram)
            hidden = DeepseekV41HyperConnection.collapse(streams, pre_mix)
...
def qualify_resident_cache(model, prompts, *, pad_token_id, context_limit, steps=16,
                          tolerance=.02, retain_weights=True, reserve_gib=16):
    ...
    from archlab.rl.rollout import _bucket, _maximum
    ...
    if not 0 < steps or len(prompts[0]) + steps > context_limit or not 0 < tolerance <= .02:
    ...
    try:
        ...
        with ExitStack() as scope:
            ...
            for index in range(steps + 1):
                if index:
                    ...
                    cached = cache.decode(token)
                    ids = torch.cat((ids, token), dim=1)
                canvas = _bucket(_maximum(ids.shape[1], device), 128, context_limit)
                ...
                all_hidden = model(input_ids=inputs, attention_mask=mask,
                ...
                relative_rms = difference.square().mean().sqrt() / reference.float().square().mean().sqrt().clamp_min(1e-12)
        ...
        return {"passed": not nonfinite and relative <= 1e-3 and logprob <= tolerance,
                "kind": "resident-cache-padded-prefix-equivalence-v1",
                "decode_steps": steps, "prefix_comparisons": steps + 1,
                "prompt_tokens_local": len(prompts[0]), "all_vocabulary_logprobs": True,
