...
def maintenance_resume_changes(parent, current):
    ...
    if old != new:
        raise ValueError(
            "maintenance continuation changed training, data, geometry, or runtime semantics"
        )
    mutable = {
        "automodel/deepseek_v41_scratch_training.py",
        "automodel/deepseek_v41_scratch_high_mfu.py",
    }
    additions = {
        "automodel/deepseek_v41_scratch_resume.py",
    ...
    return [
        "independent sparse forward/dq/dkv/dsink qualification",
        "record resolved container kernel modules",
        "record sink-gradient atomics",
    ]
