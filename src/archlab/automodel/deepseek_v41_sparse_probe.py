...
def main():
    ...
    report = {"kind": "native-sparse-tile-repeatability", "training_launched": False,
              "container_image": os.environ.get("NGA_CONTAINER_DIGEST"), "torch": str(torch.__version__),
              "cuda": torch.version.cuda, "kernel_packages": packages,
              "probe_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
              "reference_sha256": {name: hashlib.sha256((args.assets / "inference" / name).read_bytes()).hexdigest()
                                   for name in ("model.py", "kernel.py")}, "cases": []}
    ...
    def metric(first, second):
        ...
        if finite:
            ...
            result.update(max_abs=float(error.abs().max()),
            ...
            for topk in args.topks:
                if args.replay:
                    ...
                    if q.shape[1] != length or kv.shape[1] != length:
                        raise ValueError("replay must contain exactly the requested raw-window context")
                else:
                ...
                if args.compressed_joint:
                    ...
                    compressed_kv = torch.randn(1, compressed, 512, device="cuda", dtype=torch.bfloat16)
                    compressed_kv = rounded_activation(compressed_kv, bits=4, block_size=16, e4m3_scale=True)
                    ...
                    compressed_ids = torch.arange(compressed, device="cuda")[None, None].expand(1, length, compressed)
                    compressed_ids = torch.where(compressed_ids < visible, compressed_ids + length, -1).int()
                ...
                first = native.sparse_attn(q, kv, sink, ids, 512 ** -.5).clone()
                expected = query_chunked_sparse_attention(q, kv, sink, ids, 512 ** -.5,
                                                          native_rounding=True, recompute=False)
                case = {"sequence": length, "topk": topk, "heads": 64, "head_dim": 512,
                        "query_source": str(args.replay) if args.replay else "random",
                        "index_layout": "native-window-plus-synthetic-compressed" if args.compressed_joint else "right-aligned-causal",
                        "native_to_torch": metric(first, expected), "repeats": []}
                del expected
                for repeat in range(args.repeats):
                    ...
                    del scratch
                    actual = native.sparse_attn(new_q, new_kv, sink, new_ids, 512 ** -.5)
