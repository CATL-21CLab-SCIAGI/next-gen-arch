...
def _indexer_forward(self, hidden_states, *, query_latent, latent, angles, compressed_angles, state):
    ...
    result = self._archlab_original_forward(hidden_states, query_latent=query_latent, latent=latent,
                angles=angles, compressed_angles=compressed_angles, state=state)
    ...
    if width == 0:
        ...
        return result
    ids = torch.linspace(max(0, ratio - 1), sequence - 1,
                         min(self._archlab_sample_queries, sequence - ratio + 1),
                         device=hidden_states.device).long().unique()
    sampled_angles = angles[:, ids]
    h, ql = hidden_states[:, ids].detach(), query_latent[:, ids].detach()
    keys = result.index_keys.detach()
    if self.owns_keys:
        keys = quantize_cache(_apply_rope(self.k_norm(self.wk(latent.detach())), compressed_angles),
    ...
    allowed = torch.arange(width, device=h.device)[None, None, :] < ((ids + 1) // ratio)[None, :, None]
    ...
    with torch.no_grad():
        teacher_queries = _apply_rope(parent.wq_b(ql).unflatten(-1, (parent.num_heads, parent.head_dim)), sampled_angles)
    self._archlab_auxiliary = attention_indexer_kl(scores, teacher_queries,
        result.compressed_kv.detach(), allowed, scale=parent.head_dim ** -0.5)
...
def _attention_forward(self, *args, **kwargs):
    ...
    if auxiliary is not None:
        output = replace(output, hidden_states=_InjectAuxiliary.apply(output.hidden_states, auxiliary,
                                self.indexer._archlab_auxiliary_backward_scale))
    return output
...
def install_trainable_indexers(model, *, sample_queries=64):
    ...
    for attention in model.modules():
        ...
        indexer._archlab_auxiliary_backward_scale = 0.
...
def set_indexer_loss_scale(indexers, *, coefficient, world_size, accumulation):
    for indexer in indexers:
        indexer._archlab_auxiliary_backward_scale = coefficient / (len(indexers) * world_size * accumulation)
