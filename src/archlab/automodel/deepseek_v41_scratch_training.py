...
def run_qualification(model, indexers, gates, optimizer, contract, output, *, context):
    ...
    if expected != actual or first["loss"] != repeated["loss"]:
...
def main():
    ...
    p.add_argument("--variant", choices=["normal", "simplicial"], required=True)
        ...
        if a.resume:
            ...
            atomic_write_json(
                output / f"rank-{rank:02d}-resume.json",
                {
                    ...
                    "cursor": cursor,
                    "controlled_changes": [
                        "router auxiliary coefficient0.01",
                        "router bias update0.01",
                        "microbatch8 accumulation1; global64 windows unchanged",
                        "centered proportional expert-bias correction",
                    ],
                },
            )
