...
def configure_gpu_budget(gib):
    ...
    return {"enabled": True, "allocator_budget_bytes": budget, "device_total_bytes": total,
...
def _forward_with_input_offload(self, *args, **kwargs):
    ...
    eligible = {_storage_key(value) for value in inputs
...
def install_checkpoint_input_offload(model):
    ...
    for name, module in selected:
        if module.checkpoint_impl != CheckpointImpl.NO_REENTRANT:
            raise ValueError(f"only the existing non-reentrant checkpoint policy is admitted: {name}")
    ...
    return {"enabled": True, "kind": "checkpoint-input-activations-to-pinned-CPU-v1",
            "modules": [name for name, _ in selected], "weights_offloaded": False,
            "gradients_offloaded": False, "parameter_identity_preserved": True}


def input_offload_statistics(model):
    rows = [module._archlab_input_offload_stats for module in model.modules()
...
def qualify_replay_memory(model, optimizer, indexers, prompts, *, config, policy_version, stops, pad):
    ...
    watcher.start()
    try:
        rollout = sample_rollouts(model, extended, policy_version=policy_version,
            max_new_tokens=1, context_limit=config["context_limit"], eos_token_ids=stops,
            pad_token_id=pad, seed=config["seed"] + 3000000, retain_weights=False)
        rewards = torch.zeros(len(prompts) // config["group_size"], config["group_size"], device=device)
        rewards[:, 0] = 1
        audit = policy_gradient_step(model, optimizer, indexers, rollout, rewards,
            lr=config["learning_rate"], group_size=config["group_size"],
            replay_mode="sampled-prefix", replay_prefixes=1, replay_seed=config["seed"],
            replay_tolerance=config["replay_tolerance"], audit_only=True,
    ...
    local = {"rank": dist.get_rank(), "minimum_driver_free_bytes": minimum_free[0],
    ...
    return {"passed": all(row["replay_verified"] and row["optimizer_state_empty"]
                          and row["minimum_driver_free_bytes"] >= reserve for row in rows),
            "kind": "maximum-context-replay-memory-v1", "context_limit": config["context_limit"],
            ...
            "optimizer_updates": 0, "synthetic_tokens_for_memory_admission_only": True, "ranks": rows}
