...
def qualify_distributed_optimizer():
    ...
    experts = init_device_mesh('cuda', (world // 8, 8), mesh_dim_names=('optimizer_fsdp','optimizer_ep'))
    results = []
    for shape, mesh, placements in [((19,), rows, (Shard(0),)), ((33, 7), rows, (Shard(0),)),
        ...
        opt = ShardedAdafactor([actual], lr=.01, stochastic_rounding=False, chunk_elements=21)
        ...
        reference = ShardedAdafactor([expected], lr=.01, stochastic_rounding=False, chunk_elements=10**9)
        errors = []
        for step in range(3):
