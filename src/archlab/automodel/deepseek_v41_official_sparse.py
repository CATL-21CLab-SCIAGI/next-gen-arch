...
def _pad_queries(value, size, fill=0):
    ...
    return torch.cat((value, value.new_full((size - value.shape[0], *value.shape[1:]), fill))).contiguous()
...
class _DeterministicSparse(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        from nemo_automodel.components.models.deepseek_v4.kernels import tilelang_sparse_mla_bwd as kernels
        ...
        for first_head in range(0, heads, max_heads):
            ...
            deltas.append(kernels.preprocess(batch, sequence, last_head - first_head, dim)(
        ...
        for b in range(batch):
            for first in range(0, sequence, chunk):
                ...
                for h, first_head in enumerate(range(0, heads, max_heads)):
                    last_head = min(first_head + max_heads, heads)
                    kernel = kernels.bwd(chunk, 1, slots, last_head - first_head, dim, slots, ctx.sm_scale)
                    ...
                    normalizer = _pad_queries(lse[b, first:last, first_head:last_head], chunk, float("inf")).unsqueeze(1)
                    delta = _pad_queries(deltas[h][b, first:last], chunk).unsqueeze(1)
                    partial = torch.zeros_like(combined)
                    unused_sink = torch.zeros(last_head - first_head, device=q.device, dtype=torch.float32)
                    grad_q = kernel(query, private_kv, dout, sinks[first_head:last_head].contiguous(),
                                    private_indices, valid_mask, normalizer, delta, partial, unused_sink)
                    dq[b, first:last, first_head:last_head].copy_(grad_q[:count, 0])
                    combined.add_(partial)
                if ctx.needs_input_grad[1]:
                    flat_valid = valid.reshape(-1)
                    _ordered_index_sum(dkv[b], original.reshape(-1)[flat_valid].long(),
        ...
        return (dq if ctx.needs_input_grad[0] else None,
                ...
                dsink, None, None, None, None)


def deterministic_sparse_attention(q, kv, sinks, indices, scale, *, backend, reference_rounding=False):
    ...
    if backend != "tilelang" or not reference_rounding:
        raise ValueError("this precision boundary is specific to native-rounding V4.1 TileLang attention")
    if not torch.is_grad_enabled() or not (q.requires_grad or kv.requires_grad or sinks.requires_grad):
        return dsv4_sparse_attention(q, kv, sinks, indices, scale,
                                     backend=backend, reference_rounding=reference_rounding)
    ...
    output = _DeterministicSparse.apply(q.contiguous(), kv.contiguous(), sinks.float().contiguous(),
        indices.to(torch.int32).contiguous(), scale, 16 if q.shape[-1] >= 256 else 64, 128)
...
def install_official_deterministic_sparse(model):
    ...
    if after.keys() != parameters.keys() or any(after[name] is not p for name, p in parameters.items()):
        raise RuntimeError("sparse precision adaptation changed a parameter")
    model._archlab_deterministic_sparse_installed = True
    return {"implementation": "official-sparse-kernels-with-private-query-KV-reduction-v1",
            "modules": [name for name, _ in selected], "forward": "unchanged-official-native-rounding",
            "query_chunk": 128, "kv_gradient_dtype": "float32", "reduction": "stable-key-sort-segment-sum",
            "upstream_globals_unchanged": True, "original_parameters_preserved": True}
