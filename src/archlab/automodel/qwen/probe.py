...
from archlab.automodel.qwen.checkpointing import assert_state_equal, checkpoint_payload
from archlab.automodel.qwen.execution import build_frozen_base, emit, tiny_config
from archlab.automodel.qwen.runtime import configure_frozen_gdn_runtime
from archlab.automodel.qwen.simplicial import (
    ADAPTER_MARKER,
    UPSTREAM_COMMIT,
    AdditiveMoERead,
...
def main():
    ...
    try:
        ...
        emit("runtime", upstream=revision,
             ...
             project_source_sha256={str(p.relative_to(Path(__file__).parents[3])): hashlib.sha256(p.read_bytes()).hexdigest()
                                    for p in sorted([*Path(__file__).parent.glob("*.py"),
                                                     *Path(__file__).parents[2].joinpath("architectures").glob("*.py")])})
        ...
        if args.data_root is not None:
            from archlab.automodel.qwen.data import load_fineweb_windows
        ...
        if args.check_ep_reference:
            from archlab.automodel.qwen.numerics import compare_combined_batch_reference
