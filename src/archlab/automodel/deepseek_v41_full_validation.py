...
def admit_evaluation_upgrade(saved, current, update_function):
    ...
    if {k:v for k,v in saved.items() if k not in mutable} != {k:v for k,v in current.items() if k not in mutable}:
    ...
    if set(new) != set(old) | {validation} or any(new[k] != v for k,v in old.items() if k != trainer):
        raise ValueError('resume changed a protected model, optimizer, checkpoint, or data implementation')
...
def make_plan(pilot, target_budget):
    ...
    payload = {'format':'archlab-full-periodic-eval-v1', 'targets':target_budget,
               'pilot_windows_sha256':pilot.manifest['windows_sha256'],
               'source_ready_sha256':pilot.manifest['source_ready_sha256'],
               'order_seed':pilot.order_seed, 'windows':selected}
...
def head_totals(hidden, labels, weight, chunk_size=128):
    ...
    for ids in positions.split(chunk_size):
        ...
        total[3] += (logits.topk(min(5, logits.shape[-1]), -1).indices == selected[:,None]).any(-1).sum()
...
def evaluate_batches(model, batches):
    ...
    ce = nll/count
    return {'loss':ce, 'cross_entropy':ce, 'perplexity':math.exp(ce),
            'top1_token_accuracy':top1/count, 'top5_token_accuracy':top5/count,
            'mean_predictive_entropy':entropy/count, 'targets':int(count), 'seconds':float(seconds)}
...
def evaluate_pilot(model, pilot, plan):
    ...
    batches = (capped_batch(pilot, entries[first+rank] if first+rank < len(entries) else None, 'cuda')
