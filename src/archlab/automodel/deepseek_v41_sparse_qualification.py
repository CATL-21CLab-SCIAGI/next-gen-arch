...
def qualify_batched_sparse():
    ...
    with torch.random.fork_rng(devices=[torch.cuda.current_device()]):
        ...
        scores = torch.einsum("bthd,btkd->bthk", x, selected) * d**-0.5
        scores = scores.masked_fill(indices[:, :, None, :] < 0, -torch.inf)
        probabilities = torch.cat(
            (scores, s[None, None, :, None].expand(b, t, -1, -1)), -1
        ).softmax(-1)[..., :-1]
        expected = torch.einsum("bthk,btkd->bthd", probabilities, selected)
        reference = [
            expected.detach(),
            *torch.autograd.grad(expected, (x, k, s), cotangent.float()),
        ]
        x, k, s = [v.detach().requires_grad_() for v in (q, kv, sinks)]
        actual = high_mfu_sparse_attention(
            x, k, s, indices, d**-0.5, backend="tilelang", reference_rounding=True
        )
        values = [actual.detach(), *torch.autograd.grad(actual, (x, k, s), cotangent)]
        errors = {}
        for name, value, target in zip(("output", "dq", "dkv", "dsink"), values, reference, strict=False):
            ...
            errors[name] = error
        with torch.no_grad():
            native = dsv4_sparse_attention(
                q, kv, sinks, indices, d**-0.5, backend="tilelang", reference_rounding=True
            )
        ...
        return {
            ...
            "relative_l2_to_fp32": errors,
            "relative_l2_limit": 0.01,
            "masked_and_duplicate_indices": True,
            "rng_preserved": True,
            "shape": [b, t, h, d],
            "slots": slots,
        }
