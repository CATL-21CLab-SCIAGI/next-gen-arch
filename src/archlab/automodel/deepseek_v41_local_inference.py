...
def cpu_engram_lookup(module, indices):
    ...
    values = (
        ...
        .index_select(0, ids)
        .to(indices.device)
        .view(module.weight.dtype)
    )
    scales = (
        module.scale.view(torch.uint8)
        .index_select(0, ids)
        .to(indices.device)
        .view(module.scale.dtype)
    )
...
class DecodedExpertCache:
    ...
    def get(self, key, expert):
        ...
        for name in ("w1", "w2", "w3"):
            ...
            decoded[name] = dequantize_frozen_weight(
                weight.cuda(non_blocking=True), scale.cuda(non_blocking=True)
            )
...
def _adapter_block(self, x, start_pos, pre_mix, image_mask, *attn_args):
    ...
    if self._adapter_mask is not None and bool(self._adapter_mask.any()):
        ...
        self._adapter_history = history[
            :, -(self.simplicial_adapter.config.long_window - 1) :
        ].clone()
...
class LocalV41Inference:
    def __init__(
        ...
        *,
        ...
        self.files = {}
        self.mapping = json.loads((self.weights / "model.safetensors.index.json").read_text())[
            "weight_map"
        ]
        self.loaded = set()
        ...
        self.reference = load_native_reference(
            self.assets, module_name="_archlab_local_v41_reference"
        )
        ...
        with native.set_dtype(torch.bfloat16), torch.device("cpu"):
            self.model = (
                native.Transformer(args, tokenizer=self.tokenizer).requires_grad_(False).eval()
            )
        ...
        print(
            json.dumps({"event": "local_model_constructed", "parameter_tensors": len(expected)}),
            flush=True,
        )
        for number, (name, target) in enumerate(expected.items()):
            value = self.tensor(name)
            if name.endswith(".attn.wo_a.weight"):
                value = dequantize_frozen_weight(
                    value.cuda(), self.tensor(name.removesuffix("weight") + "scale").cuda()
                )
        ...
        unused = [
            name for name in self.mapping if name not in self.loaded and not allowed.search(name)
        ]
        if unused:
        ...
        for name, module in list(self.model.named_modules()):
            if re.fullmatch(r"layers\.\d+\.ffn\.experts\.\d+", name):
                module._cpu_weights = {
                    p: (getattr(module, p).weight, getattr(module, p).scale)
                    for p in ("w1", "w2", "w3")
                }
            ...
            elif isinstance(module, native.Linear) and not re.search(
                r"\.ffn\.experts\.\d+\.", name
            ):
                if module.weight.dtype == torch.float8_e4m3fn:
                    ...
                    for first in range(0, target.shape[0], 1024):
                        ...
                        target[first:last].copy_(
                            dequantize_frozen_weight(
                                module.weight[first:last].cuda(),
                                module.scale[first // 32 : (last + 31) // 32].cuda(),
                            )
                        )
        ...
        for key, state in payload["adapters"].items():
            index = int(key)
            adapter = V41SimplicialAdapter(
                V41AdapterConfig(), seed=42 + index, backend="deterministic"
            )
        ...
        self.report = {
            "hardware": "NVIDIA B300 (NVML L20D mislabel)",
            "python": os.sys.executable,
            "packages": {
                p: importlib.metadata.version(p)
                for p in ("torch", "triton", "tilelang", "transformers", "safetensors")
            },
            "container": {
                k: os.environ.get(k)
                for k in (
            ...
            "adapter_sha256": marker["state_sha256"],
            "trainable_checkpoint_parameters": marker["contract"]["trainable_parameters"],
            "frozen_tensors_loaded": len(self.loaded),
            "inactive_tensors": len(self.mapping) - len(self.loaded),
            "expert_cache_gib": expert_cache_gib,
            "cpu_expert_store": None if self.cpu_experts is None else self.cpu_experts.report,
            "context": context,
            "max_batch_size": max_batch_size,
            "construction_seconds": time.monotonic() - started,
            "resident_gpu_gib": torch.cuda.memory_allocated() / 2**30,
        }
    ...
    def tensor(self, name):
        ...
        if filename not in self.files:
            self.files[filename] = self.readers.enter_context(
                safe_open(self.weights / filename, framework="pt", device="cpu")
            )
        self.loaded.add(name)
    ...
    def forward(self, ids, *, adapted, start_pos=0, return_hidden=False):
        if (
            ids.ndim != 2
            or ids.shape[0] > self.max_batch_size
            or start_pos + ids.shape[1] > self.context
        ):
        ...
        captures = []
        handle = (
            self.model.norm.register_forward_hook(lambda m, a, o: captures.append(o))
            if return_hidden
            else None
        )
        try:
