...
def main():
    ...
    for name in sorted(set(mapping.values())):
        ...
        for key, value in header.items():
            ...
            if match and int(match.group(1)) >= args.first_layer:
                intervals.append(
                    (8 + size + value["data_offsets"][0], 8 + size + value["data_offsets"][1])
                )
        merged = []
        for lo, hi in sorted(intervals):
            if merged and lo <= merged[-1][1] + 65536:
                merged[-1] = (merged[-1][0], max(hi, merged[-1][1]))
            else:
                merged.append((lo, hi))
        for lo, hi in merged:
            for start in range(lo, hi, 32 * 1024**2):
                work.append((path, start, min(32 * 1024**2, hi - start)))
    ...
    def read(item):
        ...
        if len(data) != count:
            raise OSError(f"short read: {path}:{start}")
        return count
    ...
    print(
        json.dumps({"event": "prefetch_start", "bytes": total, "threads": args.threads}), flush=True
    )
