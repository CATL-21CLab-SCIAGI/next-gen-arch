...
def marker_contract(path, *, variant, expected_tokens=None):
    ...
    if marker['format']!='archlab-v41-full-sharded-v1' or marker['world_size']!=16:
        raise ValueError('evaluation requires a complete full-state checkpoint on the trained16-rank mesh')
    c=marker['contract']
    if c['variant']!=variant or (c['world_size'],c['ep_size'],c['expert_fsdp_size'],c['engram_owners'])!=(16,8,2,16):
...
def restore_weights(model,path,*,variant,expected_tokens=None):
    import torch
    ...
    marker=marker_contract(path,variant=variant,expected_tokens=expected_tokens)
    if dist.get_world_size()!=marker['world_size']:raise ValueError('wrong restore mesh size')
    rank=dist.get_rank();rank_path=Path(path)/f'rank-{rank:02d}'
    ...
    count=0;loaded=0;threshold=8*2**30;began=time.monotonic()
    ...
    with torch.no_grad():
        for (name,tensor),entry in zip(named,manifest['tensors'],strict=True):
            ...
            if name!=entry['name'] or list(local.shape)!=entry['shape'] or list(tensor.shape)!=entry['global_shape'] or str(local.dtype)!=entry['dtype']:
                raise ValueError(f'checkpoint tensor shape/name/dtype mismatch: {name}')
            if not local.is_contiguous():raise ValueError(f'restore destination must be contiguous: {name}')
            flat=local.view(-1);first=0
            for chunk in entry['chunks']:
                host=torch.load(rank_path/chunk['file'],map_location='cpu',weights_only=True)
                if host.numel()!=chunk['elements'] or host.dtype!=local.dtype or _checksum(host)!=chunk['sha256']:
                    raise ValueError(f'checkpoint chunk checksum or dtype mismatch: {name}')
                flat[first:first+host.numel()].copy_(host)
                first+=host.numel();loaded+=host.numel()*host.element_size();count+=1
                del host
            if first!=flat.numel():raise ValueError(f'incomplete parameter restore: {name}')
            if loaded>=threshold:
                emit('eval_weight_restore_progress',variant=variant,loaded_local_gib=loaded/2**30,seconds=time.monotonic()-began)
                threshold+=8*2**30
    ...
    result={'variant':variant,'rank':rank,'cursor':marker['cursor'],'tensor_entries':len(named),
            'verified_payload_chunks':count,'loaded_local_gib':loaded/2**30,'seconds':time.monotonic()-began,
            'all_weight_payload_sha256_verified':True,'optimizer_loaded':False,'cpu_weight_offload':False}
