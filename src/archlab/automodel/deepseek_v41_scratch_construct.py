...
def construct_scratch(*,base_config,assets,variant,tiny=False):
    ...
    adapters=install_official_adapters(model,V41AdapterConfig(width=256,head_dim=16) if tiny else V41AdapterConfig(width=640,head_dim=16),layer_indices=(1,3,5) if tiny else adapter_layers(),device='cuda',variant=variant,backend='deterministic' if variant=='simplicial' else 'flash-attn-deterministic',allow_right_padding=True)
