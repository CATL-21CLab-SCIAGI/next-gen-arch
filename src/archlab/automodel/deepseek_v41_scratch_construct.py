...
def construct_scratch(*, base_config, assets, variant, tiny=False):
    ...
    from archlab.architectures.deepseek_v41_scratch import adapter_layers, scaled_scratch_config
    ...
    if dist.get_world_size() != 8:
        raise ValueError("scratch contract requires one8-GPU node per variant")
    hosts = [None] * 8
    dist.all_gather_object(hosts, socket.gethostname())
    if len(set(hosts)) != 1:
        raise ValueError("each variant must occupy exactly one node")
    configure_official_reproducibility()
    ...
    config = (
        ...
        if tiny
        else DeepseekV41Config(**scaled_scratch_config(json.loads(Path(base_config).read_text())))
    )
    ...
    adapters = install_official_adapters(
        model,
        V41AdapterConfig(width=256, head_dim=16)
        if tiny
        else V41AdapterConfig(width=640, head_dim=16),
        layer_indices=(1, 3, 5) if tiny else adapter_layers(),
        device="cuda",
        variant=variant,
    ...
    logical = sum(
        local_tensor(p).numel() / (8 if not hasattr(p, "placements") else 1)
