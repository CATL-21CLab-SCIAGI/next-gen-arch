...
def save_full_checkpoint(path, model, optimizer, cursor, contract):
    ...
    manifest = {'rank': rank, 'world_size': world, 'tensors': [], 'cursor': cursor, 'contract': contract}
    ...
    for number, (name, tensor) in enumerate(named):
        ...
        entry = {'name': name, 'shape': list(local.shape), 'global_shape': list(tensor.shape),
        ...
        for part, first in enumerate(range(0, flat.numel(), count)):
            ...
            entry['chunks'].append({'file': filename, 'elements': host.numel(), 'sha256': _checksum(host)})
            del host
        manifest['tensors'].append(entry)
    states = []
    for number, p in enumerate(optimizer.param_groups[0]['params']):
        state = optimizer.state[p]
        payload = {k: v.detach().cpu() if isinstance(v, torch.Tensor) else v for k, v in state.items()}
    ...
    manifest['optimizer_options'] = [{k: v for k, v in group.items() if k != 'params'} for group in optimizer.param_groups]
    torch.save({'cpu_rng': torch.get_rng_state(), 'cuda_rng': torch.cuda.get_rng_state()}, rank_path / 'rng.pt')
    atomic_write_json(rank_path / 'MANIFEST.json', manifest)
    dist.barrier()
    if rank == 0:
        atomic_write_json(path / 'COMPLETE.json', {'format': 'archlab-v41-full-sharded-v1', 'world_size': world,
            'cursor': cursor, 'contract': contract, 'manifests': [f'rank-{r:02d}/MANIFEST.json' for r in range(world)]})
    dist.barrier()
...
def restore_full_checkpoint(path, model, optimizer, contract):
    ...
    if marker['format'] != 'archlab-v41-full-sharded-v1' or marker['world_size'] != dist.get_world_size() or marker['contract'] != contract:
    ...
    for (name, tensor), entry in zip(named, manifest['tensors'], strict=True):
        ...
        if name != entry['name'] or list(local.shape) != entry['shape'] or str(local.dtype) != entry['dtype']:
    ...
    for p, filename in zip(optimizer.param_groups[0]['params'], manifest['optimizer_states'], strict=True):
        saved = torch.load(rank_path / filename, map_location='cpu', weights_only=True)
        optimizer.state[p] = {k: v.to(p.device) if isinstance(v, torch.Tensor) else v for k, v in saved.items()}
