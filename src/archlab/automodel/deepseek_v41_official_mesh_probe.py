...
def _mesh_receipt(model, setup, ep_size):
    ...
    for name, module in model.named_modules():
        if isinstance(module, DeepseekV41Engram):
            ...
            ranks = (
                []
                if table.process_group is None
            ...
            engrams.append(
                {
                    ...
                    "local_shape": list(weight.to_local().shape)
    ...
    return {
        ...
        "ep_hosts": ep_hosts,
        "expert_fsdp_ranks": ep_shard.mesh.tolist(),
        "engrams": engrams,
    }
...
def _adapter_receipt(adapters, *, require_nonzero):
    ...
    for name, parameter in named:
        ...
        if parameter.grad is None or not bool(parameter.grad.isfinite().all()):
            ...
            gradients.append(
                {"name": name, "nonzero": nonzero, "max_abs": float(parameter.grad.abs().max())}
            )
    _require_all(errors)
    ...
    return {
        "replicated_fp32_masters": True,
        "world_gradient_agreement": True,
        "world_parameter_agreement": True,
        "gradient_sha256": gradient_sha,
        "adapter_sha256": parameter_sha,
        "gradients": gradients,
    }
...
def _diagnostic_comparison(before, expected_metric, expected, actual_metric, actual):
    ...
    def norms(initial, reference, observed):
        ...
        def visit(start, target, value):
            ...
            if isinstance(target, torch.Tensor):
                chunks = [
                    tensor.detach().cpu().reshape(-1).split(1024 * 1024)
    ...
    return {
        "within_existing_tolerance": not any(failures),
        ...
        "loss": {
            "expected": expected_metric["loss"],
            "actual": actual_metric["loss"],
            "abs_difference": abs(actual_metric["loss"] - expected_metric["loss"]),
        },
        **{
            component: {
                **state_difference_statistics(expected[component], actual[component]),
...
def _frozen_base_receipt(model, frozen_names, frozen_sha):
    ...
    errors = [
        ...
        if name not in current or current[name].requires_grad or current[name].grad is not None
    ]
    _require_all(errors)
    ...
    return {
        "unchanged": True,
        "no_gradients": True,
        "local_sha256": frozen_sha,
        "parameter_tensors": len(frozen_names),
    }
...
def qualify(
    *,
    ...
    context=128,
    also_512=False,
    diagnose_replay=False,
    diagnostic_repeats=3,
    adapter_variant="simplicial",
):
    import torch
    import torch.distributed as dist
    ...
    model, setup, base_report = build_official_base(
        weights=weights, assets=assets, ep_size=ep_size, tiny=True
    )
    report = {
        **base_report,
        "mesh": _mesh_receipt(model, setup, ep_size),
        "context": context,
        "optimizer": "AdamW probe only",
        "passed": False,
    }
    ...
    optimizers = [
        torch.optim.AdamW(
            parameters, lr=1e-5, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0, foreach=False
        )
    ]
    report["adapter"] = {
    ...
    _require_all(
        [] if len(set(windows)) == dist.get_world_size() else ["duplicate data-rank inputs"]
    )
    ...
    contract = {
        "probe": "official-v41-tiny-mesh-v3",
    ...
    cursor = {
        "step": 2,
        "supervised_tokens": sum(row["supervised_tokens"] for row in report["updates"]),
    }
    ...
    restored_cursor = restore_adapter_checkpoint(
        checkpoint, adapters, optimizers, contract=contract
    )
    _require_all([] if restored_cursor == cursor else ["checkpoint changed the data cursor"])
    errors = []
    try:
        restoration = assert_exact_training_state(
            saved_state, capture_adapter_training_state(adapters, optimizers)
        )
    except AssertionError as error:
    ...
    if diagnose_replay:
        diagnostic = {
            ...
            "checkpoint_replay": {
                ...
                "exact_restoration": restoration,
                "metric": replay_metric,
                **_diagnostic_comparison(
                    saved_state, expected_metric, expected_state, replay_metric, replay_state
                ),
                **_adapter_receipt(adapters, require_nonzero=True),
            },
            "in_memory_repeats": [],
        ...
        for repeat in range(diagnostic_repeats):
            ...
            errors = []
            try:
                exact = assert_exact_training_state(
                    saved_state, capture_adapter_training_state(adapters, optimizers)
                )
            except AssertionError as error:
            ...
            measured = {
                ...
                "metric": metric,
                **_diagnostic_comparison(
                    saved_state, expected_metric, expected_state, metric, state
                ),
                "difference_from_checkpoint_replay": {
                    component: state_difference_statistics(
                        replay_state[component], state[component]
                    )
                    for component in ("adapters", "optimizers")
                },
                **_adapter_receipt(adapters, require_nonzero=True),
...
def main():
    ...
    dist.init_process_group(
        "nccl", timeout=datetime.timedelta(minutes=120), device_id=torch.device("cuda", local_rank)
    )
    receipt = {
        "rank": dist.get_rank(),
        "world_size": dist.get_world_size(),
    ...
    try:
        ...
        receipt.update(
            qualify(
                ...
                also_512=args.also_512,
                diagnose_replay=args.diagnose_replay,
        ...
        atomic_write_json(
            args.output / f"rank{dist.get_rank()}{suffix}.json", receipt, allow_nan=False
        )
        dist.barrier()
        if dist.get_rank() == 0:
            ...
            atomic_write_json(
                args.output / marker,
                {
                    "passed": not args.diagnose_replay,
                    "diagnostic_only": args.diagnose_replay,
                    "world_size": dist.get_world_size(),
                    "rank_receipts": [
                        f"rank{rank}{suffix}.json" for rank in range(dist.get_world_size())
                    ],
                },
                allow_nan=False,
            )
        ...
        if fresh_output:
            atomic_write_json(
                args.output / f"rank{dist.get_rank()}-failed.json", receipt, allow_nan=False
            )
        raise
    finally:
