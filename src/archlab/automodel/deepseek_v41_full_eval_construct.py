...
def build_eval_shell(*,weights:Path,assets:Path,tiny:bool):
    ...
    if dist.get_world_size()!=16:raise ValueError('full evaluation uses the trained16-rank layout')
    ...
    setup=DistributedSetup.build(strategy=FSDP2Config(mp_policy=precision,reshard_after_forward=True,sequence_parallel=False),
        ...
        activation_checkpointing=True,world_size=16)
    group=setup.mesh_context.moe_mesh['ep'].get_group();hosts=[None]*8
    dist.all_gather_object(hosts,socket.gethostname(),group=group)
    if len(set(hosts))!=1:raise ValueError('expert groups must be node local')
    if tiny:config=tiny_official_config(assets,experts=16)
    else:
        config=DeepseekV41Config.from_pretrained(weights,local_files_only=True);config.name_or_path=str(weights)
        config.vision_config.num_hidden_layers=0;t=config.text_config
        if (t.hidden_size,t.num_hidden_layers,t.n_routed_experts,t.num_experts_per_tok,t.hc_mult)!=(5120,40,384,6,4):
            raise ValueError('wrong full V4.1 backbone geometry')
    backend=BackendConfig(attn='tilelang',linear='torch',rms_norm='torch_fp32',experts='torch_mm',dispatcher='torch',
        gate_precision='float32',rope_fusion=False,fake_balanced_gate=False,enable_hf_state_dict_adapter=True)
    emit('eval_model_layout_start',tiny=tiny)
    model=NeMoAutoModelForCausalLM.from_config(config,load_base_model=False,backend=backend,distributed_setup=setup,
        torch_dtype=torch.bfloat16,trust_remote_code=False,force_hf=False,use_liger_kernel=False,use_sdpa_patching=False,
        freeze_config={'freeze_modules':[{'glob':'*'}]})
    model.requires_grad_(False)
    moe=install_official_fp32_moe(model);hc=install_official_native_hc(model,assets);sparse=install_official_deterministic_sparse(model)
    local_bytes=sum((p.to_local() if isinstance(p,DTensor) else p).numel()*p.element_size() for p in model.parameters())
    report={**identity,'world_size':16,'ep_size':8,'expert_fsdp_size':2,'engram_owners':16,
            'construction':'from_config, initialize after sharding, full-checkpoint restore required',
            'released_base_weights_loaded':False,'local_parameter_gib':local_bytes/2**30,
            'tiny':tiny,'moe_precision':moe,'hc_precision':hc,'sparse_precision':sparse}
