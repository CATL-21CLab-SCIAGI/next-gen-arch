...
def qualify_indexer_memory(config, *, sequence):
    from nemo_automodel.components.models.deepseek_v41.attention import _Indexer, _RotaryEmbedding, DeepseekV41AttentionState
    reports = []
    with torch.random.fork_rng(devices=[torch.cuda.current_device()]):
        ...
        selected_layers = [config.kv_source_layer_ids[0], config.candidate_source_layer_id,
                           next((x for x in config.index_source_layer_ids if x > config.candidate_source_layer_id), None)]
        for layer in dict.fromkeys(x for x in selected_layers if x is not None and x >= 0):
            ...
            state = DeepseekV41AttentionState(compressed_kv=latent.clone(), compression_ratio=ratio,
            ...
            for fn in (module.forward, bind_memory_efficient_selector(module)):
                ...
                outputs.append(fn(inputs, query_latent=queries, latent=latent if module.owns_keys else None,
                    angles=angles, compressed_angles=angles[:, :width*ratio:ratio], state=state))
            ...
            reports.append({'layer':layer,'sequence':sequence,'byte_exact':True,
                            'original_peak_gib':peaks[0],'optimized_peak_gib':peaks[1]})
