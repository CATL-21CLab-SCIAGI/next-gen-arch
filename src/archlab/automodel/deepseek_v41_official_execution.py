...
def official_core_hashes():
    ...
    names = ["architectures/deepseek_v41_normal_adapter.py", "architectures/local_attention.py",
             ...
             "architectures/simplicial_deterministic.py", "architectures/ordered_reduction.py",
             *[f"automodel/deepseek_v41_{name}.py" for name in
               ("data", "loss", "training", "execution", "loading", "parallel", "runtime", "autograd",
                "native_quantization", "pytorch", "official_execution", "official_adapter",
                "official_reference", "official_training", "official_moe", "official_hc", "official_sparse")]]
...
def runtime_identity():
    ...
    return {
        "automodel_root": str(root), "automodel_commit": commit,
        "container_image": os.environ.get("NGA_CONTAINER_DIGEST"),
        "packages": {name: importlib.metadata.version(name) for name in
                     ("torch", "transformers", "transformer-engine", "megatron-core", "triton", "flash-attn")},
...
def tiny_official_config(assets: Path, *, experts=16):
    from nemo_automodel.components.models.deepseek_v41.config import (
        DeepseekV41Config, DeepseekV41TextConfig, DeepseekV41VisionConfig,
    )

    text = DeepseekV41TextConfig(
        vocab_size=129280, hidden_size=256, moe_intermediate_size=128,
        num_hidden_layers=6, num_attention_heads=4, head_dim=64, qk_rope_head_dim=32,
        q_lora_rank=64, o_lora_rank=64, o_groups=2,
        n_routed_experts=experts, num_experts_per_tok=2,
        compress_ratios=[0, 0, 2, 2, 1, 1], kv_source_layer_ids=[2, 4],
        index_source_layer_ids=[2, 4], index_n_heads=4, index_head_dim=64,
        index_topk=16, candidate_source_layer_id=4, candidate_topk_blocks=4,
        candidate_block_size=4, engram_layer_ids=[1], engram_num_embeddings=[8192],
        engram_head_dim=32, engram_n_heads=2, engram_vocab_size=100,
        engram_compressed_vocab_size=99092, max_position_embeddings=16384,
    )
    config = DeepseekV41Config(text_config=text, vision_config=DeepseekV41VisionConfig(num_hidden_layers=0))
    config.name_or_path = str(assets)
    return config


def build_official_base(*, weights: Path, assets: Path, ep_size=8,
                        activation_checkpointing=True, tiny=False):
    import torch
    ...
    setup = DistributedSetup.build(
        ...
        moe_parallel_config=MoEParallelizerConfig(
            mp_policy=precision, lm_head_precision=torch.float32,
    ...
    emit("official_mesh", world_ranks=mesh.device_mesh.mesh.tolist(),
         world_axes=list(mesh.device_mesh.mesh_dim_names),
         moe_ranks=mesh.moe_mesh.mesh.tolist(), moe_axes=list(mesh.moe_mesh.mesh_dim_names))
    if tiny:
        ...
        if (text.hidden_size, text.num_hidden_layers, text.n_routed_experts,
                text.num_experts_per_tok, text.hc_mult) != (5120, 40, 384, 6, 4):
            raise ValueError("checkpoint is not the selected DeepSeek V4.1 Flash backbone")
    backend = BackendConfig(attn="tilelang", linear="torch", rms_norm="torch_fp32",
                            experts="torch_mm", dispatcher="torch", gate_precision="float32",
                            rope_fusion=False, fake_balanced_gate=False,
                            enable_hf_state_dict_adapter=True)
    emit("official_construct_load_start", checkpoint=str(weights), tiny=tiny)
    model = NeMoAutoModelForCausalLM.from_config(
        config, load_base_model=not tiny, backend=backend, distributed_setup=setup,
        torch_dtype=torch.bfloat16, trust_remote_code=False, force_hf=False,
        use_liger_kernel=False, use_sdpa_patching=False,
    ...
    report = {**identity, "backend": "official-automodel-deepseek-v41", "tiny": tiny,
              "world_size": dist.get_world_size(), "ep_size": ep_size,
              "expert_fsdp_size": dist.get_world_size() // ep_size,
              "engram_owners": dist.get_world_size(), "base_fsdp": True,
              "local_parameter_gib": local_bytes / 2**30,
              "attention": "tilelang", "sinkhorn": "native-tilelang", "hc_precision": hc_precision,
              "sparse_backward": sparse_precision,
              "experts": "torch_mm",
              "dispatcher": "torch", "linear_compute": "bfloat16", "moe_precision": moe_precision,
              "kv_index_quantization_retained": True, "lm_head_precision": "float32"}
...
def main():
    ...
    try:
        model, setup, report = build_official_base(weights=args.weights, assets=args.assets,
                                                 ep_size=args.ep_size, tiny=args.tiny)
