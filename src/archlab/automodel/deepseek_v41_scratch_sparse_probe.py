...
def main():
    ...
    for slots in (17, 65, 512):
        ...
        def reference(x, k, s):
            selected = k[torch.arange(batch, device='cuda')[:, None, None], indices.clamp_min(0).long()]
            logits = torch.einsum('bthd,btkd->bthk', x, selected) * dim**-.5
            logits = logits.masked_fill(indices[:, :, None, :] < 0, -torch.inf)
            probs = torch.cat((logits, s[None, None, :, None].expand(batch, length, -1, -1)), -1).softmax(-1)[..., :-1]
            return torch.einsum('bthk,btkd->bthd', probs, selected)

        versions = {}
        for name, fn in [('fp32', reference), ('official', dsv4_sparse_attention),
                         ('deterministic', deterministic_sparse_attention),
                         ('batched', high_mfu_sparse_attention), ('batched_repeat', high_mfu_sparse_attention)]:
            x, k, s = [t.detach().clone().to(torch.float32 if name == 'fp32' else t.dtype).requires_grad_()
                       for t in (q, kv, sinks)]
            if name == 'fp32':
                output = fn(x, k, s)
            else:
                output = fn(x, k, s, indices, dim**-.5, backend='tilelang', reference_rounding=True)
        ...
        for name, values in versions.items():
            metrics[name] = {}
            for field, actual, expected in zip(('output', 'dq', 'dkv', 'dsink'), values, versions['fp32']):
                if not bool(actual.isfinite().all()):
                    raise AssertionError(f'{name}/{field} is nonfinite')
                relative = float((actual.float()-expected).norm()/expected.norm().clamp_min(1e-20))
        ...
        metrics['batched_repeat']['exact_equal'] = {
            name: torch.equal(a, b) for name, a, b in zip(('output', 'dq', 'dkv', 'dsink'),
                                                        versions['batched'], versions['batched_repeat'])}
        results.append({'shape': [batch, length, heads, dim], 'slots': slots, 'metrics': metrics})
    report = {'passed': True, 'cases': results, 'hardware': 'NVIDIA B300 (NVML label L20D)',
              'container_image': os.environ.get('NGA_CONTAINER_DIGEST'),
              'resolved_kernel_packages': resolved_kernel_packages,
              'packages': {name: importlib.metadata.version(name) for name in ('torch', 'tilelang', 'triton')},
