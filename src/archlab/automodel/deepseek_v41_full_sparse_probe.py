    ...
    with torch.random.fork_rng(devices=[torch.cuda.current_device()]):
        ...
        for fn in (dsv4_sparse_attention, deterministic_sparse_attention):
            x, k, s = (
                ...
                kv.clone().requires_grad_(),
                sinks.clone().requires_grad_(),
            )
            y = fn(x, k, s, indices, 64**-0.5, backend="tilelang", reference_rounding=True)
            y.backward(cotangent)
            results.append((y.detach(), x.grad, k.grad, s.grad))
        assert torch.equal(results[0][0], results[1][0])
        relative = float(
            (results[1][-1] - results[0][-1]).norm() / results[0][-1].norm().clamp_min(1e-20)
        )
        if relative > 2e-5 or not torch.isfinite(results[1][-1]).all():
            raise AssertionError(
                f"attention sink gradient differs from official kernel: {relative}"
            )
