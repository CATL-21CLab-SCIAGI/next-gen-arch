...
def verify_checkpoint(path):
    ...
    for rank, relative in enumerate(marker["manifests"]):
        ...
        for filename in (manifest["optimizer_states"][0], manifest["optimizer_states"][-1]):
            state = torch.load(
                manifest_path.parent / filename, map_location="cpu", weights_only=True
            )
        ...
        if rank == 0:
            for name in ("model.embed_tokens.weight", "lm_head.weight"):
                ...
                value = torch.load(
                    manifest_path.parent / chunk["file"], map_location="cpu", weights_only=True
                )
    ...
    return {
        ...
        "rank_manifests": len(marker["manifests"]),
        "tensor_entries": tensors,
        "tensor_payload_chunks": tensor_chunks,
        "optimizer_state_files": optimizer_files,
        "tensor_and_optimizer_payload_gib": payload_bytes / 2**30,
        "representative_checksums": samples,
        "all_payload_lengths_verified": True,
        "all_rank_rng_states_present": True,
