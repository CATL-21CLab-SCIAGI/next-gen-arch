...
def read_rows(path):
    # Only complete records count: a reader can overlap an append.
...
'''.replace('PIDS', repr(pids)).replace('CHECKPOINT', repr(str(checkpoint)))
    command = ['ssh', '-i', str(key), '-o', 'BatchMode=yes', '-o', 'StrictHostKeyChecking=no',
               '-o', 'ConnectTimeout=10', f'root@{host}', '/opt/venv/bin/python -']
    try:
        ...
        for worker in data['workers']:
            positions = [index['files'][name]['preceding_bytes'] for name in worker['checkpoint_files'] if name in index['files']]
            worker['observed_weight_progress_pct'] = 100*max(positions)/index['total_weight_bytes'] if positions else None
        return data
...
def validate_resume(rows, receipt, expected, peer_rows):
    ...
    errors = []
    for row in resumed:
        for field, want in [('step', cursor['step']+1), ('phase_step', cursor['phase_step']+1),
        ...
        for field in ['loss', 'gradient_norm_before_clip', 'seconds', 'learning_rate', 'max_memory_allocated_gib']:
            if not math.isfinite(row[field]) or row[field] <= 0:
                errors.append(f"step {row['step']} invalid {field}: {row[field]}")
        if row['updated_parameter_tensors'] != rows[0]['updated_parameter_tensors'] or row['changed_local_elements'] <= 0:
        ...
        if row['step'] in peer:
            for field in ['input_tokens', 'supervised_tokens', 'consumed_supervised_tokens', 'phase_step', 'learning_rate']:
                if row[field] != peer[row['step']][field]:
                    errors.append(f"step {row['step']} differs from peer data/schedule: {field}")
        cursor = {'step': row['step'], 'phase_step': row['phase_step'], 'supervised_tokens': row['consumed_supervised_tokens']}
    if resumed:
...
def main():
    ...
    if not 0 < args.interval <= 60 or args.samples < 1 or args.healthy_updates < 1:
        raise ValueError('positive sampling arguments required; interval must be at most 60 seconds')
    ...
    for number in range(args.samples):
        with ThreadPoolExecutor(max_workers=len(receipt['worker_pids'])) as pool:
            nodes = list(pool.map(lambda item: node_sample(item[0], item[1], receipt['resume_checkpoint'], index, args.ssh_key), receipt['worker_pids'].items()))
        ...
        observations = [worker['observed_weight_progress_pct'] for worker in workers if worker['observed_weight_progress_pct'] is not None]
        good = len(resumed) >= args.healthy_updates and not errors and not failures and live_workers == receipt['gpus'] and all(node.get('gpus') and not node.get('error') for node in nodes)
        recent = resumed[1:] if len(resumed) > 1 else resumed
        seconds = sum(row['seconds'] for row in recent)
        snapshot = {'utc': datetime.now(timezone.utc).isoformat(), 'state': 'healthy' if good else 'checking updates' if resumed else 'restoring',
                    'resumed_updates': len(resumed), 'latest': rows[-1], 'peer_latest': peer_rows[-1], 'validation_errors': errors,
                    'failure_files': failures, 'live_workers': live_workers, 'nodes': nodes,
                    'observed_weight_progress_pct': [min(observations), max(observations)] if observations else None,
                    'steady_update_seconds_median': statistics.median(row['seconds'] for row in recent) if recent else None,
                    'steady_supervised_tokens_per_second': sum(row['supervised_tokens'] for row in recent)/seconds if seconds else None,
                    'steady_input_tokens_per_second': sum(row['input_tokens'] for row in recent)/seconds if seconds else None}
        (args.resume/'LATEST_HEALTH.json').write_text(json.dumps(snapshot, indent=2, allow_nan=False)+'\n')
        with (args.resume/'health-samples.jsonl').open('a') as stream:
            stream.write(json.dumps(snapshot, allow_nan=False)+'\n')
        print(json.dumps({k: snapshot[k] for k in ['utc', 'state', 'resumed_updates', 'live_workers', 'observed_weight_progress_pct', 'validation_errors', 'failure_files', 'steady_update_seconds_median']}, allow_nan=False), flush=True)
        if good:
            (args.resume/'HEALTH_VERIFIED.json').write_text(json.dumps(snapshot, indent=2, allow_nan=False)+'\n')
            return
        if errors or failures or any(not worker['alive'] for worker in workers):
            raise RuntimeError('resume health check found an actionable problem; see LATEST_HEALTH.json')
