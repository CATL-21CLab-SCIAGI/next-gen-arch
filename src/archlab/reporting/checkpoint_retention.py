...
# Retention receipts historically permit non-finite values and retain key order.
...
def validate(path):
    ...
    if len(marker['manifests']) != marker['world_size'] or len(set(marker['manifests'])) != marker['world_size']:
        raise ValueError('missing or duplicated rank manifests')
    sizes = {'torch.bfloat16': 2, 'torch.float16': 2, 'torch.float32': 4,
    ...
    for rank, relative in enumerate(marker['manifests']):
        ...
        for entry in manifest['tensors']:
            ...
            for chunk in entry['chunks']:
                payload = confined(path, str(manifest_path.parent.relative_to(path) / chunk['file']))
        ...
        for filename in [*manifest['optimizer_states'], 'rng.pt']:
            ...
            payloads += 1
    return {'path': str(path), 'cursor': marker['cursor'], 'ranks': marker['world_size'],
            'payload_files_checked': payloads, 'validated_utc': utc(),
...
def remove_checkpoint(path, replacement, audit):
    ...
    receipt = {'original': str(path), 'staged': str(staged), 'replacement': str(replacement),
               'bytes': size, 'started_utc': utc(), 'complete': False}
...
def sweep(config, audit, apply=False):
    ...
    for group in config['groups']:
        try:
            ...
            protected_steps = set(config.get('protected_steps', [])) | set(group.get('protected_steps', []))
            older = [x[2] for x in candidates[:-1]
                     if str(x[2]) not in config.get('protected', {})
                     and x[0] not in protected_steps
                     and not (x[2] / 'PINNED.json').exists()
                     and not (x[2] / 'EVAL_PENDING.json').exists()
                     and not any((x[2] / '.checkpoint-readers').glob('*.json'))]
            ...
            cache = audit / ('verified-' + hashlib.sha256(str(newest).encode()).hexdigest()[:20] + '.json')
            ...
            record = {'name': group['name'], 'newest': str(newest), 'delete': [str(x) for x in older]}
            if apply:
                for root in group.get('roots', []):
                    atomic_json(Path(root) / 'LATEST.json', {'path': str(newest), 'marker_sha256': digest, 'utc': utc()})
