...
def snapshot(path, destination):
    ...
    for i, line in enumerate(lines):
        ...
        rows.append(row)
    if not rows or any(b["step"] != a["step"] + 1 for a, b in zip(rows, rows[1:], strict=False)):
...
def window_mean(rows, start, low, high):
    ...
    weight = np.maximum(0.0, np.minimum(end, high) - np.maximum(end - count, low))
    if weight.sum() <= 0:
...
def main():
    ...
    for variant in LABELS:
        ...
        runs[variant], metadata[variant] = snapshot(
            run / "train-metrics.jsonl", output / f"{variant}-full-snapshot.jsonl"
        )
        contracts[variant] = json.loads((run / "RUN_CONTRACT.json").read_text())
        old = Path(contracts[variant]["start_checkpoint"]).parents[1]
        prior, prior_meta = snapshot(
            old / "train.jsonl", output / f"{variant}-adapter-snapshot.jsonl"
        )
        cursor = json.loads(
            (Path(contracts[variant]["start_checkpoint"]) / "COMPLETE.json").read_text()
        )["cursor"]
        ...
        (output / f"{variant}-contract.json").write_text(
            json.dumps(contracts[variant], indent=2) + "\n"
        )
    ...
    summary = {
        ...
        "matched_full_updates": matched,
        "matched_total_supervised_tokens": stop,
        "matched_full_phase_tokens": stop - start,
        "start_supervised_tokens": start,
        ...
        "evaluation": "held-out evaluation is reported separately",
        "variants": {},
        "sources": metadata,
    }
    for variant, rows in runs.items():
        ...
        summary["variants"][variant] = {
            "latest_updates": len(rows),
            "latest_total_tokens": rows[-1]["consumed_supervised_tokens"],
            ...
            "mean_ce_matched_last_10m": window_mean(
                exact, start, max(start, stop - 10000000), stop
            ),
            "mean_ce_matched_last_5m": window_mean(exact, start, max(start, stop - 5000000), stop),
            "mean_ce_exact_last_50_updates": sum(r["loss"] * r["supervised_tokens"] for r in last50)
            / sum(r["supervised_tokens"] for r in last50),
            ...
            "mean_update_seconds": duration / len(timed),
            "timed_matched_updates": len(timed),
    ...
    summary["normal_throughput_speedup"] = (
        b["supervised_tokens_per_second"] / a["supervised_tokens_per_second"]
    )
    summary["normal_minus_simplicial_ce_last_10m"] = (
        b["mean_ce_matched_last_10m"] - a["mean_ce_matched_last_10m"]
    )
    summary["simplicial_relative_ce_advantage_last_10m_percent"] = (
        100
        * (b["mean_ce_matched_last_10m"] - a["mean_ce_matched_last_10m"])
        / b["mean_ce_matched_last_10m"]
    )
    summary["normal_minus_simplicial_ce_whole_matched_phase"] = (
        b["mean_ce_matched_full_phase"] - a["mean_ce_matched_full_phase"]
    )
        ...
        for left, right in zip(runs["simplicial"][:matched], runs["normal"][:matched], strict=True):
            writer.writerow(
                [
                    left["step"],
                    left["phase_step"],
                    left["consumed_supervised_tokens"],
                    left["supervised_tokens"],
                    left["input_tokens"],
                    left["loss"],
                    right["loss"],
    ...
    if maximum > stop / 1e6:
        top.axvspan(stop / 1e6, maximum + 0.5, color="#edf0f4", zorder=0)
        top.text(
            (maximum + stop / 1e6) / 2,
            0.99,
            "Normal-only\nextra progress",
    ...
    top.set_xlim(start / 1e6 - 0.7, maximum + 0.5)
    all_losses = np.array([r["loss"] for rows in runs.values() for r in rows])
    top.set_ylim(all_losses.min() - 0.015, all_losses.max() + 0.025)
    top.set_ylabel("Training cross-entropy  ·  lower is better")
    top.set_xlabel("Total supervised tokens (millions)")
    top.set_title(
        "Full-weight fine-tuning: training-loss comparison", loc="left", pad=13, fontsize=14
    )
    ...
    paired_a = runs["simplicial"][:matched]
    paired_b = runs["normal"][:matched]
    x, ya = smooth(paired_a, start, args.smoothing_tokens)
    _, yb = smooth(paired_b, start, args.smoothing_tokens)
    difference = (yb - ya) * 1000
    delta.axhline(0, color="#69778c", lw=1, ls="--")
    delta.plot(x, difference, color="#4b5568", lw=1.8)
    delta.fill_between(
        x, 0, difference, where=difference >= 0, color=COLORS["simplicial"], alpha=0.2
    )
    delta.fill_between(x, 0, difference, where=difference < 0, color=COLORS["normal"], alpha=0.2)
    delta.set_xlim(start / 1e6 - 0.7, stop / 1e6 + 0.5)
    ...
    delta.text(
        0.02,
        0.96,
        "Above zero: 2-simplicial has lower CE",
    ...
    delta.text(
        0.02,
        0.04,
        "Below zero: normal has lower CE",
        transform=delta.transAxes,
        va="bottom",
        fontsize=8.5,
        color=COLORS["normal"],
    ...
    speed.barh([1, 0], values, color=[COLORS[v] for v in LABELS], height=0.43)
    speed.set_yticks([1, 0], [LABELS[v] for v in LABELS], fontsize=9)
    speed.set_xlim(0, max(values) * 1.24)
    speed.set_ylim(-0.65, 1.65)
    for y, value in zip([1, 0], values, strict=True):
        speed.text(value + max(values) * 0.03, y, f"{value:,.0f}", va="center", fontsize=10)
    speed.set_xlabel("Supervised tokens / second")
    speed.grid(axis="y", visible=False)
    speed.grid(axis="x", color="#edf0f4", lw=0.7)
    speed.set_title(
        f"Normal is {(summary['normal_throughput_speedup'] - 1) * 100:.1f}% faster",
        loc="left",
        pad=10,
        fontsize=12,
    )
    speed.text(
        0.5,
        -0.33,
        "Matched updates after warmup.\nCheckpoint I/O excluded.",
        ha="center",
        va="top",
        transform=speed.transAxes,
        fontsize=8.5,
        color="#616e82",
    )
    figure.suptitle(
        f"DeepSeek V4.1 · 16 B300 GPUs per variant · snapshot {now:%Y-%m-%d %H:%M} UTC\n"
    ...
    ax.axvspan(start / 1e6, maximum + 0.5, color="#f0f3f8", alpha=0.7, zorder=0)
    ax.text(
        start / 1e6 + 1.1,
        0.94,
        "Full-weight phase\nstarts at 50.12M",
    ...
    ax.set_xlim(0, maximum + 0.5)
    ax.set_ylabel("Training cross-entropy")
    ax.set_xlabel("Total supervised tokens (millions)")
    ax.set_title(
        "Training history · optimizer and backbone trainability change at 50.12M tokens",
        loc="left",
        fontsize=13,
        pad=12,
    )
    ...
    ax.text(
        0.01,
        -0.2,
        "3M-token weighted trailing means, smoothed separately within each phase. Adapter-only training beyond the selected 50M checkpoints is omitted.",
    ...
    plt.close(history)
    text = (
        f"The training losses are practically tied through the matched {stop / 1e6:.2f}M-token point.\n\n"
    ...
    (output / "README.md").write_text(
        text.replace("matched10M", "matched 10M").replace("favoring2", "favoring 2")
    )
