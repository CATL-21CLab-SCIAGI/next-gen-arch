...
def ledger(path):
    ...
    for row in rows:
        if any(
            not math.isfinite(row[key]) for key in ("loss", "seconds", "gradient_norm_before_clip")
        ):
...
def statistics_for(rows):
    ...
    return {
        ...
        "total_update_seconds": sum(seconds),
        "mean_update_seconds": statistics.mean(seconds),
        "median_update_seconds": statistics.median(seconds),
        "p10_update_seconds": percentile(seconds, 0.1),
        "p90_update_seconds": percentile(seconds, 0.9),
        "supervised_targets_per_second": targets / sum(seconds),
        "padded_input_tokens_per_second": padded / sum(seconds),
        "peak_allocated_gib_including_qualification": max(
            row["max_memory_allocated_gib"] for row in rows
        ),
        "gradient_norm_range": [
            min(row["gradient_norm_before_clip"] for row in rows),
...
def compare(baseline, normal, *, first=21, last=50):
    ...
    if min(map(len, all_rows)) < last:
        raise ValueError(
            f"wait for the predefined step {last}; latest are {[len(rows) for rows in all_rows]}"
        )
    ...
    for _ in range(10000):
        ...
        resampled.append(
            sum(paired[0][i]["seconds"] for i in indices)
            / sum(paired[1][i]["seconds"] for i in indices)
        )
    for path, _rows, stats in zip((baseline, normal), paired, measured, strict=True):
        timed = {
            row["step"]: row["unix_time"] for row in events(path.parent / f"{path.name}-node0.log")
        }
    ...
    validations = [
        [json.loads(line) for line in (path / "validation.jsonl").read_text().splitlines()]
        for path in (baseline, normal)
    ]
    if validations[0][0] != validations[1][0]:
        raise ValueError("fresh initial held-out validation differs")
    return {
        "matched_steps": [first, last],
        "paired_update_count": last - first + 1,
        ...
        "paired_bootstrap_ratio_interval_95": [
            percentile(resampled, 0.025),
            percentile(resampled, 0.975),
        ],
        "bootstrap_scope": "variation among these paired steps; not independent repeated-run uncertainty",
        "initial_validation_exact_match": True,
        "initial_validation": validations[1][0],
        "latest_normal_validation": validations[1][-1],
        "latest_normal_update": all_rows[1][-1],
        "all_normal_updates_finite": True,
        "normal_ledger_steps": len(all_rows[1]),
        "qualification_and_initial_compilation_excluded": True,
        "parameter_matched": False,
        "core_precision_matched": False,
    }, paired
...
def main():
    ...
    (args.output / "THROUGHPUT_COMPARISON.json").write_text(
