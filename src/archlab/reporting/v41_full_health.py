...
def gpu_sample(host, key):
    ...
    for line in result.stdout.splitlines():
        ...
        gpus.append({'index':int(index),'memory_used_gib':float(used)/1024,
                     'memory_total_gib':float(total)/1024,'utilization_percent':float(utilization),
...
def sample(root, key, version):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        nodes = dict(zip(hosts,pool.map(lambda h:gpu_sample(h,key),hosts)))
    ...
    for variant, info in comparison['variants'].items():
        ...
        metrics = [json.loads(line) for line in path.read_text().splitlines()] if path.exists() else []
        failures = sorted(p.name for p in output.glob('*failure.json'))
        checkpoint_paths = sorted(output.glob('checkpoints/*/COMPLETE.json'))
        snapshot['variants'][variant] = {'nodes':[nodes[h] for h in info['nodes']],
            'updates':len(metrics),'latest':metrics[-1] if metrics else None,'failures':failures,
    ...
    for variant, info in snapshot['variants'].items():
        ...
        summary[variant] = {'updates':info['updates'],'loss':None if info['latest'] is None else info['latest']['loss'],
            'max_used_gib':max((g['memory_used_gib'] for g in gpus),default=None),
            'mean_gpu_utilization':sum(g['utilization_percent'] for g in gpus)/len(gpus) if gpus else None,
            'failures':len(info['failures']),'complete_checkpoints':len(info['complete_checkpoints'])}
