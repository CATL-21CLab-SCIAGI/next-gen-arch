...
Examples:
    ...
    torchrun --nproc_per_node=8 -m archlab.speedrun.base_eval --hf-path openai-community/gpt2

    # Evaluate a nanochat model (e.g. d24) using 8 GPUs
    torchrun --nproc_per_node=8 -m archlab.speedrun.base_eval --model-tag d24 --device-batch-size=16

    # Quick/approximate evaluation using a single GPU
    python -m archlab.speedrun.base_eval --model-tag d24 --device-batch-size=16 --max-per-task=100 --split-tokens=524288
"""
...
from archlab.speedrun.checkpoint import load_model
from archlab.speedrun.core_eval import evaluate_task
from archlab.speedrun.dataloader import tokenizing_distributed_data_loader_bos_bestfit
from archlab.speedrun.engine import Engine
from archlab.speedrun.loss_eval import evaluate_bpb
from archlab.speedrun.runtime import (
...
from archlab.speedrun.tokenizer import HuggingFaceTokenizer, get_token_bytes
...
def main():
    ...
    # --- Log to report ---
    from archlab.speedrun.report import get_report
