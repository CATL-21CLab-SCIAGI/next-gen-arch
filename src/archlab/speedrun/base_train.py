...
python -m archlab.speedrun.base_train

or distributed as:

torchrun --nproc_per_node=8 -m archlab.speedrun.base_train

If you are only on CPU/Macbook, you'll want to train a much much smaller LLM. Example:
python -m archlab.speedrun.base_train --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
"""
...
from archlab.speedrun.precision import (
...
if args.precision_recipe != "bf16" and args.arch_family != "fog":
    parser.error(
        "--precision-recipe is only supported with --arch-family=fog. Use legacy --fp8 for archlab.architectures."
    )
...
if args.fp8:
    if device_type != "cuda":
        ...
        from archlab.speedrun.fp8 import Float8LinearConfig, convert_to_float8_training
