...
from archlab.architectures.base import GPT, EngramMemory, GPTConfig, Linear, has_ve, norm
from archlab.architectures.frontier import (
    FrontierPoolGPT,
    MotifMHCConnection,
    QwenGatedDeltaAttention,
)
from archlab.architectures.sota import XIELU, SotaAttention
...
class QATLinear(Linear):
    ...
    def __init__(self, *args, group_size: int = 128, **kwargs):
        ...
        if torchao.__version__ != "0.15.0":
            raise RuntimeError(f"8da4w QAT requires torchao==0.15.0, found {torchao.__version__}")
        ...
        self.weight_fake_quantizer = IntxFakeQuantizer(
            IntxFakeQuantizeConfig(torch.int4, group_size=self.group_size, is_symmetric=True)
        )
...
class ComboSearchGPT(GPT):
        ...
        for name, child in list(module.named_children()):
            ...
            if isinstance(child, Linear):
                if min(child.in_features, child.out_features) < self.config.qat_min_size:
                    continue
                converted = QATLinear.from_linear(child, group_size=self.config.qat_group_size)
    ...
    def get_architecture_state(self) -> dict:
        ...
        return {
            ...
            "qat_group_size": self.config.qat_group_size
            if self.config.qat_recipe != "none"
            else None,
            "qat_start_step": self.config.qat_start_step
            if self.config.qat_recipe != "none"
            else None,
            ...
            "qat_backend": "torchao-0.15.0-fake-quant"
            if self.config.qat_recipe != "none"
            else None,
        }
    ...
    def __post_init__(self) -> None:
        ...
        if "engram" in components:
            if "motif_mhc_anneal" in components:
                raise ValueError(
                    "Engram and mHC stream replication are not a supported controlled pair"
                )
            if not self.engram_layers or any(
                not 0 <= layer < self.n_layer for layer in self.engram_layers
            ):
...
class ParetoComboGPT(FrontierPoolGPT):
    ...
    def init_weights(self):
        ...
        for layer_idx in range(self.config.n_layer):
            self.resid_lambdas.data[layer_idx] = 1.15 - 0.10 * layer_idx / max(
                self.config.n_layer - 1, 1
            )
            self.x0_lambdas.data[layer_idx] = 0.20 - 0.15 * layer_idx / max(
                self.config.n_layer - 1, 1
            )
    ...
    def _trunk(self, idx, kv_cache=None):
        ...
        x = norm(self.transformer.wte(idx).to(self.config.runtime.compute_dtype))
        if kv_cache is None:
            if time <= 1:
            ...
            raise NotImplementedError(
                "Pareto-combination KV-cache inference is outside this campaign"
            )
    ...
    def get_architecture_state(self) -> dict:
        return {
            ...
            "exclusive_attention_layers": [
                i + 1 for i, b in enumerate(self.transformer.h) if not b.is_gdn
            ]
            if "exclusive_attention" in self.config.components
            else [],
            "xielu_layers": self.config.n_layer if "xielu" in self.config.components else 0,
            "mhc_streams": self.config.mhc_num_streams
            if "motif_mhc_anneal" in self.config.components
            else 0,
