...
def validate_length_penalty(value):
    ...
    if not (
        0 <= value["minimum_pass_rate"] <= 1
        and 0 < value["reference_quantile"] <= 1
        and 0 <= value["maximum_deduction"] < 1
        and value["tolerated_excess"] >= 0
        and value["saturation_excess"] > value["tolerated_excess"]
        and value["exponent"] >= 1
    ):
...
def successful_length_rewards(rewards, lengths, config):
    ...
    if (
        rewards.ndim != 2
        or lengths.shape != rewards.shape
        or lengths.device != rewards.device
        or not bool(((rewards == 0) | (rewards == 1)).all())
        or not bool(torch.isfinite(lengths).all())
    ...
    if config["enabled"]:
        for index in range(rewards.shape[0]):
            ...
            if (
                int(success.sum()) < 2
                or float(success.float().mean()) <= config["minimum_pass_rate"]
            ):
                continue
            eligible += 1
            reference = torch.quantile(
                lengths[index, success].float(), config["reference_quantile"]
            )
            excess = lengths[index].float() / reference - 1
            ramp = (
                (excess - config["tolerated_excess"])
                / (config["saturation_excess"] - config["tolerated_excess"])
            ).clamp(0, 1)
