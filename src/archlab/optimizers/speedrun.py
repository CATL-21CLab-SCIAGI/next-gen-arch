...
from archlab.speedrun.runtime import COMPUTE_DTYPE, get_dist_info, print0
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    - Large params: reduce_scatter gradients so each rank gets 1/N of the grad, update
      ...
      exp_avg_sq) is sharded - each rank only stores state for its slice. The leading
      dimension is zero-padded when it is not divisible by world_size.
...
def setup_model_optimizer(
    ...
    *,
    ...
    from archlab.architectures.combinations import ParetoComboGPT
    from archlab.architectures.dsa import DeepSeekDSA
    from archlab.architectures.fog import FOG
    from archlab.architectures.frontier import FrontierPoolGPT
    from archlab.architectures.kimi import KimiAttnRes, KimiKDA
    from archlab.architectures.sota import SotaPoolGPT
