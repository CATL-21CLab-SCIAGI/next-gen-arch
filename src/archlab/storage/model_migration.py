...
def prepare(source, destination, journal):
    ...
    state = {'source': str(source), 'destination': str(destination), 'phase': 'copying',
             'started_at_utc': datetime.now(timezone.utc).isoformat(),
             'destination_children_before': before, 'files': dict(files),
...
def commit(journal):
    ...
    expected = {key: {name: value[name] for name in ('bytes', 'mtime_ns', 'inode')}
    ...
    state.update(phase='complete', completed_at_utc=datetime.now(timezone.utc).isoformat(),
