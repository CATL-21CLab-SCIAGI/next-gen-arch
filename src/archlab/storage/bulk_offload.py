...
def assert_not_in_use(source):
    ...
    for parent in Path(source).absolute().parents:
        if (parent / ".archlab-storage-in-use.json").exists() or any(
            (parent / ".checkpoint-readers").glob("*.json")
        ):
...
def main():
    ...
    with args.database.with_suffix(".lock").open("w") as lock:
        ...
        db.execute(
            "CREATE TABLE IF NOT EXISTS transfers (source TEXT PRIMARY KEY, destination TEXT NOT NULL, link INTEGER NOT NULL, bytes INTEGER, sha256 TEXT, state TEXT NOT NULL, error TEXT)"
        )
        plan = [json.loads(line) for line in args.plan.read_text().split("\n") if line.strip()]
        if len({item["source"] for item in plan}) != len(plan) or len(
            {item["destination"] for item in plan}
        ) != len(plan):
            raise ValueError("Plan contains duplicate sources or destinations")
        for item in plan:
            old = db.execute(
                "SELECT destination,link FROM transfers WHERE source=?", (item["source"],)
            ).fetchone()
        ...
        pending = [
            ...
            for item in plan
            if db.execute(
                "SELECT state FROM transfers WHERE source=?", (item["source"],)
            ).fetchone()[0]
            != "verified"
        ]
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            ...
            for completed, task in enumerate(as_completed(tasks), 1):
                ...
                try:
                    result = task.result()
                    db.execute(
                        "UPDATE transfers SET bytes=?,sha256=?,state=?,error=NULL WHERE source=?",
                        (result["bytes"], result["sha256"], "verified", item["source"]),
                    )
                except Exception as error:
                    errors += 1
                    db.execute(
                        "UPDATE transfers SET state=?,error=? WHERE source=?",
                        ("error", repr(error), item["source"]),
                    )
                ...
                if time.monotonic() - last_report > 20:
                    count, size = db.execute(
                        "SELECT COUNT(*),COALESCE(SUM(bytes),0) FROM transfers WHERE state='verified'"
                    ).fetchone()
                    print(
                        json.dumps(
                            {
