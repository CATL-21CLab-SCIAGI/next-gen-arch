...
def main():
    ...
    if args.wait_for_pid:
        ...
        while process.exists():
            ...
            save(status, {'state': 'waiting_for_initial_transfer', 'initial_pid': args.wait_for_pid,
    ...
    while True:
        existing = {row['source']: row for row in
        ...
        if added:
            ...
            save(status, {'state': 'transferring_new_payloads', 'new_files': added,
                          'utc': datetime.now(timezone.utc).isoformat()})
            subprocess.run([sys.executable, '-u', '-m', 'archlab.storage.bulk_offload',
                            '--plan', str(args.plan), '--database', str(args.database),
                            '--workers', str(args.workers)], check=True)
        save(status, {'state': 'caught_up', 'new_files': added, 'scan': summary,
                      'utc': datetime.now(timezone.utc).isoformat()})
        time.sleep(120)
