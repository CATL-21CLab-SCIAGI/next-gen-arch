...
def prepare(*, capability, piqa, train, validation, checkpoint, output, seed=20260914):
    ...
    rows = [
        json.loads(x) for x in (capability / "cases.jsonl").read_text().split("\n") if x.strip()
    ]
    source_manifest = json.loads((capability / "manifest.json").read_text())
    if digest(capability / "cases.jsonl") != source_manifest["cases_sha256"]:
        raise ValueError("capability cases changed")
    selected = []
    for task, count in (("mmlu", 57), ("arc_challenge", 32)):
        ...
        if task == "mmlu":
            ...
            if len(subjects) != 57:
                raise ValueError("MMLU subject count changed")
            chosen = [
                rng.choice([r for r in pool if r["subject"] == subject]) for subject in subjects
            ]
        else:
    ...
    if len(ds) != 1838:
        raise ValueError("use the public labeled PIQA validation split")
    ...
    training = [
        json.loads(x) for x in (train / "windows.jsonl").read_text().split("\n") if x.strip()
    ]
    ...
    windows = [
        json.loads(x) for x in (validation / "windows.jsonl").read_text().split("\n") if x.strip()
    ]
    ...
    manifest = {
        ...
        "counts": {
            task: sum(r["task"] == task for r in selected)
            for task in ("mmlu", "arc_challenge", "piqa")
        },
        "math_windows": len(chosen_math),
        "math_supervised_targets": sum(r["targets"] for r in chosen_math),
        "math_lengths": [r["length"] for r in chosen_math],
        "cases_sha256": digest(output / "cases.jsonl"),
        "math_windows_sha256": digest(output / "math_windows.json"),
        "sources": source_manifest["sources"],
        "piqa": {"path": str(piqa), "sha256": digest(piqa), "split": "validation", "rows": len(ds)},
        "train_manifest_sha256": digest(train / "PILOT_READY.json"),
        "validation_manifest_sha256": digest(validation / "PILOT_READY.json"),
        "exact_consumed_training_overlaps": [
            r["id"] for r in selected if r["exact_overlap_consumed_training"]
        ],
        "exact_planned_training_overlaps": [
            r["id"] for r in selected if r["exact_overlap_planned_training"]
        ],
...
def main():
    ...
    for name in ("capability", "piqa", "train", "validation", "checkpoint", "output"):
