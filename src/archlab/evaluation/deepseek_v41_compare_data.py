...
def build_jobs(tokenizer,cases,templates):
    ...
    for case in cases:
        ...
        options=[chr(65+i) for i in range(len(case['choices']))] if case['task']=='mmlu' else case['choices']
        pairs=[]
        for option in options:
            prefix_text=context.rstrip();continuation=context[len(prefix_text):]+' '+option
        ...
        if all(len(t)==1 and p==pairs[0][0] for p,t in pairs):
            jobs.append({'id':case['id'],'input_ids':pairs[0][0],'prefix_length':len(pairs[0][0]),
                         'fast_targets':[t[0] for _,t in pairs]})
        else:
            for choice,(prefix,target) in enumerate(pairs):
                jobs.append({'id':case['id'],'choice':choice,'input_ids':prefix+target[:-1],
                             'prefix_length':len(prefix),'targets':target})
...
def prepare(args):
    ...
    if sha(args.capability/'cases.jsonl')!=source['cases_sha256']:raise ValueError('capability source changed')
    cases=[]
    subjects=sorted({r['subject'] for r in pool if r['task']=='mmlu'})
    if len(subjects)!=57:raise ValueError('expected all 57 MMLU subjects')
    for subject in subjects:
        candidates=[r for r in pool if r['task']=='mmlu' and r['subject']==subject]
        cases+=random.Random(f'{args.seed}:mmlu:{subject}').sample(candidates,args.mmlu_per_subject)
    ...
    if len(piqa)!=1838:raise ValueError('use the labeled PIQA validation split')
    for i in sorted(random.Random(f'{args.seed}:piqa').sample(range(len(piqa)),args.piqa_count)):
        r=piqa[i];cases.append({'id':f'piqa:{i}','task':'piqa','index':i,'subject':None,
                              'question':r['goal'],'choices':[r['sol1'],r['sol2']],'answer':r['label']})
    cases=sorted(cases,key=lambda r:(r['task'],r['index']))
    checkpoints={v:json.loads((getattr(args,v)/'COMPLETE.json').read_text()) for v in ('simplicial','normal')}
    if checkpoints['simplicial']['cursor']!=checkpoints['normal']['cursor']:raise ValueError('checkpoint cursors differ')
    cursor=checkpoints['simplicial']['cursor']
    if cursor['step']!=552 or cursor['supervised_tokens']!=91035439:raise ValueError('use the matched 91.04M checkpoints')
    ...
    for path,manifest in ((args.train,train_manifest),(args.validation,val_manifest)):
        if sha(path/'windows.jsonl')!=manifest['windows_sha256']:raise ValueError('pilot windows changed')
    train=read_jsonl(args.train/'windows.jsonl');random.Random(2234).shuffle(train)
    planned={r['problem_sha256'] for r in train};consumed={r['problem_sha256'] for r in train[:cursor['step']*32]}
    validation=read_jsonl(args.validation/'windows.jsonl')
    if val_manifest['supervised_tokens']!=1000000 or sum(r['targets'] for r in validation)!=1000000:
    ...
    if overlap:raise ValueError('held-out math overlaps planned training problems')
    for r in cases:
        forms=[r['question'],r['question']+'\n'+'\n'.join(f'{chr(65+i)}. {c}' for i,c in enumerate(r['choices']))]
    ...
    templates=yaml.safe_load(args.prompts.read_text());jobs=build_jobs(tokenizer,cases,templates)
    if max(len(r['input_ids']) for r in jobs)>16384:raise ValueError('benchmark prompt exceeds model context')
    args.output.mkdir(parents=True,exist_ok=False)
    (args.output/'cases.jsonl').write_text(''.join(json.dumps(r,ensure_ascii=False)+'\n' for r in cases))
    (args.output/'jobs.json').write_text(json.dumps(jobs,ensure_ascii=False)+'\n')
    manifest={'format':'archlab-v41-paired-full-eval-data-v1','seed':args.seed,'checkpoint_cursor':cursor,
              'counts':dict(Counter(r['task'] for r in cases)),'mmlu_per_subject':args.mmlu_per_subject,
              'jobs':len(jobs),'job_max_context':max(len(j['input_ids']) for j in jobs),
              'cases_sha256':sha(args.output/'cases.jsonl'),'jobs_digest':jobs_digest(jobs),'prompts_sha256':sha(args.prompts),
              'capability_sources':source['sources'],'piqa_source':{'path':str(args.piqa),'sha256':sha(args.piqa),'split':'validation'},
              'validation_manifest_sha256':sha(args.validation/'PILOT_READY.json'),
              'train_manifest_sha256':sha(args.train/'PILOT_READY.json'),
              'heldout_math_windows':len(validation),'heldout_math_targets':1000000,
              'heldout_math_unique_problems':len({r['problem_sha256'] for r in validation}),
              'exact_math_planned_training_overlaps':0,
              'exact_consumed_mc_overlaps':[r['id'] for r in cases if r['exact_overlap_consumed_training']],
              'exact_planned_mc_overlaps':[r['id'] for r in cases if r['exact_overlap_planned_training']],
...
def main():
    ...
    for name in ('capability','piqa','train','validation','assets','prompts','simplicial','normal','output'):
