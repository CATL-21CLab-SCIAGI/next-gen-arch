...
def mc_summary(records):
    ...
    for task in sorted({r['task'] for r in records}):
        ...
        result[task]={metric:paired_binary([r['result']['simplicial'][metric] for r in selected],
                                          [r['result']['normal'][metric] for r in selected])
                      for metric in ('accuracy','accuracy_norm')}
        result[task]['samples']=len(selected)
        result[task]['prediction_agreement']=sum(r['result']['simplicial']['prediction']==r['result']['normal']['prediction'] for r in selected)/len(selected)
        result[task]['exact_consumed_training_overlaps']=[r['id'] for r in selected if r.get('exact_overlap_consumed_training')]
    return result


def aggregate_math(records):
    total=sum(r['targets'] for r in records)
    if not total:raise ValueError('empty math evaluation')
    result={'windows':len(records),'supervised_targets':total,'variants':{}}
    for v in VARIANTS:
        nll=sum(r[v]['nll'] for r in records)/total
        result['variants'][v]={'cross_entropy':nll,'perplexity':math.exp(nll),
    ...
    result['normal_minus_simplicial_ce']=result['variants']['normal']['cross_entropy']-result['variants']['simplicial']['cross_entropy']
    return result
...
def math_summary(records,*,seed=20260915,replicates=10000):
    ...
    for tools in (False,True):
        ...
        if subset:result['by_tools']['tools' if tools else 'no_tools']=aggregate_math(subset)
    groups={}
    for r in records:
        totals=groups.setdefault(r['problem_sha256'],np.zeros(4,dtype=np.float64))
        totals+=np.array([r['targets'],r['normal']['nll']-r['simplicial']['nll'],
                          r['normal']['top1']-r['simplicial']['top1'],r['normal']['top5']-r['simplicial']['top5']])
    ...
    bounds=np.quantile(np.array(samples),[.025,.975],axis=0)
    result['paired_uncertainty']={'method':'paired percentile bootstrap clustered by exact problem hash',
        'problem_clusters':len(groups),'replicates':replicates,'seed':seed,'delta_definition':'normal minus simplicial',
