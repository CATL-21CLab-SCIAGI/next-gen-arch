...
def make_twins(rng, *, earlier_count, recent_count=2, modulus=97):
    ...
    for _ in range(1000):
        ...
        recent = list(zip(rng.sample(range(modulus), recent_count), rng.sample(range(modulus), recent_count)))
        ...
        pool = sorted({(x, v) for x, _ in complements for v in range(modulus)}
        ...
        for value in pool:
            ...
            add(value)
        rng.shuffle(earlier)
        swaps = [(i, j) for i in range(earlier_count) for j in range(i + 1, earlier_count)
        ...
        for i, j in swaps:
            ...
            if len(set(positive)) == earlier_count and len(witnesses(recent, positive, target, modulus)) == 1:
...
def generate_suite(*, seed=20260920, sizes=(8, 16, 32, 64), pairs_per_size=1,
                   recent_count=2, modulus=97):
        ...
        for number in range(pairs_per_size):
            ...
            for index, earlier in enumerate(twins):
                ...
                rows.append({
                    ...
                    "text": template.format(modulus=modulus, long_entries=fmt(earlier),
                                            recent_entries=fmt(recent), target=fmt([target])),
                })
    return {
        "schema_version": 1,
...
def main():
    ...
    suite = generate_suite(seed=args.seed, sizes=tuple(args.sizes), pairs_per_size=args.pairs_per_size,
                           recent_count=args.recent_count, modulus=args.modulus)

    class PromptDumper(yaml.SafeDumper):
        pass

    PromptDumper.add_representer(str, lambda dumper, value: dumper.represent_scalar(
