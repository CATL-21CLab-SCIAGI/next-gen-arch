...
The experiment identity is `deepseek-v41-normal-adam-bf16-fp8-local-disk-v1`:
...
This is a maintained entry page, not a live monitor. Evidence below was recorded
at **2026-09-25 02:26 UTC**; the overnight process was left running. Restructuring
changes the launcher and documentation, not that process or its runtime hooks.

| Claim | Evidence / limitation |
| --- | --- |
| Canonical launch contract | CPU rendering and equivalence tests compare every effective argument against attempt 6. The new launcher has not itself been launched on 32 GPUs. |
...
4. Choose a **new** `RUN_ROOT`. Supply the prepared `model/` metadata/tokenizer
   ...
   preparation. The model config must reference the finetuned normal parent via
...
6. Reserve durable storage for the parent, results, and every simultaneous full
   ...
   saves every 20 rollouts over 64 rollouts; budget all retained checkpoints.
...
`results/deepseek-v41-stock-fp8-resident-policy-20260924/runtime-NODE.json`.
...
PYTHONPATH=src python -m archlab.megatron.miles_v41_stock_launch render \
  --config recipes/experiments/deepseek_v41_stock_fp8.yaml \
  --run-root "$RUN_ROOT" --miles "$MILES_ROOT" --address "$RAY_ADDRESS"
```
