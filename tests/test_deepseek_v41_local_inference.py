...
from archlab.automodel.deepseek_v41_local_inference import (
    DecodedExpertCache,
    _adapter_block,
    _offloaded_expert,
    cpu_engram_lookup,
...
def test_cpu_engram_rows_match_gpu_lookup_byte_exact():
    ...
    scales = torch.tensor([[0.5, 2.0]] * 9).to(torch.float8_e8m0fnu)
    ...
    expected = (
        (
            F.embedding(ids, values.cuda()).float().unflatten(-1, (-1, 32))
            * F.embedding(ids, scales.cuda()).float().unsqueeze(-1)
        )
        .flatten(-2)
        .bfloat16()
    )
    ...
    with unittest.TestCase().assertRaises(ValueError):
        cpu_engram_lookup(table, torch.tensor([9], device="cuda"))
    ...
    def __init__(self):
        ...
        for name in ("w1", "w2", "w3"):
            ...
            module.weight = nn.Parameter(
                torch.randint(-128, 128, (32, 16), dtype=torch.int8), requires_grad=False
            )
            module.scale = nn.Parameter(
                torch.full((32, 1), 0.03125).to(torch.float8_e8m0fnu), requires_grad=False
            )
            setattr(self, name, module)
        self._cpu_weights = {
            n: (getattr(self, n).weight, getattr(self, n).scale) for n in ("w1", "w2", "w3")
        }
...
def test_expert_offload_matches_decoding_and_restores_cpu_storage():
    ...
    expert = _Expert()
    expert._weight_cache = DecodedExpertCache(6144)
    expert._cache_key = "first"
    original = {n: getattr(expert, n).weight for n in ("w1", "w2", "w3")}
    x = torch.randn(7, 32, device="cuda", dtype=torch.bfloat16)
    weights = torch.rand(7, 1, device="cuda")
    decoded = {
        n: dequantize_frozen_weight(w.cuda(), s.cuda()) for n, (w, s) in expert._cpu_weights.items()
    }
    h = F.silu(F.linear(x, decoded["w1"]).float()) * F.linear(x, decoded["w3"]).float() * weights
    expected = F.linear(h.bfloat16(), decoded["w2"])
    for _ in range(2):
        torch.testing.assert_close(_offloaded_expert(expert, x, weights), expected, rtol=0, atol=0)
    assert expert._weight_cache.misses == 1 and expert._weight_cache.hits == 1
    assert all(
        getattr(expert, n).weight is p and p.device.type == "cpu" for n, p in original.items()
    )
    expert._cache_key = "second"
    _offloaded_expert(expert, x, weights)
...
class _Block:
    def __init__(self, adapter):
        ...
        self._adapter_mask = torch.tensor([False, True], device="cuda")
        self._adapter_history = None
        ...
        self.attn = lambda x, *args: x * 0.2
        self.ffn = lambda x, *args: x * 0.3

    def hc_mixes(self, *args):
        return None, None, None

    def hc_pre(self, x, mix):
        return x.mean(-2)

    def hc_post(self, x, residual, *args):
...
def test_cached_adapter_decode_matches_full_prefill_and_preserves_base_rows():
    ...
    a.output.weight.normal_(std=0.08)
    ...
    for index in range(6, 13):
        chunks.append(_adapter_block(decoded, x[:, index : index + 1], index, None, None)[0])
    torch.testing.assert_close(torch.cat(chunks, 1), expected, rtol=2e-6, atol=2e-6)
    base = _Block(a)
    base._adapter_mask.zero_()
