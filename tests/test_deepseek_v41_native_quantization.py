...
from archlab.automodel.deepseek_v41_native_quantization import (
    _fp8_quantize,
    install_native_row_padding,
)
...
def test_native_format_matches_rounded_activation_and_repeats(shape, block_size):
    ...
    x = (torch.randn(shape) * 0.02).bfloat16()
...
def test_unrounded_scales_and_inplace_alias_keep_native_equations():
    x = torch.linspace(-0.37, 0.42, 17 * 128).reshape(17, 128).bfloat16()
    grouped = x.float().unflatten(-1, (4, 32))
    expected_scales = grouped.abs().amax(-1).clamp_min(1e-4) * (1.0 / 448)
    expected_values = (
        (grouped / expected_scales.unsqueeze(-1)).clamp(-448, 448).to(torch.float8_e4m3fn)
    )
    ...
    torch.testing.assert_close(
        mutable,
        (expected_values.float() * scales.unsqueeze(-1)).flatten(-2).bfloat16(),
        rtol=0,
        atol=0,
    )
...
def test_installer_uses_stable_fp8_keeps_gemm_padding_and_records_evidence():
    ...
    reference = SimpleNamespace(
        linear=old_linear, act_quant=forbidden_fp8, fp4_act_quant=lambda *args: None
    )
    ...
    assert evidence["native_gemms_retained"] and evidence["fp8_activation_backend"].startswith(
        "pytorch"
    )
