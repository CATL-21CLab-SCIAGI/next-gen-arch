...
class RetentionTest(unittest.TestCase):
    ...
    def checkpoint(self, step, complete=True):
        ...
        marker = {'format': 'archlab-v41-full-sharded-v1', 'world_size': 1,
                  'contract': {'variant': 'test'}, 'cursor': {'step': step, 'supervised_tokens': step * 100},
                  'manifests': ['rank-00/MANIFEST.json']}
        manifest = {**marker, 'rank': 0, 'optimizer_states': ['optimizer.pt'],
                    'tensors': [{'name': 'weight', 'shape': [2], 'dtype': 'torch.bfloat16',
    ...
    def test_step_pin_survives_newer_checkpoints(self):
        pinned = self.checkpoint(5000)
        self.checkpoint(5500)
        self.config['protected_steps'] = [5000]
        self.assertFalse(sweep(self.config, self.audit, True)['errors'])
        self.assertTrue(pinned.exists())

    def test_checkpoint_pin_marker_survives_policy_refresh(self):
        pinned = self.checkpoint(5000)
        (pinned / 'PINNED.json').write_text('{"reason":"matched comparison"}')
        self.checkpoint(5500)
        self.assertFalse(sweep(self.config, self.audit, True)['errors'])
        self.assertTrue(pinned.exists())

    def test_pending_evaluation_and_reader_lease_preserve_weights(self):
        pending=self.checkpoint(4000);reading=self.checkpoint(4500);self.checkpoint(5000)
        (pending/'EVAL_PENDING.json').write_text('{}')
        (reading/'.checkpoint-readers').mkdir();(reading/'.checkpoint-readers/reader.json').write_text('{}')
        self.assertFalse(sweep(self.config,self.audit,True)['errors'])
        self.assertTrue(pending.exists());self.assertTrue(reading.exists())
    ...
    def test_resume_interrupted_removal(self):
        ...
        receipt.write_text(json.dumps({'original': str(old), 'staged': str(staged), 'complete': False}))
        self.assertFalse(sweep(self.config, self.audit, True)['errors'])
