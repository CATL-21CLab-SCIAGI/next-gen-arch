...
from archlab.storage.checkpoint_retention import sweep, validate
...
class RetentionTest(unittest.TestCase):
    ...
    def test_pending_evaluation_and_reader_lease_preserve_weights(self):
        pending = self.checkpoint(4000)
        reading = self.checkpoint(4500)
        self.checkpoint(5000)
        (pending / "EVAL_PENDING.json").write_text("{}")
        (reading / ".checkpoint-readers").mkdir()
        (reading / ".checkpoint-readers/reader.json").write_text("{}")
        self.assertFalse(sweep(self.config, self.audit, True)["errors"])
