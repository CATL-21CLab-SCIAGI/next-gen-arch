...
class Client:
    def __init__(self):self.metrics=[];self.created=0;self.fail_once=False;self.status=None;self.tags={}
    ...
    def update_run(self,*args,**kwargs):self.status=kwargs['status']
    def set_terminated(self,*args,**kwargs):self.status=kwargs['status']
...
class SyncTests(unittest.TestCase):
    ...
    def test_explicit_native_failure_terminates_mlflow_run(self):
        ...
        client=Client();sync=Sync(client,self.root/'state.json')
    ...
    def test_serving_failure_overrides_fresh_chat_heartbeat(self):
        self.spec['state']='running'
        service=self.root/'service';service.mkdir()
        self.spec['serving_path']=str(service)
        (service/'rank-00-failure.json').write_text('{"reason":"serving failure"}')
        (self.root/'ACTIVITY.json').write_text(json.dumps({'phase':'chat','heartbeat_unix':time.time()}))
        client=Client();sync=Sync(client,self.root/'state.json')
        self.assertEqual(sync.sync_run(self.spec)['state'],'failed')
        self.assertEqual(client.status,'FAILED')
    ...
    def test_frozen_training_stays_paused_while_chat_is_active(self):
        ...
        (self.root/'ACTIVITY.json').write_text(json.dumps({'phase':'chat','heartbeat_unix':time.time(),'stage':'playground'}))
        client=Client();sync=Sync(client,self.root/'state.json')
    ...
    def test_stale_stream_is_distinct_from_checkpointing(self):
        self.spec['state']='running';os.utime(self.history,(time.time()-3600,time.time()-3600))
        client=Client();sync=Sync(client,self.root/'state.json')
        self.assertEqual(sync.sync_run(self.spec)['state'],'stalled')
        checkpoint=self.root/'checkpoints/step-000002';checkpoint.mkdir(parents=True)
        self.assertEqual(sync.sync_run(self.spec)['state'],'checkpointing')
        (checkpoint/'COMPLETE.json').write_text(json.dumps({'cursor':{'step':2,'supervised_tokens':200}}))
    ...
    def test_qualified_source_transition_preserves_run_identity(self):
        client=Client();sync=Sync(client,self.root/'state.json');sync.sync_run(self.spec)
        contract=json.loads((self.root/'RUN_CONTRACT.json').read_text());contract['project_commit']='def'
        (self.root/'RUN_CONTRACT.json').write_text(json.dumps(contract))
        with self.assertRaises(FileNotFoundError):sync.sync_run(self.spec)
        (self.root/'SOURCE_TRANSITION.json').write_text(json.dumps({'from_commit':'abc','to_commit':'def','training_math_unchanged':True,'cursor':{'step':2}}))
        sync.sync_run(self.spec);self.assertEqual(client.created,1);self.assertEqual(sync.state['runs']['test']['source_commit'],'def')
    ...
    def test_full_eval_resume_point_need_not_be_in_training_ledger(self):
        ...
        evaluation={'step':0,'consumed_supervised_tokens':0,'loss':1.,'cross_entropy':1.,
            'perplexity':2.718,'top1_token_accuracy':.5,'top5_token_accuracy':.9,
            'mean_predictive_entropy':1.,'targets':64000,'seconds':20.}
        ...
        self.assertEqual([(m.step,m.value) for m in points],[(0,1.)])
        first_time=points[0].timestamp
        evaluation.update(step=2,consumed_supervised_tokens=200)
        with (self.root/'validation.jsonl').open('a') as stream:stream.write(json.dumps(evaluation)+'\n')
        with patch('archlab.tracking.mlflow_sync.time.time',return_value=(first_time+60000)/1000):
