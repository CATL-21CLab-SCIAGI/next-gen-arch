...
from archlab.automodel.deepseek_v41_scratch_evaluate import (
    checkpoint_marker,
    choice_result,
    json_lines,
)
...
class ScratchEvalTests(unittest.TestCase):
    def test_checkpoint_requires_matched_variant_tokens_and_mesh(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            m = {
                ...
                "world_size": 8,
                "cursor": {"supervised_tokens": 240875236},
                "contract": {
            ...
            for v, t in [("simplicial", 240875236), ("normal", 240875237)]:
                with self.assertRaises(ValueError):
                    checkpoint_marker(p, v, t)
            m["world_size"] = 16
            (p / "COMPLETE.json").write_text(json.dumps(m))
            with self.assertRaises(ValueError):
                checkpoint_marker(p, "normal", 240875236)
    ...
    def test_choice_character_normalization(self):
        ...
        r = choice_result(c, [-2.0, -3.0])
        self.assertEqual(r["prediction"], 0)
        self.assertEqual(r["normalized_prediction"], 1)
        self.assertEqual(r["accuracy_norm"], 1)

    def test_mmlu_scores_answer_letters(self):
        c = {"task": "mmlu", "choices": ["aa", "aaaaaaaaaa"], "answer": 0}
        r = choice_result(c, [-2.0, -3.0])
        self.assertEqual(r["prediction"], r["normalized_prediction"])
        self.assertEqual(r["accuracy_norm"], 1)

    def test_incomplete_or_nonfinite_scores_rejected(self):
        c = {"task": "arc_challenge", "choices": ["a", "b"], "answer": 0}
        for scores in [[None, 0.0], [float("nan"), 0.0], [0.0]]:
            with self.assertRaises(ValueError):
                choice_result(c, scores)
