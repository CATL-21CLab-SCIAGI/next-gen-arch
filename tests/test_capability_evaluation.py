...
from archlab.automodel.qwen.evaluate import (
    added_modules,
    continuation_scores,
    encode_pair,
    math_completion,
)
from archlab.evaluation.capability import (
    EvaluationConfig,
    MathGrader,
    paired_statistics,
    select_cases,
...
def pinned_harness_available():
    try:
        return importlib.metadata.version("lm-eval") == "0.4.13"
...
class TinyCausalModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer(
            "table", torch.randn(128, 128, generator=torch.Generator().manual_seed(42))
        )
...
class CapabilityTests(unittest.TestCase):
    def test_paired_direction_and_uncertainty(self):
        ...
        self.assertEqual(
            (stats["gains"], stats["regressions"], stats["delta_percentage_points"]), (2, 1, 25)
        )
        ...
        self.assertAlmostEqual(
            paired_statistics([0] * 5, [1] * 5)["mcnemar_exact_two_sided_p"], 0.0625
        )
    ...
    def test_selection_and_budget_contract(self):
        rows = [
            {"id": str(i), "index": i, "subject": "ab"[i % 2], "task": "mmlu"} for i in range(20)
        ]
    ...
    def test_loglikelihood_matches_tokenwise_oracle(self):
        ...
        for choices in (["A", "B"], [" A", " BB"]):
            result = continuation_scores(
                model, tokenizer, "Q:", choices, max_context=20, device="cpu"
            )
            for index, choice in enumerate(choices):
                ids = tokenizer.encode("Q:" + choice)
                expected = sum(
                    model.table[ids[j - 1]].log_softmax(-1)[ids[j]].item()
                    for j in range(2, len(ids))
                )
                self.assertAlmostEqual(result["loglikelihoods"][index], expected, places=5)
                self.assertAlmostEqual(
                    result["character_normalized_loglikelihoods"][index],
                    expected / len(choice.removeprefix(" ")),
                    places=5,
                )
        self.assertEqual(
            encode_pair(tokenizer, "Q: ", "A"), (tokenizer.encode("Q:"), tokenizer.encode(" A"))
        )
        with self.assertRaises(ValueError):
    ...
    def test_generation_eos_and_cap(self):
        ...
        result = math_completion(
            model, tokenizer, "Q", config=config, eos_ids={ord("z"), ord("x")}, device="cpu"
        )
        self.assertEqual(
            (result["stop_reason"], result["new_tokens"], result["completion"]), ("eos", 1, "")
        )
        result = math_completion(
            model, tokenizer, "Q", config=config, eos_ids={ord("x")}, device="cpu"
        )
        self.assertEqual((result["stop_reason"], result["completion"]), ("token_cap", "zzzz"))

    @unittest.skipUnless(
        pinned_harness_available(), "requires installed lm-eval==0.4.13"
    )
    def test_multiple_choice_prompts_match_pinned_harness(self):
        ...
        prompts = yaml.safe_load(
            (project / "src/archlab/prompts/capability_regression.yaml").read_text()
        )
        ...
        for path in subjects:
            ...
            row = {
                "subject": task["dataset_name"],
                "question": " Example? ",
                "choices": ["a", "b", "c", "d"],
            }
            expected = task["description"] + template.from_string(default["doc_to_text"]).render(
                **row
            )
            self.assertEqual(
                template.from_string(prompts["multiple_choice"]["mmlu"]).render(**row), expected
            )
        arc = yaml.safe_load((root / "arc/arc_easy.yaml").read_text())
        self.assertEqual(
            template.from_string(prompts["multiple_choice"]["arc_challenge"]).render(
                question="Example?"
            ),
    ...
    def test_final_answer_grading(self):
        ...
        self.assertEqual(
            grader.score(gsm, "thought 99</think>The answer is $1,234.")["metrics"][
                "flexible_extract"
            ],
            1,
        )
        ...
        self.assertEqual(
            grader.score(gsm, "1234</think>The answer is 42.")["metrics"]["flexible_extract"], 0
        )
        self.assertEqual(
            grader.score({"task": "aime24", "answer": "42"}, "thinking</think>Thus \\boxed{42}.")[
                "metrics"
            ]["exact_match"],
            1,
        )
        self.assertEqual(
            grader.score({"task": "aime25", "answer": "42"}, "thinking \\boxed{42}")["metrics"][
                "exact_match"
            ],
            0,
        )
    ...
    def test_disabled_nonzero_additions_recover_original_exactly(self):
        ...
        from archlab.automodel.qwen.simplicial import AdditiveMoERead, install_simplicial_modules
        ...
        with torch.no_grad():
            for adapter in adapters.values():
                adapter.output.weight.normal_(std=0.02)
        ...
        self.assertTrue(
            all(m.adapter_enabled for m in model.modules() if isinstance(m, AdditiveMoERead))
        )
