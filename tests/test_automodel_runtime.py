...
from archlab.automodel.qwen.runtime import restrict_autotuner


def test_restrict_existing_configs_and_clear_unsafe_cache():
    configs = [
        SimpleNamespace(num_warps=w, num_stages=s, kwargs={"BV": 32})
        for w in (2, 4)
        for s in (2, 3, 4)
    ]
...
def test_checkpoint_cache_provenance_and_index_drift(tmp_path):
    from archlab.automodel.qwen.loading import audit_checkpoint_keys
    ...
    cache.write_text(
        json.dumps(
            {
                "source": "/example/read-only-original",
...
def test_fresh_optimizer_checkpoint_load_uses_upstream_materialization(tmp_path, monkeypatch):
    pytest.importorskip(
        "nemo_automodel", reason="requires upstream optimizer-state materialization"
    )
    import torch
