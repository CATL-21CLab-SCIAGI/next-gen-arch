...
from archlab.architectures.linsimp_attention import (
    linear_rf_attention, linsimp_attention, orthogonal_feature_bank, positive_features,
)
...
class LinSimpTests(unittest.TestCase):
    ...
    def test_gaussian_marginals_and_kernel(self):
        ...
        self.assertLess(abs(bank.square().mean().item()-1), .03)
        self.assertLess(bank.mean((0, 1)).abs().max().item(), .025)
        blocks = bank.reshape(4, -1, 16, 16)
        directions = torch.nn.functional.normalize(blocks, dim=-1)
        torch.testing.assert_close(directions @ directions.transpose(-1, -2),
                                   torch.eye(16).expand_as(blocks), atol=1e-6, rtol=1e-5)
        x = torch.zeros(4, 16); x[:, 0] = .5
        estimate = (positive_features(x, bank)**2).sum(-1).mean().item()
        self.assertLess(abs(estimate - math.exp(.25)), .025)
        self.assertTrue(torch.equal(bank, orthogonal_feature_bank(4, 16, 16384,
    ...
    def test_causality_and_large_temperature(self):
        ...
        for fn, args, kwargs in ((linsimp_attention, inputs, {'window': 32}),
                                  (linear_rf_attention, [inputs[0], inputs[1], inputs[3]], {})):
            output = fn(*args, bank, temperature=100., **kwargs)
            self.assertTrue(output.isfinite().all())
            first = inputs[3][:, 0] * inputs[4][:, 0] if fn is linsimp_attention else inputs[3][:, 0]
