...
class ToyActor(nn.Module):
    def __init__(self, *, include_branch=True):
        ...
        self.extra = nn.Parameter(torch.tensor([0.1, -0.2, 0.3]))
        self.selector = nn.Linear(3, 3, bias=False)
        self.selector._archlab_auxiliary_backward_scale = 0.1
        self.selector._archlab_auxiliary = None
        self.router = nn.Module()
        self.router.aux_loss_coeff = 0.2
        self.router.bias_update_factor = 0.01
    ...
    def forward(self, input_ids, attention_mask=None, return_hidden_states=True):
        self.forward_receipts.append(
            (
                self.training,
                self.selector._archlab_auxiliary_backward_scale,
                self.router.aux_loss_coeff,
                self.router.bias_update_factor,
            )
        )
        ...
        if self.training:
            hidden = (
                hidden
                + self.selector.weight.sum() * self.selector._archlab_auxiliary_backward_scale
            )
    ...
    def step(self):
        ...
        for group in self.param_groups:
            for parameter in group["params"]:
                ...
                updated += 1
        self.last_metrics = {
            "changed_local_elements": changed,
...
def make_rollout(model, *, rank=0, world=1, generated=None):
    ...
    receipt = {
        "rank": rank,
        "world_size": world,
        "policy_version": "fixture-policy-v0",
        "temperature": 1.0,
        "top_p": 1.0,
...
def dense_oracle(model, rollout, *, denominator=2):
    ...
    selected = (
        logits.log_softmax(-1).gather(-1, rollout.labels.clamp_min(0).unsqueeze(-1)).squeeze(-1)
    )
...
def test_prompt_token_mean_packed_and_all_prefixes_match_dense_gradient():
    for mode in ("packed", "sampled-prefix"):
        ...
        rollout.receipt.update(
            forward_count=2, forward_shapes=[[2, 4], [2, 4]], pad_token_id=0, eos_token_ids=[4]
        )
        ...
        metric = policy_gradient_step(
            ...
            replay_mode=mode,
            replay_prefixes=2,
            loss_normalization="prompt_token_mean",
        )
        assert abs(metric["policy_loss"] - expected_loss) < 1e-6
        assert metric["normalization"] == "global-prompt-mean-of-group-token-means"
        for observed, expected in zip(optimizer.gradients, expected_gradients, strict=False):
            if expected is None:
...
class BatchDependentActor(ToyActor):
    ...
    def forward(self, input_ids, attention_mask=None, return_hidden_states=True):
        ...
        return SimpleNamespace(
            hidden_states=result.hidden_states + attention_mask.sum() * self.extra
        )
...
def independent_prefix_oracle(model, rollout, *, times=None, denominator=2):
    ...
    for step in times:
        ...
        for row, advantage in enumerate((1.0, -1.0)):
    ...
    return float(loss.detach()), torch.autograd.grad(
...
def prefix_distributed_worker(rank, init_file, output_dir):
    ...
    try:
        for prefix_count in (3, 1):
            ...
            rollout = make_prefix_rollout(
                actor, rank=rank, world=2, generated=generated, global_steps=3
            )
            optimizer = RecordingSGD(actor.parameters())
            rewards = torch.tensor([[0.0, 0.0]]) if rank == 0 else torch.tensor([[1.0, 0.0]])
            ...
            _, expected = independent_prefix_oracle(
                oracle, oracle_rollout, times=times, denominator=4
            )
            for (name, parameter), gradient in zip(actor.named_parameters(), expected, strict=True):
                wanted = initial[name] if gradient is None else initial[name] - 0.01 * gradient
...
def distributed_worker(rank, init_file, output_dir):
    ...
    try:
        ...
        norm = torch.sqrt(
            sum(gradient.double().square().sum() for gradient in expected if gradient is not None)
        )
        for (name, parameter), gradient in zip(actor.named_parameters(), expected, strict=True):
            wanted = initial[name] if gradient is None else initial[name] - 0.01 * gradient
            torch.testing.assert_close(parameter, wanted, rtol=2e-6, atol=2e-7)
        ...
        skipped = policy_gradient_step(
            actor, optimizer, [actor.selector], flat, torch.ones(1, 2), lr=0.01, group_size=2
        )
        ...
        if rank == 0:
            invalid.behavior_log_probs[0, 1] += 0.001
        try:
            policy_gradient_step(
                actor,
                optimizer,
        ...
        (Path(output_dir) / f"rank-{rank}.json").write_text(
            json.dumps(
                {
                    "passed": True,
                    "norm": metric["gradient_norm_before_clip"],
                    "replay": metric["replay_max_abs_error"],
                }
            )
        )
    finally:
...
class RLUpdateTests(unittest.TestCase):
    ...
    def test_actual_signed_policy_gradient_matches_dense_and_masks(self):
        ...
        self.assertEqual(self.model.forward_receipts[-1], (False, 0.0, 0.0, 0.0))
        self.assertEqual(self.model.selector._archlab_auxiliary_backward_scale, 0.1)
        self.assertEqual(self.model.router.aux_loss_coeff, 0.2)
    ...
    def test_global_norm_clipping_and_no_change_are_reported(self):
        metric = self.update(max_grad_norm=0.01)
        self.assertLess(metric["gradient_clip_scale"], 1)
        norm = torch.sqrt(
            sum(g.double().square().sum() for g in self.optimizer.gradients if g is not None)
        )
        self.assertAlmostEqual(float(norm), 0.01, places=7)
        self.rollout = make_rollout(self.model)
    ...
    def test_two_rank_sum_matches_global_oracle_with_one_flat_rank(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            processes = [
                context.Process(target=distributed_worker, args=(rank, init_file, temporary))
                for rank in range(2)
            ]
            try:
                ...
                self.assertTrue(
                    ...
                    "distributed collective hung",
                )
                self.assertEqual([process.exitcode for process in processes], [0, 0])
                self.assertTrue(
                    all(
                        json.loads((Path(temporary) / f"rank-{rank}.json").read_text())["passed"]
                        for rank in range(2)
                    )
                )
            finally:
    ...
    def test_uniform_single_prefix_gradient_average_equals_full_trajectory(self):
        ...
        for step in range(3):
            ...
            metric = policy_gradient_step(
                ...
                replay_mode="sampled-prefix",
                replay_prefixes=1,
                replay_seed=seed_for_time(3, step),
            )
    ...
    def test_two_rank_prefix_normalization_and_locally_inactive_time(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            context = multiprocessing.get_context("spawn")
            processes = [
                context.Process(target=prefix_distributed_worker, args=(rank, init_file, temporary))
                for rank in range(2)
            ]
            try:
                for process in processes:
                ...
                for process in processes:
                    process.join(40)
                self.assertTrue(
                    all(not process.is_alive() for process in processes), "prefix collective hung"
                )
                self.assertEqual([process.exitcode for process in processes], [0, 0])
                self.assertTrue(
                    all(
