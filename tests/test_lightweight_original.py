...
from archlab.automodel.deepseek_v41_lightweight_original import read_reference, select_cases, summarize, training_ready
...
def fixture_cases():
    ...
    for subject in range(57):
        for index in range(4):
            result.append({'id':f'mmlu:{subject}:{index}', 'task':'mmlu', 'subject':str(subject)})
    for task, count in (('arc_challenge', 40), ('piqa', 80)):
        result.extend({'id':f'{task}:{index}', 'task':task, 'subject':None} for index in range(count))
    for row in result:
        row.update(exact_overlap_consumed_training=False, exact_overlap_planned_training=False,
...
def test_subset_is_small_subject_balanced_and_independent_of_scores_and_row_order():
    ...
    assert Counter(row['task'] for row in selected) == {'mmlu':114, 'arc_challenge':32, 'piqa':64}
...
def test_original_scorer_retains_the_existing_normalization_exactly():
    ...
    assert score_choices(case, {'normal':scores, 'simplicial':scores}) == {'normal':expected, 'simplicial':expected}
...
def test_training_gate_waits_for_real_updates_and_rejects_lost_headroom(tmp_path):
    ...
    (paths[0]/'MEMORY_QUALIFICATION.json').write_text(json.dumps({'passed':True,'minimum_driver_free_gib':63}))
...
def test_no_observed_regression_is_not_reported_as_certain_equivalence():
    ...
    rows = [{'task':'mmlu','original':result,'result':{'normal':result,'simplicial':result}} for _ in range(5)]
