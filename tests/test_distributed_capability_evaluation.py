...
from archlab.automodel.qwen.evaluate import continuation_scores, math_completion
from archlab.automodel.qwen.evaluate_distributed import (
    evaluation_rounds,
    restore_distributed_adapters,
)
from archlab.evaluation.capability import EvaluationConfig
...
def _collective_worker(rank, rendezvous):
    ...
    try:
        ...
        expected = continuation_scores(
            serial, tokenizer, "Q:", choices, max_context=20, device="cpu"
        )
        actual = continuation_scores(
            collective, tokenizer, "Q:", choices, max_context=20, device="cpu", synchronize=True
        )
        ...
        collective.calls = 0
        actual = math_completion(
            collective, tokenizer, "Q", config=config, eos_ids=eos, device="cpu", synchronize=True
        )
        for key in (
            "completion",
            "generated_token_ids",
...
class DistributedEvaluationTests(unittest.TestCase):
    def test_scoring_and_generation_preserve_version_counters_without_gradients(self):
        ...
        generated = math_completion(
            model,
            tokenizer,
            "Q:",
            config=EvaluationConfig(max_context=20, max_new_tokens=2),
            eos_ids={0},
            device="cpu",
        )
    ...
    def test_uneven_choices_eos_and_collective_nonfinite_failures(self):
        with tempfile.TemporaryDirectory(prefix="archlab-eval-collective-") as temporary:
            mp.spawn(
                _collective_worker, args=(f"file://{temporary}/rendezvous",), nprocs=2, join=True
            )
    ...
    def test_adapter_subset_rejects_mismatched_shapes(self):
        ...
        with tempfile.TemporaryDirectory(prefix="archlab-eval-weights-") as temporary:
            path = Path(temporary)
            expected = {
                name: {key: t.clone() for key, t in module.state_dict().items()}
                for name, module in adapters.items()
            }
            dcp.save(
                {"adapters": expected, "optimizer": {"unused": torch.ones(3)}},
...
def distributed_gpu_smoke():
    ...
    from archlab.automodel.qwen.checkpointing import state_digest
    from archlab.automodel.qwen.evaluate import added_modules
    from archlab.automodel.qwen.execution import build_frozen_base, tiny_config
    from archlab.automodel.qwen.runtime import configure_frozen_gdn_runtime
    ...
    try:
        ...
        model, mesh, precision = build_frozen_base(
            tiny_config(), tiny=True, checkpoint=None, ep_size=2, activation_checkpointing=False
        )
        ...
        for enabled in (False, True, False):
            with added_modules(model, enabled=enabled):
                result = continuation_scores(
                    model, tokenizer, "Q:", choices, max_context=32, synchronize=True
                )
                ...
                generated = math_completion(
                    ...
                    tokenizer,
                    "Q:",
                    config=EvaluationConfig(max_context=32, max_new_tokens=2),
                    eos_ids={0},
                    synchronize=True,
                )
        ...
        print(
            json.dumps(
                {
                    "event": "distributed_evaluation_gpu_smoke_pass",
                    "rank": dist.get_rank(),
