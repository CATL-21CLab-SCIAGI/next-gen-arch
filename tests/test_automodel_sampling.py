...
from archlab.automodel.qwen.sample import SamplingConfig, attach_cpu_lookup_hook, restore_adapters
...
class SamplingTests(unittest.TestCase):
    ...
    def test_adapter_subset_load_exact_and_fail_closed(self):
        ...
        saved = {
            name: {key: t.clone() for key, t in m.state_dict().items()}
            for name, m in adapters.items()
        }
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary)
            dcp.save(
                {"adapters": saved, "optimizer": {"unused": torch.ones(23)}},
    ...
    @unittest.skipUnless(
        importlib.util.find_spec("nemo_automodel"), "requires the pinned upstream loader"
    )
    def test_upstream_single_owner_base_loader_exact_roundtrip(self):
        ...
        from archlab.automodel.qwen.execution import tiny_config
        ...
        hf = {
            ...
            for name, value in model.state_dict_adapter.to_hf(model.state_dict()).items()
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            loader = CheckpointingConfig(
                checkpoint_dir="",
                model_repo_id=str(path),
    ...
    def test_original_ple_cpu_lookup_matches_gpu_bitwise(self):
        ...
        table = Qwen3_8_FlashNextEngramTableConfig(256, 160).build(
            process_group=None, dtype=torch.bfloat16
        )
        with torch.no_grad():
    ...
    @unittest.skipUnless(
        torch.cuda.is_available(), "requires GPU for FlexAttention dispatch parity"
    )
    def test_short_prefix_flex_unfused_matches_compiled(self):
        ...
        with torch.inference_mode():
            for length in (17, 129, 2048):
                q = torch.randn(
                    1, 24, length, 256, generator=generator, device="cuda", dtype=torch.bfloat16
                )
                k, v = [
                    torch.randn(
                        1, 2, length, 256, generator=generator, device="cuda", dtype=torch.bfloat16
                    )
                    for _ in range(2)
                ]
