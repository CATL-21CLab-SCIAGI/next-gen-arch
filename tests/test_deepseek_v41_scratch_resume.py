...
from archlab.automodel.deepseek_v41_scratch_resume import TILELANG_PARENT, maintenance_resume_changes


class MaintenanceResumeTests(unittest.TestCase):
    def setUp(self):
        self.parent = {'project_commit': TILELANG_PARENT, 'variant': 'normal',
                       'data_contract': {'sha256': 'sealed'}, 'peak_relative_learning_rate': .01,
                       'runtime': {'geometry': {'width': 640}, 'sparse_precision': {'implementation': 'v1'}},
                       'implementation_sha256': {'architecture.py': 'fixed', 'automodel/deepseek_v41_scratch_training.py': 'old'}}
        ...
        self.current['implementation_sha256']['automodel/deepseek_v41_sparse_qualification.py'] = 'probe'
    ...
    def test_training_and_architecture_changes_are_rejected(self):
        for mutation in ('data', 'variant', 'learning_rate', 'kernel', 'hash', 'inventory'):
            ...
            if mutation == 'data': value['data_contract']['sha256'] = 'different'
            if mutation == 'variant': value['variant'] = 'simplicial'
            if mutation == 'learning_rate': value['peak_relative_learning_rate'] = .001
            if mutation == 'kernel': value['runtime']['sparse_precision']['implementation'] = 'v2'
            if mutation == 'hash': value['implementation_sha256']['architecture.py'] = 'changed'
            if mutation == 'inventory': value['implementation_sha256']['surprise.py'] = 'extra'
