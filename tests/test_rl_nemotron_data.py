...
from archlab.rl.nemotron_data import audit_exposure, prepare, problem_key, select_records, sha256_file
...
class NemotronDataTest(unittest.TestCase):
    ...
    def row(self, problem, answer="2", uuid="", index=0):
        return {"problem": problem, "expected_answer": answer, "uuid": uuid,
    ...
    def select(self, rows, **kwargs):
        return select_records(rows, self.exposure(), train_size=1, heldout_size=1,
                              seed=123, canonicalizer=canonical_math_answer, **kwargs)
    ...
    def test_cross_mode_repeats_and_exposure_do_not_leak(self):
        rows = [self.row(" already\nseen "), self.row("new wording", uuid="old-uuid"),
                self.row("clean A", "0.5", index=2), self.row("clean A", "1/2", index=3),
    ...
    def test_conflicting_gold_and_uuid_aliases_are_discarded(self):
        rows = [self.row("conflict", "2"), self.row("conflict", "3"),
                self.row("alias a", uuid="same-id"), self.row("alias b", uuid="same-id"),
                self.row("valid a"), self.row("valid b")]
        train, heldout, stats = self.select(rows)
        self.assertEqual({x["prompt"][0]["content"] for x in train + heldout}, {"valid a", "valid b"})
    ...
    def test_order_does_not_change_split_or_duplicate_choice(self):
        ...
        self.assertEqual(next(x for x in first[0] + first[1] if x["prompt"][0]["content"] == "a")["source"]["row"], 1)
    ...
    def fixture(self, root):
        ...
        rows = [{"problem": f"question {i}", "expected_answer": str(i), "uuid": f"uuid-{i}",
                 "messages": "SECRET SOLUTION"} for i in range(8)]
        ...
        for split, index in (("train", 0), ("validation", 1)):
            ...
            with gzip.open(path, "wt") as stream:
                stream.write(json.dumps({"problem_sha256": problem_key(f"question {index}"), "uuid": f"uuid-{index}"}) + "\n")
            files.append({"name": path.name, "bytes": path.stat().st_size, "sha256": sha256_file(path)})
        ready = {"status": "complete", "contract_sha256": "contract",
                 "parts": [{"id": part_id, "documents": 2, "files": files}]}
        (old / "DATA_READY.json").write_text(json.dumps(ready))
        (old / "manifest.json").write_text(json.dumps({"sources": [{"path": str(data), "bytes": data.stat().st_size, "rows": 8}]}))
        pilot = root / "pilot"
        pilot.mkdir()
        (pilot / "windows.jsonl").write_text(json.dumps({
        ...
        spec = files[0]
        manifest = {"source": str(old), "source_ready_sha256": sha256_file(old / "DATA_READY.json"),
                    "source_contract_sha256": "contract", "split": "train", "supervised_tokens": 10,
                    "selected_conversations": 1, "windows_sha256": sha256_file(pilot / "windows.jsonl"),
    ...
    def test_entire_part_excluded_and_changed_sidecar_fails_closed(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(audit["problem_hashes"], {problem_key("question 0"), problem_key("question 1")})
    ...
    def test_real_parquet_projection_and_manifest(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            records = [json.loads(line) for name in ("train", "heldout")
            ...
            self.assertEqual(manifest["files"]["train"]["sha256"], sha256_file(output / "train.jsonl"))
