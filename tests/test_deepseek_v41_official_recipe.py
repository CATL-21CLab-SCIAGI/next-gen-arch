...
from archlab.automodel.deepseek_v41_official_recipe import (
    OfficialV41Config,
    admit_mesh,
    resolve_path,
)
...
def test_official_migration_preserves_the_existing_experiment_settings():
    ...
    existing = yaml.safe_load(
        (root / "recipes/experiments/deepseek_v41_simplicial_math_1b.yaml").read_text()
    )
    config = _config()
    for key in (
        "world_size",
...
def test_config_rejects_a_different_forward_or_checkpoint_precision():
    config = _config()
    for key, value in (
        ("upstream_commit", "stale"),
        ("linear_compute", "float32"),
        ("kv_index_quantization", "disabled"),
        ("dense_fsdp", 8),
    ):
...
def mesh_receipts(tmp_path):
    ...
    replicas = {
        ...
        "gradient_sha256": "same-gradient",
        "adapter_sha256": "same-adapter",
        "gradients": [{"name": "fixture", "nonzero": True}],
    }
    for rank in range(32):
        ...
        receipt = {
            ...
            "tiny": True,
            "context": 128,
            "automodel_commit": UPSTREAM_COMMIT,
            "adapter": {"backend": "deterministic"},
            "implementation_sha256": hashes,
            "unique_rank_windows": True,
            "mesh": {
                "ep_ranks": group,
                "ep_hosts": [f"node-{rank // 8}"] * 8,
            ...
            "checkpoint_replay": {
                ...
                "protocol_config": REPLAY_PROTOCOL,
                **replicas,
            },
            "frozen_base": {"unchanged": True, "no_gradients": True},
            "container_image": "fixture-container",
...
def test_mesh_admission_returns_all_rank_content_hashes(mesh_receipts):
    ...
    assert admit_mesh(path, hashes) == [
        sha256_file(path / f"rank{rank}.json") for rank in range(32)
    ]
...
def test_mesh_admission_requires_every_rank_to_have_passed_the_actual_contract(
    mesh_receipts, change
):
    path, hashes = mesh_receipts
...
@pytest.mark.parametrize(
    "marker", [{"passed": False, "world_size": 32}, {"passed": True, "world_size": 8}]
)
