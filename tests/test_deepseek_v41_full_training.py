...
class FullTrainingTests(unittest.TestCase):
    def test_hc_all_derivatives(self):
        torch.manual_seed(2)
        args = (
            torch.randn(2, 3, 24, requires_grad=True),
    ...
    def test_trainable_head_against_full_logits(self):
        ...
        for a, b in zip(
            torch.autograd.grad(got * 0.25, (hidden, weight)),
            torch.autograd.grad(want * 0.25, (hidden, weight)),
            strict=True,
        ):
            torch.testing.assert_close(a, b)
        self.assertEqual(
            float(
                TrainableHeadCrossEntropy.apply(hidden, torch.full_like(labels, -100), weight, 4)
            ),
            0,
        )
    ...
    def test_indexer_mask_and_teacher_detachment(self):
        ...
        mask[:, 0] = False
        mask[:, 1:, -1] = False
        loss = attention_indexer_kl(scores, query, keys, mask, scale=7**-0.5)
    ...
    def test_adafactor_matches_dense_equations(self):
        ...
        for shape in [(7,), (9, 7), (3, 9, 7)]:
            ...
            opt = ShardedAdafactor([p], lr=0.01, chunk_elements=14, stochastic_rounding=False)
            ...
            for step in (1, 2, 3):
                ...
                if len(shape) > 1:
                    rm, cm = (
                        grad.square().add(1e-30).mean(-1, keepdim=True),
                        grad.square().add(1e-30).mean(-2, keepdim=True),
                    )
                    ...
                    v = (
                        grad.square().add(1e-30) * beta
                        if v is None
                        else v.lerp(grad.square().add(1e-30), beta)
                    )
                    variance, axes = v, (-1,)
                update = grad / variance.clamp_min(1e-30).sqrt()
                rms = expected.square().mean(axes, keepdim=True).sqrt().clamp_min(1e-3)
                clip = update.square().mean(axes, keepdim=True).sqrt().clamp_min(1.0)
                expected = expected - 0.01 * rms * update / clip
                opt.step()
